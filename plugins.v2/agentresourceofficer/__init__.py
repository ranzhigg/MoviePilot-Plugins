...
class AgentResourceOfficer(_PluginBase):
    ...
    @staticmethod
    def _keyword_prefers_mp_pt_search(text: str) -> bool:
        raw = str(text or "").strip()
        if not raw:
            return False
        compact = re.sub(r"\s+", "", raw).lower()
        if re.search(r"\b(?:s\d{1,2}e\d{1,3}|e\d{1,3}|ep\d{1,3})\b", raw, flags=re.IGNORECASE):
            return True
        if re.search(r"第\s*\d+\s*(?:季|集|话|回|期)", raw):
            return True
        if re.search(r"更新(?:至)?\s*(?:ep|e)?\s*0*\d{1,3}", raw, flags=re.IGNORECASE):
            return True
        if re.search(r"更(?:至)?\s*0*\d{1,3}\s*集", raw, flags=re.IGNORECASE):
            return True
        return any(token in compact for token in ["全集", "全季", "完结", "短剧", "剧集", "season"])
    ...
    def _normalize_search_prefix(text: str) -> Tuple[str, str]:
        ...
        for pattern in ('^(?:mp|pt)\\s*搜索\\s*(.*)$', '^原生\\s*搜索\\s*(.*)$'):
            match = re.match(pattern, raw, flags=re.IGNORECASE)
            if match:
                return ('mp', match.group(1).strip())
        mappings = [('盘搜更新检查', 'pansou'), ('盘搜检查', 'pansou'), (), (), ('更新检查', 'smart'), ('更新搜索', 'smart'), ('查更新', 'smart'), ('更新', 'smart'), ('资源决策', 'smart_decision'), ('智能决策', 'smart_decision'), ('智能执行', 'smart_execute'), ('智能搜执行', 'smart_execute'), ('智能计划', 'smart_plan'), ('智能搜计划', 'smart_plan'), ('智能搜索', 'smart'), ('智能搜', 'smart'), ('MP搜索', 'mp'), ('MP 搜索', 'mp'), ('PT搜索', 'mp'), ('PT 搜索', 'mp'), ('pt搜索', 'mp'), ('pt 搜索', 'mp'), ('原生搜索', 'mp'), ('原生 搜索', 'mp'), ('搜索资源', 'pansou'), ('找资源', 'pansou'), ('搜索', 'search'), ('找', 'search'), ('1搜索', 'pansou'), ('2搜索',), (), ('yc',), ('2',), ('盘搜搜索', 'pansou'), ('盘搜', 'pansou'), ('ps', 'pansou'), ('1', 'pansou')]
    ...
    def _is_generic_search_command(text: str) -> bool:
        ...
        return any(
            ...
            for prefix in ["搜索资源", "找资源", "搜索", "找"]
        )

    @staticmethod
    def _is_explicit_pansou_command(text: str) -> bool:
        raw = str(text or "").strip()
        raw = re.sub(r"[\u00a0\u1680\u180e\u2000-\u200b\u202f\u205f\u3000]+", " ", raw)
        raw = re.sub(r"\s+", " ", raw).strip().lower()
        return any(
            raw == prefix or raw.startswith(prefix + " ")
            for prefix in ["盘搜搜索", "盘搜", "ps", "1搜索", "1"]
        )
    ...
    def _extract_mp_result_filter_intent(cls, text: str) -> Tuple[str, str]:
        ...
        for pattern in episode_patterns:
            ...
            cleaned = re.sub(r"(?:的|第)$", "", cleaned).strip()
    ...
    def _parse_pending_plan_numeric_confirmation(cls, value: Any) -> int:
        ...
        # "下载1" is treated as the current PT-list download command, not a saved-plan
        # confirmation, so it must not match here.
    ...
    @classmethod
    def _normalize_mp_recommend_short_action(cls, value: Any, *, state: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        current_state = dict(state or {})
        if cls._clean_text(current_state.get('kind')) != 'assistant_mp_recommend':
            return {}
        raw = cls._clean_text(value)
        patterns = [('^\\s*(?:决策|资源决策|智能决策)\\s*(\\d+)\\s*$', {'mode': 'smart_decision'}), ('^\\s*(?:详情|查看详情|看详情)\\s*(\\d+)\\s*$', {'action': 'detail'}), ('^\\s*(?:计划|生成计划|先计划)\\s*(\\d+)\\s*$', {'action': 'plan'}), ('^\\s*(?:确认|执行|直接执行)\\s*(\\d+)\\s*$', {'mode': 'smart_execute'}), ('^\\s*(?:盘搜|ps)\\s*(\\d+)\\s*$', {'mode': 'pansou'}), ({},), ('^\\s*(?:原生|mp|pt)\\s*(\\d+)\\s*$', {'mode': 'mp'})]
        ...
        if selected_index <= 0:
            ...
            if items:
                selected_index = 1
        if selected_index > 0:
            no_index_aliases = {'详情': {'action': 'detail'}, '查看详情': {'action': 'detail'}, '看详情': {'action': 'detail'}, '决策': {'mode': 'smart_decision'}, '资源决策': {'mode': 'smart_decision'}, '智能决策': {'mode': 'smart_decision'}, '计划': {'action': 'plan'}, '生成计划': {'action': 'plan'}, '先计划': {'action': 'plan'}, '确认': {'mode': 'smart_execute'}, '执行': {'mode': 'smart_execute'}, '直接执行': {'mode': 'smart_execute'}, '盘搜': {'mode': 'pansou'}, 'ps': {'mode': 'pansou'}, 'yc': {}, '原生': {'mode': 'mp'}, 'mp': {'mode': 'mp'}, 'pt': {'mode': 'mp'}}
    ...
    @classmethod
    def _normalize_recommend_handoff_action(cls, value: Any, *, state: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        current_state = dict(state or {})
        kind = cls._clean_text(current_state.get('kind'))
        handoff = current_state.get('recommend_handoff')
        if not isinstance(handoff, dict) or not handoff:
            return {}
        compact = re.sub('[\\s，。？！!?,、:：]+', '', cls._clean_text(value)).lower()
        if compact not in {'回推荐', '回榜单', '返回推荐', '返回榜单', '推荐', '榜单'}:
            switch_mode_aliases = {'盘搜': 'pansou', 'ps': 'pansou', '原生': 'mp', 'mp': 'mp'}
            switch_mode = switch_mode_aliases.get(compact)
            if not switch_mode:
                return {}
            return {'action': 'switch_recommend_handoff_source', 'mode': switch_mode}
        return {'action': 'return_to_recommend'}

    @classmethod
    def _normalize_recommend_handoff_short_action(cls, value: Any, *, state: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        current_state = dict(state or {})
        kind = cls._clean_text(current_state.get('kind'))
        handoff = current_state.get('recommend_handoff')
        if not isinstance(handoff, dict) or not handoff:
    ...
    @classmethod
    def _normalize_recommend_source_compound_action(cls, value: Any, *, state: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        current_state = dict(state or {})
        kind = cls._clean_text(current_state.get('kind'))
        compact = re.sub('[\\s，。？！!?,、:：]+', '', cls._clean_text(value)).lower()
        source_aliases = {'盘搜': 'pansou', 'ps': 'pansou', '原生': 'mp', 'mp': 'mp'}
        action_aliases = {'详情': 'best', '看详情': 'best', '计划': 'best_plan', '确认': 'best_execute', '执行': 'best_execute'}
        for alias, mode in source_aliases.items():
            ...
            if not followup_action:
                continue
            return {'action': 'recommend_source_compound', 'mode': mode, 'followup_action': followup_action}
        return {}
    ...
    def _normalize_mp_recommend_direct_intent(cls, value: Any) -> Dict[str, Any]:
        ...
        if not remain:
            return {}
        suffix_aliases: List[Tuple[str, Dict[str, str]]] = [('资源决策', {'mode': 'smart_decision'}), ('智能决策', {'mode': 'smart_decision'}), ('查看详情', {'action': 'detail'}), ('看详情', {'action': 'detail'}), ('确认执行', {'mode': 'smart_execute'}), ('直接执行', {'mode': 'smart_execute'}), ('生成计划', {'action': 'plan'}), ('先计划', {'action': 'plan'}), ('详情', {'action': 'detail'}), ('决策', {'mode': 'smart_decision'}), ('计划', {'action': 'plan'}), ('确认', {'mode': 'smart_execute'}), ('执行', {'mode': 'smart_execute'}), ('盘搜', {'mode': 'pansou'}), ({},), ('原生', {'mode': 'mp'})]
        for suffix, action_payload in suffix_aliases:
            ...
            if not keyword:
                return {}
            return {'keyword': keyword, 'index': 1}
        return {}
    ...
    def _normalize_pick_mode(value: Any) -> str:
        ...
        if re.search('(^|[^a-z])smart($|[^a-z])', text):
            return 'smart_decision'
        if any((token in compact for token in ['pansou', '盘搜', '走盘搜', '用盘搜'])):
            return 'pansou'
        if re.search('(^|[^a-z])ps($|[^a-z])', text):
            return 'pansou'
    ...
    def init_plugin(self, config: Optional[Dict[str, Any]]=None):
        ...
        self._pansou_enabled = bool(config.get('pansou_enabled', True))
        ...
        self._mp_pt_enabled = bool(config.get('mp_pt_enabled', True))
        self._mp_download_save_path = self._clean_text(config.get('mp_download_save_path'))
        ...
        self._feishu_command_whitelist = FeishuChannel.merge_command_whitelist(FeishuChannel.split_commands(config.get('feishu_command_whitelist')))
        self._feishu_command_aliases = FeishuChannel.merge_command_aliases(self._clean_text(config.get('feishu_command_aliases')))
        self._feishu_command_mode = self._clean_text(config.get('feishu_command_mode') or 'resource_officer')
        self._quark_service = QuarkTransferService(cookie=self._quark_cookie, timeout=self._quark_timeout, default_target_path=self._quark_default_path, auto_import_cookiecloud=self._quark_auto_import_cookiecloud, cookie_refresh_callback=self._refresh_quark_cookie_from_cookiecloud)
        self._p115_service = P115TransferService(default_target_path=self._p115_default_path, cookie=self._p115_cookie, prefer_direct=self._p115_prefer_direct)
    ...
    def get_agent_tools(self) -> List[type]:
        return [AssistantRouteTool, AssistantPickTool, AssistantWorkflowTool, ShareRouteTool, P115QRCodeStartTool, P115QRCodeCheckTool, P115StatusTool, P115PendingTool, P115ResumePendingTool, P115CancelPendingTool]
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        return [{'path': '/config/get', 'endpoint': self.api_config_get, 'methods': ['GET'], 'auth': 'bear', 'summary': '设置页获取 Agent影视助手 当前配置'}, {'path': '/config/save', 'endpoint': self.api_config_save, 'methods': ['POST'], 'auth': 'bear', 'summary': '设置页保存 Agent影视助手 当前配置'}, {'path': '/quark/health', 'endpoint': self.api_quark_health, 'methods': ['GET'], 'summary': '检查 Agent影视助手 的夸克配置'}, {'path': '/quark/transfer', 'endpoint': self.api_quark_transfer, 'methods': ['POST'], 'summary': '通过 Agent影视助手 执行夸克分享转存'}, {'path': '/p115/health', 'endpoint': self.api_p115_health, 'methods': ['GET'], 'summary': '检查 Agent影视助手 的 115 转存依赖状态'}, {'path': '/p115/qrcode', 'endpoint': self.api_p115_qrcode, 'methods': ['GET'], 'summary': '获取 Agent影视助手 的 115 扫码登录二维码'}, {'path': '/p115/qrcode/check', 'endpoint': self.api_p115_qrcode_check, 'methods': ['GET'], 'summary': '检查 Agent影视助手 的 115 扫码登录状态'}, {'path': '/p115/ui/health', 'endpoint': self.api_p115_ui_health, 'methods': ['GET'], 'auth': 'bear', 'summary': '设置页检查 Agent影视助手 的 115 转存依赖状态'}, {'path': '/p115/ui/qrcode', 'endpoint': self.api_p115_ui_qrcode, 'methods': ['GET'], 'auth': 'bear', 'summary': '设置页获取 Agent影视助手 的 115 扫码登录二维码'}, {'path': '/p115/ui/qrcode/check', 'endpoint': self.api_p115_ui_qrcode_check, 'methods': ['GET'], 'auth': 'bear', 'summary': '设置页检查 Agent影视助手 的 115 扫码登录状态'}, {'path': '/p115/qrcode/page', 'endpoint': self.api_p115_qrcode_page, 'methods': ['GET'], 'summary': '打开 Agent影视助手 的 115 扫码登录网页'}, {'path': '/p115/qrcode/page/check', 'endpoint': self.api_p115_qrcode_page_check, 'methods': ['GET'], 'summary': '检查 Agent影视助手 的 115 扫码登录网页会话'}, {'path': '/p115/qrcode/page/refresh', 'endpoint': self.api_p115_qrcode_page_refresh, 'methods': ['GET'], 'summary': '就地重新生成 Agent影视助手 的 115 扫码登录二维码'}, {'path': '/p115/transfer', 'endpoint': self.api_p115_transfer, 'methods': ['POST'], 'summary': '通过 Agent影视助手 执行 115 分享转存'}, {'path': '/p115/pending', 'endpoint': self.api_p115_pending, 'methods': ['GET', 'POST'], 'summary': '查看指定会话中待继续的 115 任务'}, {'path': '/p115/pending/resume', 'endpoint': self.api_p115_pending_resume, 'methods': ['POST'], 'summary': '继续执行指定会话中待处理的 115 任务'}, {'path': '/p115/pending/cancel', 'endpoint': self.api_p115_pending_cancel, 'methods': ['POST'], 'summary': '取消指定会话中待处理的 115 任务'}, {'path': '/share/route', 'endpoint': self.api_share_route, 'methods': ['POST'], 'summary': '通过 Agent影视助手 自动识别 115 / 夸克分享链接并执行对应转存'}, {'path': '/feishu/health', 'endpoint': self.api_feishu_health, 'methods': ['GET'], 'summary': '检查 Agent影视助手 内置飞书入口状态'}, {'path': '/assistant/route', 'endpoint': self.api_assistant_route, 'methods': ['POST']}, {'path': '/assistant/pick', 'endpoint': self.api_assistant_pick, 'methods': ['POST'], 'summary': '统一智能入口的按编号继续执行'}, {'path': '/assistant/capabilities', 'endpoint': self.api_assistant_capabilities, 'methods': ['GET'], 'summary': '查看统一智能入口支持的结构化参数、默认值与推荐调用方式'}, {'path': '/assistant/readiness', 'endpoint': self.api_assistant_readiness, 'methods': ['GET'], 'summary': '检查 Agent影视助手 是否已准备好给外部智能体调用'}, {'path': '/assistant/cookie/update', 'endpoint': self.api_assistant_cookie_update, 'methods': ['POST']}, {'path': '/assistant/pulse', 'endpoint': self.api_assistant_pulse, 'methods': ['GET'], 'summary': '轻量启动探针：返回版本、关键服务状态、警告和最佳恢复建议'}, {'path': '/assistant/startup', 'endpoint': self.api_assistant_startup, 'methods': ['GET'], 'summary': '启动聚合包：一次返回 pulse、自检、核心工具、端点、默认目录和恢复建议'}, {'path': '/assistant/maintain', 'endpoint': self.api_assistant_maintain, 'methods': ['GET', 'POST'], 'summary': '低风险维护：查看或执行过期会话、已执行计划清理'}, {'path': '/assistant/toolbox', 'endpoint': self.api_assistant_toolbox, 'methods': ['GET'], 'summary': '轻量工具清单：返回推荐工具、端点、工作流、动作名、默认目录和命令示例'}, {'path': '/assistant/request_templates', 'endpoint': self.api_assistant_request_templates, 'methods': ['GET', 'POST'], 'summary': '轻量请求模板：返回外部智能体常用 assistant 请求模板'}, {'path': '/assistant/selfcheck', 'endpoint': self.api_assistant_selfcheck, 'methods': ['GET'], 'summary': '轻量自检：确认 compact 协议、模板默认 compact 和布尔字符串解析是否正常'}, {'path': '/assistant/history', 'endpoint': self.api_assistant_history, 'methods': ['GET'], 'summary': '查看最近执行历史，便于外部智能体判断上一步是否完成'}, {'path': '/assistant/action', 'endpoint': self.api_assistant_action, 'methods': ['POST'], 'summary': '直接执行统一智能入口返回的动作模板名，适合外部智能体无映射继续执行'}, {'path': '/assistant/actions', 'endpoint': self.api_assistant_actions, 'methods': ['POST'], 'summary': '批量执行多个动作模板，适合外部智能体一次请求串起多步工作流，减少往返'}, {'path': '/assistant/workflow', 'endpoint': self.api_assistant_workflow, 'methods': ['GET', 'POST'], 'summary': '运行预设工作流，适合外部智能体用更短参数完成常见资源任务'}, {'path': '/assistant/preferences', 'endpoint': self.api_assistant_preferences, 'methods': ['GET', 'POST', 'DELETE'], 'summary': '读取、保存或重置智能体片源偏好画像，用于云盘和 PT 分源评分'}, {'path': '/assistant/plan/execute', 'endpoint': self.api_assistant_plan_execute, 'methods': ['POST'], 'summary': '执行 dry_run 保存的工作流计划，避免外部智能体重复携带大 JSON'}, {'path': '/assistant/plans', 'endpoint': self.api_assistant_plans, 'methods': ['GET'], 'summary': '查看 dry_run 保存的工作流计划，便于断线恢复和选择 plan_id'}, {'path': '/assistant/plans/clear', 'endpoint': self.api_assistant_plans_clear, 'methods': ['POST'], 'summary': '清理 dry_run 保存的工作流计划'}, {'path': '/assistant/recover', 'endpoint': self.api_assistant_recover, 'methods': ['GET', 'POST'], 'summary': '查看或直接执行当前最推荐的恢复动作，给外部智能体提供单入口续跑能力'}, {'path': '/assistant/session', 'endpoint': self.api_assistant_session_state, 'methods': ['GET', 'POST'], 'summary': '查看统一智能入口当前会话状态与建议动作'}, {'path': '/assistant/session/clear', 'endpoint': self.api_assistant_session_clear, 'methods': ['POST'], 'summary': '清理统一智能入口当前会话缓存'}, {'path': '/assistant/sessions', 'endpoint': self.api_assistant_sessions, 'methods': ['GET'], 'summary': '列出当前活跃的统一智能入口会话，便于外部智能体恢复和接续'}, {'path': '/assistant/sessions/clear', 'endpoint': self.api_assistant_sessions_clear, 'methods': ['POST'], 'summary': '按 session_id、类型或过滤条件批量清理统一智能入口会话'}]
    ...
    def get_page(self) -> List[dict]:
        ...
        def text_line(text: str, css_class: str='text-body-2 py-1') -> Dict[str, Any]:
            return {'component': 'div', 'props': {'class': css_class}, 'text': text}

        def status_card(title: str, subtitle: str, lines: List[str], color: str='primary', actions: Optional[List[Dict[str, Any]]]=None) -> Dict[str, Any]:
            content = [{'component': 'VCardTitle', 'props': {'class': 'text-subtitle-1 font-weight-bold pb-1'}, 'text': title}, {'component': 'VCardSubtitle', 'props': {'class': 'text-body-2'}, 'text': subtitle}, {'component': 'VCardText', 'props': {'class': 'py-2'}, 'content': [text_line(line, 'text-body-2 py-0') for line in lines]}]
            if actions:
                content.append({'component': 'VCardActions', 'props': {'class': 'pt-0'}, 'content': actions})
            return {'component': 'VCard', 'props': {'variant': 'tonal', 'color': color, 'class': 'h-100'}, 'content': content}
        return [{'component': 'VContainer', 'props': {'fluid': True, 'class': 'pa-0'}, 'content': [{'component': 'VRow', 'props': {'dense': True}, 'content': [{'component': 'VCol', 'props': {'cols': 12, 'md': 3}, 'content': [status_card('115', '可用' if p115_health_ok else '待修复', [f'默认目录：{self._p115_default_path}', f'登录方式：{p115_ready}', f'扫码客户端：{self._p115_client_type_title(self._p115_client_type)}', f'Cookie：{p115_cookie_message}'], 'success' if p115_health_ok else 'error', [{'component': 'VBtn', 'props': {'href': self._p115_qrcode_page_url(), 'target': '_blank', 'color': 'primary', 'variant': 'tonal', 'size': 'small'}, 'text': '选择扫码途径'}])]}, {'component': 'VCol', 'props': {'cols': 12, 'md': 3}, 'content': [status_card('夸克', quark_ready, [f'默认目录：{self._quark_default_path}', '分享转存：' + ('可用' if self._quark_cookie else '待配置')], 'success' if self._quark_cookie else 'warning')]}, {'component': 'VCol', 'props': {'cols': 12, 'md': 3}, 'content': [status_card('飞书', f'{feishu_state}，长连接：{feishu_running}', ['模式：内置 Channel'], 'success' if feishu_health.get('running') else 'secondary')]}]}]}]
        ...
        form = [
            {
                ...
                "content": [
                    ...
                    {"component": "VRow", "content": [{"component": "VCol", "props": {"cols": 12}, "content": [{"component": "VAlert", "props": {"type": "info", "variant": "tonal", "text": "资源入口已收敛到盘搜、115、夸克与 MoviePilot 原生搜索。"}}]}]},
                    {"component": "VRow", "content": [
                        {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VSwitch", "props": {"model": "mp_pt_enabled", "label": "启用 MP/PT 原生搜索/下载"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VSwitch", "props": {"model": "pansou_enabled", "label": "启用盘搜"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VTextField", "props": {"model": "pansou_timeout", "label": "盘搜超时(秒)", "type": "number"}}]},
                    ]},
                    {"component": "VRow", "content": [
                        {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "assistant_default_pt_min_seeders", "label": "PT 最低做种数", "type": "number"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "assistant_default_confirm_score_threshold", "label": "建议确认分数线", "type": "number"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "assistant_default_auto_ingest_score_threshold", "label": "自动入库分数线", "type": "number"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "assistant_default_auto_ingest_enabled", "label": "允许高分自动入库"}}]},
                    ]},
                    {"component": "VRow", "content": [
                        {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VTextField", "props": {"model": "pansou_base_url", "label": "盘搜 API 地址", "placeholder": "http://host.docker.internal:805"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VTextField", "props": {"model": "mp_download_save_path", "label": "PT 下载保存路径（可选）"}}]},
                    ]},
                    {"component": "VRow", "content": [
                        {"component": "VCol", "props": {"cols": 12}, "content": [{"component": "VTextarea", "props": {"model": "quark_cookie", "label": "夸克 Cookie", "rows": 3}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VTextField", "props": {"model": "quark_default_path", "label": "夸克默认目录"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 2}, "content": [{"component": "VTextField", "props": {"model": "quark_timeout", "label": "夸克超时(秒)", "type": "number"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "quark_auto_import_cookiecloud", "label": "自动刷新 Cookie"}}]},
                    ]},
                    {"component": "VRow", "content": [
                        {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "p115_default_path", "label": "115 默认目录"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VSelect", "props": {"model": "p115_client_type", "label": "115 扫码客户端", "items": self._p115_client_type_items()}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 2}, "content": [{"component": "VSwitch", "props": {"model": "p115_prefer_direct", "label": "115 优先直转"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "p115_cookie", "label": "115 Cookie", "type": "password"}}]},
                    ]},
                    {"component": "VRow", "content": [
                        {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "feishu_enabled", "label": "启用内置飞书入口"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "feishu_allow_all", "label": "允许所有飞书会话"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "feishu_reply_enabled", "label": "发送飞书回复"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSelect", "props": {"model": "feishu_reply_receive_id_type", "label": "回复 ID 类型", "items": [{"title": "群聊 chat_id", "value": "chat_id"}, {"title": "用户 open_id", "value": "open_id"}]}}]},
                    ]},
                    {"component": "VRow", "content": [
                        {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VTextField", "props": {"model": "feishu_app_id", "label": "飞书 App ID"}}]},
                        {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VTextField", "props": {"model": "feishu_app_secret", "label": "飞书 App Secret", "type": "password"}}]},
                        {"component": "VCol", "props": {"cols": 12}, "content": [{"component": "VTextarea", "props": {"model": "feishu_allowed_chat_ids", "label": "允许的群聊 Chat ID", "rows": 2}}]},
                        {"component": "VCol", "props": {"cols": 12}, "content": [{"component": "VTextarea", "props": {"model": "feishu_allowed_user_ids", "label": "允许的用户 Open ID", "rows": 2}}]},
                        {"component": "VCol", "props": {"cols": 12}, "content": [{"component": "VTextarea", "props": {"model": "feishu_command_whitelist", "label": "飞书命令白名单", "rows": 2}}]},
                        {"component": "VCol", "props": {"cols": 12}, "content": [{"component": "VTextarea", "props": {"model": "feishu_command_aliases", "label": "飞书命令别名", "rows": 3, "placeholder": FeishuChannel.default_command_aliases()}}]},
                    ]},
                ],
            },
        ]
    ...
    def _pansou_disabled_response(self) -> Dict[str, Any]:
        return {
            "success": False,
            "message": "盘搜入口已关闭：当前不会执行盘搜搜索或盘搜补查。可在插件设置中开启“启用盘搜”。",
            "data": {
                "provider": "pansou",
                "resource_enabled": False,
                "error_code": "pansou_disabled",
            },
        }

    def _ensure_pansou_enabled(self) -> Tuple[bool, Dict[str, Any]]:
        if self._pansou_enabled:
            return True, {}
        return False, self._pansou_disabled_response()

    def _mp_pt_disabled_response(self) -> Dict[str, Any]:
        return {
            "success": False,
            "message": "MP/PT 原生搜索入口已关闭：当前不会执行 MP搜索、PT搜索、下载或原生补查。可在插件设置中开启“启用 MP/PT 原生搜索/下载”。",
            "data": {
                "provider": "mp_pt",
                "resource_enabled": False,
                "error_code": "mp_pt_disabled",
            },
        }

    def _ensure_mp_pt_enabled(self) -> Tuple[bool, Dict[str, Any]]:
        if self._mp_pt_enabled:
            return True, {}
        return False, self._mp_pt_disabled_response()

    def _cloud_sources_disabled_response(self) -> Dict[str, Any]:
        return {'success': False, 'data': {'error_code': 'cloud_sources_disabled', 'pansou_enabled': self._pansou_enabled}}

    def _cloud_no_result_suggestion_response(
        self,
        *,
        keyword: str,
        session: str,
        primary_source: str,
        checked_fallback: bool = False,
        fallback_source_enabled: bool = False,
        mp_pt_enabled: bool = True,
        detail: str = "",
    ) -> Dict[str, Any]:
        clean_keyword = self._clean_text(keyword)
        lines = [f"{primary_source}暂无结果：{clean_keyword}"]
        if checked_fallback:
            lines.append("已完成可用云盘来源检查，暂时没有可用结果。")
        elif not fallback_source_enabled and primary_source == "盘搜":
            lines.append("没有其他云盘来源可继续补查。")
        if detail:
            lines.append(f"原因：{detail}")
        next_command = f"PT搜索 {clean_keyword}" if clean_keyword else "PT搜索 片名"
        if mp_pt_enabled:
            lines.append(f"如果想继续扩搜，可以回复：{next_command}")
        else:
            lines.append("MP/PT 原生搜索也已关闭；如需继续扩搜，请先在插件设置中开启。")
        return {
            "success": False,
            "message": "\n".join(line for line in lines if line).strip(),
            "data": self._assistant_response_data(
                session=session,
                data={
                    "action": "cloud_source_no_result",
                    "ok": False,
                    "keyword": clean_keyword,
                    "primary_source": primary_source,
                    "checked_fallback": checked_fallback,
                    "fallback_source_enabled": fallback_source_enabled,
                    "suggested_command": next_command if mp_pt_enabled else "",
                    "decision_summary": {
                        "stage": "cloud_no_result",
                        "preferred_command": next_command if mp_pt_enabled else "",
                        "fallback_command": "",
                        "compact_commands": [next_command] if mp_pt_enabled else [],
                        "recommended_agent_behavior": "ask_user",
    ...
    def _all_search_sources_disabled_response(self) -> Dict[str, Any]:
        return {'success': False, 'data': {'error_code': 'all_search_sources_disabled', 'pansou_enabled': self._pansou_enabled, 'mp_pt_enabled': self._mp_pt_enabled}}


    @staticmethod
    def _resource_has_free_marker(item: Optional[Dict[str, Any]]) -> bool:
        if not isinstance(item, dict) or not item:
            return False
        for key in ["unlock_points", "cost", "points", "price", "point_text"]:
            text = str(item.get(key) or "").strip().lower()
            if text in {"0", "free", "免费", "0分"}:
                return True
        blob = " ".join(
            str(item.get(key) or "")
            for key in [
        ...
        return "免费" in blob or " free " in f" {blob} "
    ...
    def _resource_points_value(item: Optional[Dict[str, Any]]) -> Optional[int]:
        ...
        text = str(raw or "").strip()
        if not text:
            return 0 if AgentResourceOfficer._resource_has_free_marker(item) else None
    ...
    def _default_assistant_preferences(self) -> Dict[str, Any]:
        return {'schema_version': 'preferences.v1', 'initialized': False, 'prefer_resolution': '4K', 'prefer_dolby_vision': True, 'prefer_hdr': True, 'prefer_chinese_subtitle': True, 'prefer_complete_series': True, 'prefer_cloud_provider': '', 'enable_pansou': True, 'enable_mp_pt': True, 'has_quark': True, 'has_115': True, 'quark_default_path': self._quark_default_path, 'p115_default_path': self._p115_default_path, 'pt_require_free': False, 'pt_min_seeders': self._assistant_default_pt_min_seeders, 'pt_prefer_free': True, 'auto_ingest_enabled': self._assistant_default_auto_ingest_enabled, 'auto_ingest_score_threshold': self._assistant_default_auto_ingest_score_threshold, 'confirm_score_threshold': self._assistant_default_confirm_score_threshold, 'updated_at': 0}
    ...
    def _normalize_assistant_preferences(self, value: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        ...
        if 'subtitle_priority' in payload and 'prefer_chinese_subtitle' not in payload:
            ...
            payload['prefer_chinese_subtitle'] = bool(re.search('中文|中字|简中|繁中|双语|chinese', subtitle_text, flags=re.IGNORECASE))
        ...
        if not self._pansou_enabled:
            normalized['enable_pansou'] = False
        if not self._mp_pt_enabled:
            normalized['enable_mp_pt'] = False
        ...
        normalized['cloud_default_path'] = self._normalize_path(normalized.get('cloud_default_path') or self._p115_default_path)
    ...
    def _assistant_preferences_public_data(self, *, session: str='', user_key: str='') -> Dict[str, Any]:
        ...
        questions = ['你更偏好 4K 还是 1080P？', '是否优先杜比视界 / HDR？', '是否必须中文字幕？', '电视剧是否优先全集或完整季？', '你可用哪些云盘？例如：只有夸克、只有 115、两者都可。', 'PT 资源最低做种数是多少？默认 3。', '是否允许 90 分以上资源自动入库？默认关闭。']
        return {'schema_version': 'preferences.v1', 'key': key, 'initialized': bool(preferences.get('initialized')), 'preferences': preferences, 'needs_onboarding': not bool(preferences.get('initialized')), 'onboarding_questions': questions if not bool(preferences.get('initialized')) else [], 'defaults': self._default_assistant_preferences()}
    ...
    def _assistant_preferences_status_brief(self, *, session: str='', user_key: str='') -> Dict[str, Any]:
        ...
        brief = {'key': data.get('key'), 'initialized': bool(data.get('initialized')), 'needs_onboarding': bool(data.get('needs_onboarding')), 'summary': {'prefer_resolution': self._clean_text(prefs.get('prefer_resolution')), 'prefer_dolby_vision': bool(prefs.get('prefer_dolby_vision')), 'prefer_hdr': bool(prefs.get('prefer_hdr')), 'prefer_chinese_subtitle': bool(prefs.get('prefer_chinese_subtitle')), 'prefer_complete_series': bool(prefs.get('prefer_complete_series')), 'prefer_cloud_provider': self._clean_text(prefs.get('prefer_cloud_provider')), 'enable_pansou': bool(prefs.get('enable_pansou')), 'enable_mp_pt': bool(prefs.get('enable_mp_pt')), 'has_quark': bool(prefs.get('has_quark')), 'has_115': bool(prefs.get('has_115')), 'pt_min_seeders': self._safe_int(prefs.get('pt_min_seeders'), 3), 'pt_require_free': bool(prefs.get('pt_require_free')), 'auto_ingest_enabled': bool(prefs.get('auto_ingest_enabled')), 'auto_ingest_score_threshold': self._safe_int(prefs.get('auto_ingest_score_threshold'), 90)}}
    ...
    def _assistant_default_preferences_template(self) -> Dict[str, Any]:
        ...
        return {'prefer_resolution': prefs.get('prefer_resolution'), 'prefer_dolby_vision': prefs.get('prefer_dolby_vision'), 'prefer_hdr': prefs.get('prefer_hdr'), 'prefer_chinese_subtitle': prefs.get('prefer_chinese_subtitle'), 'prefer_complete_series': prefs.get('prefer_complete_series'), 'prefer_cloud_provider': prefs.get('prefer_cloud_provider'), 'enable_pansou': prefs.get('enable_pansou'), 'enable_mp_pt': prefs.get('enable_mp_pt'), 'has_quark': prefs.get('has_quark'), 'has_115': prefs.get('has_115'), 'pt_require_free': prefs.get('pt_require_free'), 'pt_min_seeders': prefs.get('pt_min_seeders'), 'p115_default_path': prefs.get('p115_default_path'), 'quark_default_path': prefs.get('quark_default_path'), 'auto_ingest_enabled': prefs.get('auto_ingest_enabled'), 'auto_ingest_score_threshold': prefs.get('auto_ingest_score_threshold')}
    ...
    def _assistant_enabled_cloud_source_order(
        self,
        *,
        session: str = "",
        source_order: Optional[List[str]] = None,
        ...
        preferences = self._assistant_smart_merge_session_preferences(
            self._assistant_preferences_for_session(session=session),
            session_overrides=session_overrides,
        )
        return self._assistant_smart_search_source_order(
            preferences,
            source_order=source_order or ["pansou"],
        )

    def _assistant_transfer_source_order(self, *, preferences: Optional[Dict[str, Any]], prefer_provider: str='', requested_source_order: Optional[List[str]]=None) -> List[str]:
        prefs = self._normalize_assistant_preferences(preferences)
        pansou_enabled = self._assistant_source_enabled(prefs, 'pansou')
        provider = self._clean_text(prefer_provider).lower()
        if provider == 'quark':
            return ['pansou'] if pansou_enabled else []
        if provider == '115':
            return ['pansou'] if pansou_enabled else []
        return self._assistant_smart_search_source_order(prefs, source_order=requested_source_order or ['pansou'])
    ...
    def _assistant_smart_search_source_order(self, preferences: Optional[Dict[str, Any]], source_order: Optional[List[str]]=None) -> List[str]:
        prefs = self._normalize_assistant_preferences(preferences)
        requested = [self._clean_text(item).lower() for item in source_order or [] if self._clean_text(item)]
        order = requested or ['pansou', 'mp_pt']
        normalized: List[str] = []
        for item in order:
            if item in {'mp', 'pt'}:
                item = 'mp_pt'
    ...
    def _assistant_smart_source_availability(self, preferences: Optional[Dict[str, Any]], *, source_order: Optional[List[str]]=None) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        prefs = self._normalize_assistant_preferences(preferences)
        ...
        source_labels = {'pansou': '盘搜', 'mp_pt': 'MP/PT'}
    ...
    def _assistant_smart_apply_session_override(self, session_overrides: Optional[Dict[str, Any]], *, adjust_action: str, source_order: Optional[List[str]]=None) -> Tuple[Dict[str, Any], List[str]]:
    ...
    def _parse_assistant_preferences_text(self, text: str) -> Dict[str, Any]:
        ...
        compact = re.sub('\\s+', '', raw).lower()
        payload: Dict[str, Any] = {}
        if re.search('1080|fhd', raw, flags=re.IGNORECASE):
            payload['prefer_resolution'] = '1080P'
        elif re.search('4k|2160|uhd', raw, flags=re.IGNORECASE):
            payload['prefer_resolution'] = '4K'
        if re.search('(不要|不需要|关闭|禁用).{0,4}(杜比|dv)', raw, flags=re.IGNORECASE):
            payload['prefer_dolby_vision'] = False
        elif re.search('杜比|dv|dolby', raw, flags=re.IGNORECASE):
            payload['prefer_dolby_vision'] = True
        if re.search('(不要|不需要|关闭|禁用).{0,4}hdr', raw, flags=re.IGNORECASE):
            payload['prefer_hdr'] = False
        elif re.search('hdr', raw, flags=re.IGNORECASE):
            payload['prefer_hdr'] = True
        if re.search('(不要|不需要|关闭|禁用).{0,6}(中字|中文|字幕)', raw, flags=re.IGNORECASE) or re.search('无字幕也可|字幕无所谓', raw):
            payload['prefer_chinese_subtitle'] = False
        elif re.search('中字|中文|简中|繁中|双语字幕|字幕', raw, flags=re.IGNORECASE):
            payload['prefer_chinese_subtitle'] = True
        if re.search('不强求全集|不要全集|单集也可|更新也可', raw):
            payload['prefer_complete_series'] = False
        elif re.search('全集|完整季|整季|完结', raw):
            payload['prefer_complete_series'] = True
        if re.search('夸克优先|优先夸克|quark', raw, flags=re.IGNORECASE):
            payload['prefer_cloud_provider'] = 'quark'
        elif re.search('115优先|优先115', raw):
            payload['prefer_cloud_provider'] = '115'
        if re.search('只用盘搜|仅用盘搜|只搜盘搜', raw):
            payload['enable_pansou'] = True
            payload['enable_mp_pt'] = False
        elif re.search('只用原生|只用pt|仅用pt|只搜pt|只搜原生|只要原生', raw, flags=re.IGNORECASE):
            payload['enable_pansou'] = False
            payload['enable_mp_pt'] = True
        if re.search('(不要|不用|关闭|禁用).{0,4}盘搜', raw):
            payload['enable_pansou'] = False
        elif re.search('(启用|使用|打开).{0,4}盘搜', raw):
            payload['enable_pansou'] = True
        if re.search('(不要|不用|关闭|禁用).{0,4}(pt|原生)', raw, flags=re.IGNORECASE):
            payload['enable_mp_pt'] = False
        elif re.search('(启用|使用|打开).{0,4}(pt|原生)', raw, flags=re.IGNORECASE):
            payload['enable_mp_pt'] = True
        if re.search('只有夸克|仅有夸克|只用夸克|没有115|不用115', raw, flags=re.IGNORECASE):
            payload['has_quark'] = True
            payload['has_115'] = False
        elif re.search('只有115|仅有115|只用115|没有夸克|不用夸克', raw, flags=re.IGNORECASE):
            payload['has_115'] = True
            payload['has_quark'] = False
        if re.search('(不要|不用|关闭|禁用).{0,4}夸克', raw, flags=re.IGNORECASE):
            payload['has_quark'] = False
        elif re.search('(启用|使用|打开).{0,4}夸克', raw, flags=re.IGNORECASE):
            payload['has_quark'] = True
        if re.search('(不要|不用|关闭|禁用).{0,4}115', raw):
            payload['has_115'] = False
        elif re.search('(启用|使用|打开).{0,4}115', raw):
            payload['has_115'] = True
        if re.search('pt.{0,4}(只要|必须).{0,4}免费|只下免费|只要免费', raw, flags=re.IGNORECASE):
            payload['pt_require_free'] = True
        elif re.search('pt.{0,4}(不限|不强求).{0,4}免费|不只要免费|免费不强求', raw, flags=re.IGNORECASE):
            payload['pt_require_free'] = False
        seed_match = re.search('(?:做种|种子|seeders?|seeder)[^\\d]{0,8}(\\d+)', raw, flags=re.IGNORECASE)
        if seed_match:
            payload['pt_min_seeders'] = self._safe_int(seed_match.group(1), 3)
        if re.search('(不|不要|关闭|禁用).{0,4}自动入库', raw):
            payload['auto_ingest_enabled'] = False
        elif re.search('自动入库|自动下载|自动转存', raw):
            payload['auto_ingest_enabled'] = True
        threshold_match = re.search('(?:自动入库(?:评分|分数|阈值)|评分阈值|分数阈值|阈值)[^\\d]{0,10}(\\d{2,3})', raw)
        if threshold_match:
            payload['auto_ingest_score_threshold'] = self._safe_int(threshold_match.group(1), 90)
        for key, target in [('115目录', 'p115_default_path'), ('p115目录', 'p115_default_path'), ('夸克目录', 'quark_default_path'), ('quark目录', 'quark_default_path'), ('云盘目录', 'cloud_default_path')]:
            match = re.search(f'{re.escape(key)}\\s*=\\s*([^\\s，,]+)', raw, flags=re.IGNORECASE)
            if match:
    ...
    def _extract_series_progress(text: str) -> Dict[str, int]:
        ...
        result = {"max_episode": 0, "episode_count": 0}
        episodes: List[int] = []
        for pattern in (
            r"\be(\d{1,3})\b",
            r"第\s*(\d{1,3})\s*集",
            r"更新(?:至)?\s*(?:ep|e)?\s*0*(\d{1,3})",
            r"更(?:至)?\s*0*(\d{1,3})\s*集",
        ):
            for match in re.finditer(pattern, blob, flags=re.IGNORECASE):
                try:
                    ...
                    continue
        for pattern in (
            r"e0*(\d{1,3})\s*[-~—]\s*e?0*(\d{1,3})",
            r"第?\s*0*(\d{1,3})\s*[-~—]\s*0*(\d{1,3})\s*集",
        ):
            ...
            try:
                ...
                continue
            if end_ep >= start_ep > 0:
                result["episode_count"] = max(result["episode_count"], end_ep - start_ep + 1)
                episodes.extend([start_ep, end_ep])
        if episodes:
            result["max_episode"] = max(episodes)
        if result["episode_count"] <= 0 and len(set(episodes)) >= 2:
            sorted_eps = sorted(set(episodes))
            if sorted_eps == list(range(sorted_eps[0], sorted_eps[-1] + 1)):
                result["episode_count"] = len(sorted_eps)
        return result
    ...
    def _score_cloud_resource(self, item: Dict[str, Any], *, preferences: Optional[Dict[str, Any]]=None, source_type: str='cloud', target_path: str='') -> Dict[str, Any]:
        prefs = self._normalize_assistant_preferences(preferences)
        ...
        series_like = media_type in {'tv', 'series', '电视剧', '剧集', '番剧'} or bool(re.search('\\bs\\d{1,2}\\b|\\be\\d{1,3}\\b|season|第\\s*\\d+\\s*集|[全整]季|全集|完结|更至|更新至|短剧|剧集', text, flags=re.IGNORECASE))
        series_progress = self._extract_series_progress(text)
        ...
        completeness_detected = self._score_has_any(text, ['全集', '全季', '完结', 'complete', '全 ', '更至', '更0', '更新至'])
        if not completeness_detected and series_like and (series_progress.get('episode_count', 0) >= 3):
            completeness_detected = True
        if completeness_detected:
            score += 12
        ...
        if series_like and series_progress.get('max_episode', 0) > 0:
            progress_bonus = min(10, series_progress['max_episode'])
            if progress_bonus > 0:
                score += progress_bonus
                reasons.append(f'更新到 E{series_progress['max_episode']:02d} +{progress_bonus}')
            if series_progress['max_episode'] >= 7:
                latest_bonus = min(4, series_progress['max_episode'] - 6)
                if latest_bonus > 0:
                    score += latest_bonus
                    reasons.append(f'更新进度领先 +{latest_bonus}')
        if series_like and series_progress.get('episode_count', 0) >= 3:
            pack_bonus = min(12, series_progress['episode_count'] + 2)
            score += pack_bonus
            reasons.append(f'连续剧集包 +{pack_bonus}')
            if series_progress['episode_count'] >= series_progress.get('max_episode', 0) >= 3:
                score += 4
                reasons.append('从 E01 连续覆盖到当前 +4')
        ...
        if provider == 'quark' and (not self._parse_bool_value(prefs.get('has_quark'), True)):
            ...
            hard_risks.append(message)
            score -= 30
        if target_path and target_path in {self._clean_text(prefs.get('cloud_default_path')), self._clean_text(prefs.get('p115_default_path')), self._clean_text(prefs.get('quark_default_path'))}:
            score += 3
            reasons.append('匹配默认目录 +3')
        final_score = max(0, min(100, score))
        decision = self._score_decision(score=final_score, risk_reasons=risks, hard_risk_reasons=hard_risks, preferences=prefs, default_action='auto_ingest_cloud')
        return {'source_type': source_type, 'score_reasons': reasons}
    ...
    def _rank_pansou_items(
        ...
        *,
        ...
        for item in items:
            if not isinstance(item, dict):
                continue
            channel = self._normalize_pansou_channel_name(item.get("channel"))
            grouped.setdefault(channel, []).append(dict(item))
        ...
        def sort_key(entry: Dict[str, Any]) -> Tuple[Any, ...]:
            ...
            hard_risk_count = len(score.get("hard_risk_reasons") or [])
            risk_count = len(score.get("risk_reasons") or [])
            dt = self._clean_text(entry.get("datetime"))
        ...
        ordered_channels = channel_order or ["115", "quark"]
        ranked: List[Dict[str, Any]] = []
        for channel in ordered_channels:
            channel_items = sorted(grouped.get(channel) or [], key=sort_key, reverse=True)
            ranked.extend(channel_items[:max(1, limit_per_channel)])
        for index, item in enumerate(ranked, start=1):
            item["index"] = index
        return ranked

    def _public_pansou_items(self, items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        public_items: List[Dict[str, Any]] = []
        for item in items or []:
            if not isinstance(item, dict):
                continue
            public_item = {
                "index": self._safe_int(item.get("index"), 0),
                "channel": self._normalize_pansou_channel_name(item.get("channel")),
            ...
            for extra_key in [
                ...
                if value not in (None, "", []):
                    public_item[extra_key] = value
            public_items.append(public_item)
        return public_items


    @staticmethod
    def _prepend_search_note(message: str, note: str) -> str:
    ...
    def _assistant_finalize_pansou_result(self, *, session: str, cache_key: str, keyword: str, items: List[Dict[str, Any]], total: int, target_path: str, action_name: str, search_scope: str, recommend_handoff: Optional[Dict[str, Any]]=None, lead_note: str='') -> Dict[str, Any]:
        page_size = self._assistant_result_page_size
        self._save_session(cache_key, {'kind': 'assistant_pansou', 'stage': 'result', 'keyword': keyword, 'items': items, 'total': self._safe_int(total, len(items)), 'page': 1, 'page_size': page_size})
        ...
        result_data = {'action': action_name, 'ok': True, 'items': public_items, 'total': self._safe_int(total, len(items)), 'page': 1, 'page_size': page_size, 'total_pages': max(1, (len(items) + page_size - 1) // page_size) if items else 1, 'decision_summary': self._assistant_pansou_entry_summary(items), 'search_scope': search_scope}
        ...
        return {'success': True, 'message': text_message, 'data': self._assistant_response_data(session=session, data=result_data)}


    def _format_pansou_body_lines(self, items: List[Dict[str, Any]], total: int) -> List[str]:
        count_115 = len([x for x in items if x.get("channel") == "115"])
        count_quark = len([x for x in items if x.get("channel") == "quark"])
        lines = [f"共找到 {total} 条结果，当前展示 115 {count_115} 条、夸克 {count_quark} 条："]
        seen_115 = False
        seen_quark = False
        for cached in items:
            idx = cached["index"]
    ...
    def _format_pansou_text(
        ...
        *,
        ...
        last_visible_index = self._safe_int((page_items[-1] or {}).get("index"), first_visible_index) if page_items else first_visible_index
        lines = [
            f"盘搜搜索：{keyword}",
            f"共找到 {total} 条结果，当前第 {safe_page}/{total_pages} 页，展示全局编号 {first_visible_index}-{last_visible_index}（本次缓存 {len(items)} 条候选），本页 115 {count_115} 条、夸克 {count_quark} 条：",
        ]
        seen_115 = False
        seen_quark = False
        ...
        lines.append("下一步：直接回全局编号即可转存；想先确认可发“选择 编号 详情”。")
        if next_quark_hint > 0 and next_quark_hint != first_visible_index:
            lines.append(f"注意：本页夸克结果从全局编号 {next_quark_hint} 开始；例如直接回复“{next_quark_hint}”即可处理本页第 1 条夸克结果，不要重新从 1 开始。")
    ...
    def _format_cloud_selection_suggestion(self, pansou_items: List[Dict[str, Any]]) -> List[str]:
        ...
        lines = ['智能建议']
        lines.append(self._human_resource_suggestion_line(best_index, best_title, self._clean_text(best.get('provider')), best_reasons))
        quark_candidates = [dict(item or {}) for item in combined if isinstance(item, dict) and self._normalize_pansou_channel_name(item.get('channel') or item.get('pan_type')) == 'quark']
    ...
    def _assistant_cloud_entry_summary(self, pansou_items: List[Dict[str, Any]]) -> Dict[str, Any]:
        preferred_index = 0
        if pansou_items:
            preferred_index = self._safe_int((pansou_items[0] or {}).get('index'), 0)
        if preferred_index <= 0:
            return {'stage': 'cloud_search', 'preferred_command': '', 'fallback_command': '', 'recommended_agent_behavior': 'show_only'}
        return {'stage': 'cloud_search', 'decision_hint': '默认按编号直接选择；想先看详情可回复“选择 编号 详情”。', 'preferred_command': str(preferred_index), 'fallback_command': f'选择 {preferred_index} 详情', 'compact_commands': [str(preferred_index), f'选择 {preferred_index} 详情'], 'preferred_requires_confirmation': True, 'fallback_requires_confirmation': False, 'can_auto_run_preferred': False, 'recommended_agent_behavior': 'show_only'}

    def _assistant_finalize_cloud_result(self, *, session: str, cache_key: str, keyword: str, pansou_items: List[Dict[str, Any]], pansou_total: int, target_path: str, lead_note: str='') -> Dict[str, Any]:
        page_size = self._assistant_result_page_size
        self._save_session(cache_key, {'kind': 'assistant_cloud', 'stage': 'result', 'keyword': keyword, 'pansou_items': pansou_items, 'pansou_total': pansou_total, 'page': 1, 'page_size': page_size})
        text_message = self._format_cloud_text(keyword=keyword, pansou_items=pansou_items, pansou_total=pansou_total, page=1, page_size=page_size)
        if lead_note:
            text_message = self._prepend_search_note(text_message, lead_note)
        return {'success': True, 'message': text_message, 'data': self._assistant_response_data(session=session, data={'action': 'cloud_search', 'ok': True, 'items': self._public_pansou_items(pansou_items), 'page': 1, 'page_size': page_size})}
    ...
    def _format_cloud_text(
        ...
        *,
        ...
        total_items = len(pansou_items)
        safe_page, safe_page_size, total_pages, start, end = self._page_bounds(
            total_items,
            page=page,
            page_size=page_size,
        )
        lines = [f"盘搜结果：{keyword}"]
        if total_items:
            lines.append(f"当前第 {safe_page}/{total_pages} 页，展示编号 {start + 1}-{min(total_items, end)} / 共 {total_items} 条结果：")
        page_items = [
            item for item in pansou_items
            if start < self._safe_int(item.get("index"), 0) <= end
        ]
        lines.append("")
        if page_items:
            lines.extend(self._format_pansou_body_lines(page_items, pansou_total))
        else:
            lines.append("暂无结果")
        suggestion_lines = self._format_cloud_selection_suggestion(pansou_items)
        if suggestion_lines:
            lines.extend(["", *suggestion_lines])
    ...
    def _score_summary_decision(self, summary: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "requires_confirmation": False if is_pt else not bool(best.get("can_auto_execute")),
            "prefer_plan_first": not is_pt,
            "command_policy": "auto_continue" if is_pt else "read_then_confirm_write" if len(commands) > 1 else "safe_read_only",
            "preferred_requires_confirmation": False,
            "fallback_requires_confirmation": False if is_pt else bool(len(commands) > 1),
    ...
    def _reindex_mp_preview_items(self, items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _assistant_mp_current_items(
        self,
        *,
        cache_key: str,
        preferences: Dict[str, Any],
        ...
        if all_items:
            return [dict(item or {}) for item in all_items if isinstance(item, dict)]
        items = state.get("items") if isinstance(state.get("items"), list) else []
        if items:
            return [dict(item or {}) for item in items if isinstance(item, dict)]
        return self._mp_search_cache_preview(
            cache_key,
            preferences=preferences,
            limit=self._assistant_result_page_size,
        )
    ...
    def _assistant_mp_item_by_display_index(
        ...
        *,
        ...
        items = self._assistant_mp_current_items(cache_key=cache_key, preferences=preferences)
        available_indexes = [
            self._safe_int(item.get("index"), 0)
            for item in items
            if self._safe_int(item.get("index"), 0) > 0
        ]
        selected = next(
            (dict(item or {}) for item in items if self._safe_int(item.get("index"), 0) == self._safe_int(choice, 0)),
            {},
        )
        return selected, available_indexes
    ...
    def _format_mp_search_text(
        ...
        *,
        ...
        if preview and (
            header.startswith("搜索资源失败")
            or header.startswith("MP 原生搜索失败")
            or header.startswith("未识别到媒体信息")
        ):
            header = f"{keyword} — MP搜索"
        lines = [header]
        if preview:
            ...
            lines.append("")
            header_suffix = "按做种数优先排序"
            target_episode = self._safe_int(episode_filter, 0)
            if result_filter == "latest_episode" and self._safe_int(latest_episode, 0) > 0:
                header_suffix = f"已按最新集 E{self._safe_int(latest_episode, 0):02d} 优先排序"
            elif result_filter.startswith("episode:") and target_episode > 0:
                header_suffix = f"已按第 {target_episode} 集优先排序"
            lines.append(f"当前第 {max(1, page)}/{total_pages} 页，共 {total_results} 条结果（{header_suffix}）：")
            lines.append("PT 资源：")
            for item in preview:
            ...
            if result_filter == "latest_episode" or result_filter.startswith("episode:"):
                first_item = preview[0] if preview else {}
                first_index = self._safe_int(first_item.get("index"), 0)
                first_title = self._display_pt_title(((first_item.get("torrent_info") or {}).get("title")))
                decision_lines = []
                if first_index > 0:
                    decision_lines.append("决策建议：建议确认")
                    if result_filter == "latest_episode":
                        decision_lines.append(
                            f"建议：当前已按最新集优先排序，首选 #{first_index}：{first_title}。"
                        )
                    elif target_episode > 0:
                        decision_lines.append(
                            f"建议：当前已按第 {target_episode} 集优先排序，首选 #{first_index}：{first_title}。"
                        )
                    decision_lines.append(f"下一步：{first_index}")
            lines.extend(decision_lines)
            if page < total_pages:
                lines.append("如需继续翻页，可回复：n 下一页")
            lines.append("回复编号直接下载；n 下一页。")
    ...
    async def _assistant_mp_media_search(
        ...
        *,
        ...
        try:
            effective_filter = self._clean_text(result_filter).lower()
            latest_episode = 0
            episode_filter = 0
            ...
            filtered_items = [dict(item or {}) for item in all_items if isinstance(item, dict)]
            if effective_filter == "latest_episode":
                filtered_items, latest_episode = self._latest_episode_mp_items(filtered_items)
                filtered_items = self._sort_mp_episode_filtered_items(filtered_items, latest_episode)
            elif effective_filter.startswith("episode:"):
                episode_filter = self._safe_int(effective_filter.split(":", 1)[1], 0)
                filtered_items = self._episode_filter_mp_items(filtered_items, episode_filter)
                filtered_items = self._sort_mp_episode_filtered_items(filtered_items, episode_filter)
            filtered_items = self._reindex_mp_preview_items(filtered_items)
            total = len(filtered_items)
            preview = (
                self._slice_mp_preview_items(filtered_items, page=page, page_size=page_size)
                if filtered_items
                else []
            )
            self._save_session(cache_key, {
            ...
            logger.error(
                f"[AgentResourceOfficer] MP 搜索处理失败：{keyword} {exc}\n{traceback.format_exc()}"
            )
            return {
                "success": False,
                "message": f"MP 搜索处理失败：{exc}",
                "data": self._assistant_response_data(session=session, data={
                    "action": "mp_media_search",
                    "ok": False,
                    "keyword": keyword,
                    "error_code": "mp_media_search_exception",
                    "error": str(exc),
                }),
            }
    ...
    async def _assistant_mp_candidate_search(
        ...
        *,
        ...
        try:
            ...
            item = dict(result.get("item") or {}) if isinstance(result.get("item"), dict) else {}
            candidates = []
            if result.get("success") and item.get("title"):
                item["media_type"] = self._clean_text(item.get("type") or media_type or "auto").lower() or "auto"
                candidates = [item]
            if not candidates:
                return {
                    "success": False,
                    "message": self._clean_text(result.get("message")) or f"未识别到媒体信息：{clean_keyword}",
                    "data": self._assistant_response_data(
                        session=session,
                        data={
            ...
            self._save_session(
                cache_key,
                {
                    ...
                    "stage": "candidate",
                    "keyword": clean_keyword,
                    "media_type": item.get("media_type") or "auto",
                    "year": self._clean_text(year),
            ...
            message_text = self._format_mp_candidate_lines(candidates, page=page, page_size=page_size)
            ...
            logger.error(f"[AgentResourceOfficer] MP 候选解析异常：{clean_keyword} {exc}\n{traceback.format_exc()}")
            return {
                "success": False,
                "message": f"MP 候选解析异常：{exc}",
                "data": self._assistant_response_data(
                    session=session,
                    data={
                        "action": "mp_media_candidates",
                        "ok": False,
                        "keyword": clean_keyword,
                        "error_code": "candidate_search_exception",
                        "error": str(exc),
                    },
                ),
            }
    ...
    async def _assistant_attach_download_plan_choices(
        ...
        *,
        ...
        current_state = self._load_session(cache_key) or {}
        result_filter = self._clean_text(current_state.get("result_filter")).lower()
        target_episode = 0
        filter_notice = ""
        if result_filter == "latest_episode":
            target_episode = self._safe_int(current_state.get("latest_episode"), 0)
            if target_episode > 0:
                filter_notice = f"已按最新集 E{target_episode:02d} 优先筛选，并生成 3 个待确认下载方案，均未实际下载："
        elif result_filter.startswith("episode:"):
            target_episode = self._safe_int(current_state.get("episode_filter"), 0)
            if target_episode > 0:
                filter_notice = f"已优先按第 {target_episode} 集单集匹配，并生成 3 个待确认下载方案，均未实际下载："
        preview = self._assistant_mp_current_items(cache_key=cache_key, preferences=preferences)
        if not preview:
            preview = self._mp_search_cache_preview(cache_key, preferences=preferences, limit=self._assistant_result_page_size)
        scored = [
            item for item in preview
        ...
        if target_episode > 0:
            scored.sort(
                key=lambda item: (
                    self._mp_episode_plan_priority(item, target_episode),
                    -self._safe_int((item.get("score") or {}).get("score"), 0),
                    -self._safe_int(((item.get("torrent_info") or {}).get("seeders")), 0),
                    -self._score_quality_rank(item),
                    self._safe_int(item.get("index"), 0),
        ...
        merged = {
            "success": True,
            "message": "\n".join([
                filter_notice or f"已按当前偏好生成 {len(plan_items)} 个待确认下载方案，均未实际下载：",
                *plan_rows,
    ...
    def _assistant_smart_search_candidate_from_pt_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        detail_command = ""
        next_command = str(index) if index > 0 else plan_command
        return {
            "source_type": "mp_pt",
    ...
    def _assistant_smart_search_decision_summary(self, *, keyword: str, preferences: Dict[str, Any], checked: List[Dict[str, Any]], best_candidate: Dict[str, Any], available_sources: Optional[List[Dict[str, Any]]]=None, blocked_sources: Optional[List[Dict[str, Any]]]=None, decision_profile: str='') -> Dict[str, Any]:
        prefs = self._normalize_assistant_preferences(preferences)
        threshold = self._safe_int(prefs.get('confirm_score_threshold'), self._assistant_default_confirm_score_threshold)
        auto_threshold = self._safe_int(prefs.get('auto_ingest_score_threshold'), self._assistant_default_auto_ingest_score_threshold)
        source_names = {'pansou': '盘搜', 'mp_pt': 'MP/PT'}
        ...
        is_pt_source = source_type == 'mp_pt'
        fallback_command = self._clean_text(best_candidate.get('plan_command' if is_pt_source else 'detail_command'))
        detail_command = '' if is_pt_source else '先看详情' if best_candidate.get('choice') else ''
        detail_short_command = '' if is_pt_source else '详情' if best_candidate.get('choice') else ''
        ...
        if hard_risks:
            ...
            confirmation_prompt = '直接回编号下载，或换源后再试。' if is_pt_source else '先看详情，或换源后再试。'
            ...
            if is_pt_source:
                ...
                decision_reason = 'PT 搜索结果默认回编号直接下载。'
                fallback_command = self._clean_text(best_candidate.get('plan_command')) or fallback_command
                confirm_required = False
                confirmation_prompt = '直接回编号下载。'
            elif score_value >= auto_threshold:
                decision_mode = 'execute_now'
            ...
            if is_pt_source:
                decision_mode = 'execute_now'
                decision_reason = '当前最佳 PT 候选分数偏低，但仍可直接回编号下载。'
                fallback_command = self._clean_text(best_candidate.get('plan_command')) or fallback_command
                confirmation_prompt = '直接回编号下载。'
            else:
                decision_mode = 'show_detail'
                ...
                confirmation_prompt = '先看详情；如果仍要继续，回复：先计划 或 换盘搜 / 换PT。'
        if title:
            hint = f'{hint} {title}'
        auto_run_command = ''
        confirm_command = ''
    ...
    def _assistant_smart_decision_entry_message(self, *, keyword: str, decision_summary: Dict[str, Any], available_sources: List[Dict[str, Any]], blocked_sources: List[Dict[str, Any]]) -> str:
        checked = decision_summary.get('checked_sources') or []
        checked_text = ' -> '.join(({'pansou': '盘搜', 'mp_pt': 'MP/PT'}.get(self._clean_text(item).lower(), self._clean_text(item)) for item in checked if self._clean_text(item)))
        ...
        if blocked_sources:
            blocked_text = '；'.join((f'{self._clean_text(item.get('label'))}：{self._clean_text(item.get('reason'))}' for item in blocked_sources[:3] if self._clean_text(item.get('label')) and self._clean_text(item.get('reason'))))
            if blocked_text:
    ...
    def _assistant_smart_source_item_by_choice(self, source_states: Dict[str, Any], *, source_type: str, choice: int) -> Dict[str, Any]:
    ...
    def _assistant_smart_best_plan_response(self, *, session: str, cache_key: str, state: Optional[Dict[str, Any]]=None, target_path: str='') -> Dict[str, Any]:
        ...
        if self._clean_text(current_state.get('kind')) != 'assistant_smart_search':
            return {'success': False, 'message': '当前没有可继续的智能搜索结果，请先发送：智能搜索 片名 或 智能计划 片名。', 'data': self._assistant_response_data(session=session, data={'action': 'smart_resource_plan', 'ok': False, 'error_code': 'smart_search_session_not_found'})}
        ...
        if not best_candidate:
            return {'success': False, 'message': '智能搜索当前没有可用于生成计划的候选，请先换片名，或调整偏好后再试。', 'data': self._assistant_response_data(session=session, data={'action': 'smart_resource_plan', 'ok': False, 'error_code': 'smart_best_candidate_not_found', 'decision_summary': decision_summary, 'sources_checked': checked})}
        source_type = self._clean_text(best_candidate.get('source_type')).lower()
        ...
        final_path = target_path or self._clean_text(current_state.get('target_path')) or self._clean_text((source_states.get(source_type) or {} if isinstance(source_states, dict) else {}).get('target_path'))
        if hard_risks:
            head = self._clean_text(decision_summary.get('decision_hint')) or '当前首选存在硬风险。'
            tail = '；'.join(hard_risks[:2])
            return {'success': False, 'message': f'{head}\n未生成计划：{tail}', 'data': self._assistant_response_data(session=session, data={'action': 'smart_resource_plan', 'ok': False, 'error_code': 'smart_best_candidate_blocked', 'best_candidate': best_candidate, 'decision_summary': decision_summary, 'sources_checked': checked})}
        result: Dict[str, Any]
        if source_type == 'pansou':
        ...
        if choice and (not data.get('choice')):
            data['choice'] = choice
        result['data'] = data
        message_lines = ['已根据智能搜索结果自动选择当前首选并生成待确认计划', f'首选：{source_type} #{choice} | {score_value}分' + (f' | {title}' if title else ''), result.get('message') or '']
        if risks:
    ...
    async def _assistant_smart_best_execute_response(self, request: Request, *, session: str, cache_key: str, state: Optional[Dict[str, Any]]=None, target_path: str='') -> Dict[str, Any]:
        plan_result = self._assistant_smart_best_plan_response(session=session, cache_key=cache_key, state=state, target_path=target_path)
        plan_data = dict(plan_result.get('data') or {})
        ...
        score_value = self._safe_int(best_candidate.get('score'), 0)
        title = self._clean_text(best_candidate.get('title'))
        source_label = {'pansou': '盘搜', 'mp_pt': 'MP/PT'}.get(source_type, source_type or '当前源')
        transfer_source_text = {'pansou': '盘搜搜索转存', 'mp_pt': 'MP/PT 下载'}.get(source_type, source_label)
        prefix = '已根据智能搜索结果自动生成并执行当前首选计划'
        prefix += f'\n执行方式：{transfer_source_text}'
        if choice:
            prefix += f'\n首选：{source_label} #{choice} | {score_value}分' + (f' | {title}' if title else '')
        result = await self._assistant_execute_prepared_plan_result(request, session=session, cache_key=cache_key, plan_result=plan_result, message_prefix=prefix, extra_data={'smart_execute_auto_selected': True, 'source_type': source_type})
        if result.get('success'):
            overrides = dict((state or {}).get('session_preference_overrides') or {})
            prefer_provider = self._clean_text(overrides.get('prefer_cloud_provider')).lower()
            provider_label = '115 转存' if prefer_provider == '115' else '夸克转存' if prefer_provider == 'quark' else '资源转存'
            compact_lines = [f'已自动完成 {provider_label} ✅']
            if choice:
                resource_line = f'• 资源：{source_label} #{choice}'
                if title:
                    resource_line += f' — {title}'
                if score_value > 0:
                    resource_line += f'，{score_value}分'
                compact_lines.append(resource_line)
            compact_lines.append(f'• 执行方式：{transfer_source_text}')
            compact_lines.append('• 状态：执行成功')
            result['message'] = '\n'.join(compact_lines)
        return result

    async def _assistant_cloud_transfer_execute(self, request: Request, *, keyword: str, session: str, cache_key: str, media_type: str, year: str, source_order: Optional[List[str]]=None, target_path: str='', session_preference_overrides: Optional[Dict[str, Any]]=None, origin: str='') -> Dict[str, Any]:
        effective_target_path = self._clean_text(target_path)
        prefer_provider = self._clean_text((session_preference_overrides or {}).get('prefer_cloud_provider')).lower()
        if not effective_target_path:
            if prefer_provider == 'quark':
                effective_target_path = self._quark_default_path
            elif prefer_provider == '115':
                effective_target_path = self._p115_default_path
        merged_preferences = self._assistant_smart_merge_session_preferences(self._assistant_preferences_for_session(session=session), session_overrides=session_preference_overrides)
        enabled_cloud_source_order = self._assistant_transfer_source_order(preferences=merged_preferences, prefer_provider=prefer_provider, requested_source_order=source_order)
        if not enabled_cloud_source_order:
            if prefer_provider == 'quark':
                return {'success': False, 'message': '夸克转存只走盘搜；当前盘搜入口已关闭，可在插件设置中开启“启用盘搜”。', 'data': self._assistant_response_data(session=session, data={'action': 'cloud_transfer_execute', 'ok': False, 'error_code': 'pansou_required_for_quark_transfer'})}
            if prefer_provider == '115':
                message = '115转存当前只走盘搜；当前盘搜入口已关闭，可在插件设置中开启“启用盘搜”。'
                return {'success': False, 'message': message, 'data': self._assistant_response_data(session=session, data={'action': 'cloud_transfer_execute', 'ok': False, 'error_code': 'pansou_required_for_115_transfer'})}
            return self._cloud_sources_disabled_response()
        search_result = await self._assistant_smart_resource_search(request, keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, source_order=enabled_cloud_source_order, target_path=effective_target_path, session_preference_overrides=session_preference_overrides, origin=origin)
        smart_state = self._assistant_smart_state_after_search(cache_key=cache_key, keyword=keyword, media_type=media_type, year=year, source_order=enabled_cloud_source_order, target_path=effective_target_path, search_result=search_result, origin=origin)
        best_candidate = dict(smart_state.get('best_candidate') or {})
        if not search_result.get('success') and (not best_candidate.get('source_type')):
            if prefer_provider == '115' and enabled_cloud_source_order == ['pansou']:
                return {'message': '\n'.join((line for line in [self._clean_text(search_result.get('message'))] if line))}
            if prefer_provider == 'quark' and enabled_cloud_source_order == ['pansou']:
                return {'message': '\n'.join((line for line in [self._clean_text(search_result.get('message'))] if line))}
        ...
        score_value = self._safe_int(best_candidate.get('score'), 0)
        hard_risks = [self._clean_text(value) for value in best_candidate.get('hard_risk_reasons') or [] if self._clean_text(value)]
        if best_candidate.get('source_type') and score_value >= threshold and (not hard_risks):
            execute_result = await self._assistant_smart_best_execute_response(request, session=session, cache_key=cache_key, state=smart_state, target_path=effective_target_path)
            source_type = self._clean_text(best_candidate.get('source_type')).lower()
            return execute_result
        source_states = smart_state.get('source_states') if isinstance(smart_state.get('source_states'), dict) else {}
        pansou_state = dict(source_states.get('pansou') or {})
        cloud_result = self._assistant_finalize_cloud_result(session=session, cache_key=cache_key, keyword=keyword, pansou_items=pansou_state.get('items') or [], pansou_total=len(pansou_state.get('items') or []))
        top_choices: List[str] = []
        if best_candidate.get('choice') and best_candidate.get('source_type'):
            source_label = {'pansou': '盘搜'}.get(self._clean_text(best_candidate.get('source_type')).lower(), self._clean_text(best_candidate.get('source_type')))
            top_choices.append(f'{source_label} #{self._safe_int(best_candidate.get('choice'), 0)}')
        for item in smart_state.get('alternatives') or []:
            if not isinstance(item, dict):
                continue
            source_type = self._clean_text(item.get('source_type')).lower()
            source_label = {'pansou': '盘搜'}.get(source_type, source_type)
            label = f'{source_label} #{self._safe_int(item.get('choice'), 0)}'
            if label not in top_choices and self._safe_int(item.get('choice'), 0) > 0:
                top_choices.append(label)
            if len(top_choices) >= 3:
                break
        if top_choices:
            cloud_result['message'] = '\n'.join([self._clean_text(cloud_result.get('message')), '推荐优先看：' + ' / '.join(top_choices[:3])]).strip()
        if isinstance(cloud_result.get('data'), dict):
            cloud_result['data']['action'] = 'cloud_transfer_execute'
            cloud_result['data']['best_candidate'] = best_candidate
        return cloud_result

    async def _assistant_smart_resource_search(self, request: Request, *, keyword: str, session: str, cache_key: str, media_type: str, year: str, source_order: Optional[List[str]]=None, target_path: str='', decision_profile: str='', session_preference_overrides: Optional[Dict[str, Any]]=None, action_name: str='smart_resource_search', origin: str='') -> Dict[str, Any]:
        base_preferences = self._assistant_preferences_for_session(session=session)
        preferences = self._assistant_smart_merge_session_preferences(base_preferences, session_overrides=session_preference_overrides, decision_profile=decision_profile)
        search_order = self._assistant_smart_search_source_order(preferences, source_order=source_order)
        available_sources, blocked_sources = self._assistant_smart_source_availability(preferences, source_order=source_order)
        if not search_order:
            return {'success': False, 'data': self._assistant_response_data(session=session, data={'action': action_name, 'ok': False, 'error_code': 'no_enabled_sources', 'preference_status': self._assistant_preferences_status_brief(session=session), 'available_sources': available_sources, 'blocked_sources': blocked_sources})}
        ...
        for current_source in search_order:
            if current_source == 'pansou':
                ...
                channel_115 = self._collect_pansou_channel_items(merged, '115', 20)
                channel_quark = self._collect_pansou_channel_items(merged, 'quark', 20)
                items: List[Dict[str, Any]] = []
                for item in channel_115 + channel_quark:
                    items.append({'index': len(items) + 1})
                items = self._assistant_filter_cloud_items_by_preferences(items, preferences)
                for idx, item in enumerate(items, start=1):
                    item['index'] = idx
                items = self._attach_cloud_scores(items, preferences=preferences, source_type='pansou')
                items = self._rank_pansou_items(items, limit_per_channel=6)
                source_state = {'kind': 'assistant_pansou', 'stage': 'result', 'keyword': keyword, 'items': items}
                ...
                checked.append({'source_type': current_source, 'ok': bool(search_ok), 'result_count': len(items), 'best_score': self._safe_int((source_best or {}).get('score'), 0), 'continued': not self._assistant_smart_search_stop_after_source(source_best, preferences), 'reason': search_message or ('盘搜无结果' if not items else '')})
                alternatives.extend((self._assistant_smart_search_candidate_from_cloud_item(item, source_type='pansou') for item in items[:4]))
                ...
                if self._assistant_smart_search_stop_after_source(source_best, preferences):
                    break
                continue
            mp_result = await self._assistant_mp_media_search(keyword=keyword, session=session, cache_key=cache_key, preferences=preferences)
            ...
            checked.append({'source_type': current_source, 'ok': bool(mp_result.get('success')), 'result_count': len(items), 'best_score': self._safe_int((source_best or {}).get('score'), 0), 'continued': not self._assistant_smart_search_stop_after_source(source_best, preferences), 'reason': self._assistant_result_message_head(mp_result.get('message'))})
            alternatives.extend((self._assistant_smart_search_candidate_from_pt_item(item) for item in items[:4]))
            if source_best and self._safe_int(source_best.get('score'), 0) > best_source_score:
                best_source_score = self._safe_int(source_best.get('score'), 0)
                best_candidate = source_best
                active_state = source_state
        ...
        alternatives.sort(key=lambda item: (self._safe_int(item.get('score'), 0), 0 if item.get('hard_risk_reasons') else 1), reverse=True)
        unique_alternatives: List[Dict[str, Any]] = []
        seen_keys: set = set()
        for item in alternatives:
            dedupe_key = (self._clean_text(item.get('source_type')).lower(), self._safe_int(item.get('choice'), 0))
            if dedupe_key in seen_keys:
                continue
            seen_keys.add(dedupe_key)
            unique_alternatives.append(item)
        decision_summary = self._assistant_smart_search_decision_summary(keyword=keyword, preferences=preferences, checked=checked, best_candidate=best_candidate, available_sources=available_sources, blocked_sources=blocked_sources, decision_profile=decision_profile)
        if active_state:
            smart_state = {'kind': 'assistant_smart_search', 'stage': 'result', 'keyword': keyword, 'media_type': media_type, 'year': year, 'target_path': target_path or active_state.get('target_path') or '', 'active_source': self._clean_text(best_candidate.get('source_type')), 'active_state': copy.deepcopy(active_state), 'source_states': source_states, 'sources_checked': checked, 'best_candidate': best_candidate, 'alternatives': unique_alternatives[:6], 'decision_summary': decision_summary, 'available_sources': available_sources, 'blocked_sources': blocked_sources, 'source_order': search_order, 'decision_profile': self._clean_text(decision_profile), 'decision_entry': action_name, 'origin': self._clean_text(origin), 'session_preference_overrides': dict(session_preference_overrides or {})}
            self._save_session(cache_key, smart_state)
        if action_name == 'smart_resource_decision':
            message_lines = [self._assistant_smart_decision_entry_message(keyword=keyword, decision_summary=decision_summary, available_sources=available_sources, blocked_sources=blocked_sources)]
        else:
            message_lines = [f'智能搜索：{keyword}', '已检查：' + ' -> '.join(({'pansou': '盘搜', 'mp_pt': 'MP/PT'}.get(self._clean_text(item.get('source_type')).lower(), self._clean_text(item.get('source_type'))) for item in checked if self._clean_text(item.get('source_type'))))]
            if best_candidate:
    ...
    async def _assistant_smart_resource_decision_adjust(self, request: Request, *, session: str, cache_key: str, state: Optional[Dict[str, Any]], adjust_action: str) -> Dict[str, Any]:
        current_state = dict(state or self._load_session(cache_key) or {})
        if self._clean_text(current_state.get('kind')) != 'assistant_smart_search':
            return {'success': False, 'message': '当前没有可继续的资源决策会话，请先发送：资源决策 片名。', 'data': self._assistant_response_data(session=session, data={'action': 'smart_resource_decision', 'ok': False, 'error_code': 'smart_decision_session_not_found'})}
        keyword = self._clean_text(current_state.get('keyword'))
        if not keyword:
            return {'success': False, 'message': '当前资源决策会话缺少片名，请重新发送：资源决策 片名。', 'data': self._assistant_response_data(session=session, data={'action': 'smart_resource_decision', 'ok': False, 'error_code': 'smart_decision_missing_keyword'})}
        ...
        if not source_order:
            base_preferences = self._assistant_preferences_for_session(session=session)
            source_order = self._assistant_smart_search_source_order(self._assistant_smart_merge_session_preferences(base_preferences, session_overrides=session_preference_overrides))
        decision_profile = self._clean_text(current_state.get('decision_profile'))
        if adjust_action == 'decision_pansou':
            source_order = ['pansou', 'mp_pt']
        elif adjust_action == 'decision_mp_pt':
            source_order = ['mp_pt', 'pansou']
            ...
            pass
        return await self._assistant_smart_resource_decision(request, keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, source_order=source_order, target_path=target_path, decision_profile=decision_profile, session_preference_overrides=session_preference_overrides)
    ...
    def _assistant_smart_state_after_search(self, *, cache_key: str, keyword: str, media_type: str, year: str, source_order: Optional[List[str]]=None, target_path: str='', search_result: Optional[Dict[str, Any]]=None, origin: str='') -> Dict[str, Any]:
    ...
    def _assistant_mp_download_plan_response(
        ...
        *,
        ...
        selected, available_indexes = self._assistant_mp_item_by_display_index(
            choice=choice,
            cache_key=cache_key,
            preferences=preferences,
        )
        ...
        if not selected:
            ...
            return {
                ...
                "message": (
                    f"当前列表没有 #{choice}。可选编号：{available_text}。"
                    if available_text
                    else "没有可继续的 MP/PT 搜索结果，请先发送“搜索 片名”或“MP搜索 片名”。"
                ),
                "data": self._assistant_response_data(session=session, data={
                    "action": "mp_download",
                    "ok": False,
    ...
    async def _assistant_mp_subscribe_control(
        ...
        *,
        ...
        if subscribe_id <= 0:
            return {
                "success": False,
                "message": "操作订阅失败：请先发送“订阅列表”获取列表，再按编号操作，例如“刷新订阅 1”。",
                "data": self._assistant_response_data(session=session, data={
                    "action": "mp_subscribe_control",
                    "ok": False,
    ...
    def _format_quark_transfer_failure(self, detail: str = "", target_path: str = "") -> str:
        text = self._clean_text(detail)
        target = self._clean_text(target_path) or self._quark_default_path or "/飞书"
        if "41031" in text or "分享者用户封禁链接查看受限" in text:
            return (
        ...
        if not text or text == f"无法转存到 {target}":
            try:
                cookie_ok, cookie_message = self._ensure_quark_service().check_cookie()
            except Exception:
                cookie_ok, cookie_message = True, ""
            cookie_text = self._clean_text(cookie_message)
            if (not cookie_ok) and (
                "require login [guest]" in cookie_text
                or "未配置夸克 Cookie" in cookie_text
                or "cookie" in cookie_text.lower()
                or "login" in cookie_text.lower()
            ):
                return (
                    f"夸克转存失败：当前夸克登录态不足，无法转存到 {target}。"
                    "建议先刷新夸克登录后再试。"
    ...
    @staticmethod
    def _is_quark_share_restricted_message(detail: str) -> bool:
        text = str(detail or "").strip()
        if not text:
            return False
        return "41031" in text or "分享者用户封禁链接查看受限" in text

    async def _assistant_retry_pansou_quark_candidates(
        self,
        request: Request,
        *,
        items: List[Dict[str, Any]],
        ...
        for offset, raw_item in enumerate(items[selected_index:], start=selected_index + 1):
            item = dict(raw_item or {})
            share_url = self._clean_text(item.get("url"))
            if not share_url or not self._is_quark_url(share_url):
                continue
            if share_url == selected_url:
                continue
            candidates.append((offset, item))
            if len(candidates) >= 3:
                break
        ...
        for alt_index, alt_item in candidates:
            alt_url = self._clean_text(alt_item.get("url"))
            alt_code = self._clean_text(alt_item.get("password"))
            alt_result = await self.api_share_route(
                _JsonRequestShim(request, {
                    "url": alt_url,
                    "access_code": alt_code,
                    "path": target_path,
                    "trigger": "Agent影视助手 盘搜夸克候选自动切换",
                    "apikey": apikey,
                })
            )
            if alt_result.get("success"):
                message = (
                    f"原夸克链接已受限，已自动切换到同列表夸克候选 #{alt_index}。\n"
                    f"{str(alt_result.get('message') or '夸克转存成功')}"
                )
                data = dict(alt_result.get("data") or {})
                data.update(
                    {
                        "provider": "quark",
                        "selected_choice": alt_index,
                        "auto_switched_from_choice": selected_index,
                        "auto_switched": True,
        ...
        hint_choices = "、".join(f"#{idx}" for idx, _ in candidates)
        return {
            "success": False,
            "message": (
                "夸克转存失败：当前分享链接已受限或分享者账号受限，"
                f"可改试同列表夸克候选 {hint_choices}。"
            ),
            "data": self._assistant_response_data(
                session=session,
                data={
                    "provider": "quark",
                    "auto_switched": False,
                    "fallback_choices": [idx for idx, _ in candidates],
                },
            ),
        }
    ...
    @staticmethod
    def _assistant_quarkdisk_plugin() -> Any:
        if PluginManager is None:
            return None
        try:
            running_plugins = PluginManager().running_plugins or {}
            ...
            return None
        plugin = running_plugins.get("QuarkDisk") or running_plugins.get("quarkdisk")
        if plugin is not None:
            return plugin
        for candidate in running_plugins.values():
            if candidate is None:
                continue
            if candidate.__class__.__name__ == "QuarkDisk":
                return candidate
            if getattr(candidate, "plugin_name", "") == "夸克网盘存储":
                return candidate
        return None

    def _assistant_quarkdisk_clear_directory(self, path: str) -> Optional[Tuple[bool, Dict[str, Any], str]]:
        plugin = self._assistant_quarkdisk_plugin()
        if plugin is None or app_schemas is None:
            return None
        try:
            storage_name = self._clean_text(getattr(plugin, "_disk_name", "")) or "夸克网盘"
            clear_cache = getattr(plugin, "clear_cache", None)
            ...
            def _resolve_folder() -> Any:
                resolved = None
                if hasattr(plugin, "get_folder"):
                    resolved = plugin.get_folder(storage=storage_name, path=Path(path))
                if resolved is None and hasattr(plugin, "get_file_item"):
                    resolved = plugin.get_file_item(storage=storage_name, path=Path(path))
                return resolved
            ...
            if hasattr(plugin, "list_files"):
                items = plugin.list_files(folder, recursion=False) or []
                if not items:
                    _safe_clear_cache()
                    folder = _resolve_folder() or folder
                    items = plugin.list_files(folder, recursion=False) or []
            ...
            for item in items:
                ok = plugin.delete_file(item) if hasattr(plugin, "delete_file") else None
                if ok:
                    removed_count += 1
                    continue
                item_path = self._clean_text(getattr(item, "path", "")) or self._clean_text(getattr(item, "name", ""))
                if item_path:
            ...
            result = {
                ...
                "items": [self._clean_text(getattr(item, "name", "")) for item in items[:20] if self._clean_text(getattr(item, "name", ""))],
                "failed_items": failed_items[:20],
                "time": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                "bridge": "quarkdisk",
            }
            if failed_items:
                return False, result, f"夸克网盘存储删除失败：{len(failed_items)} 项未删除"
            return True, result, "success"
        except Exception as exc:
            logger.warning(f"[AgentResourceOfficer] 调用 quarkdisk 清空目录失败，改走直连夸克接口：{exc}")
            return None
    ...
    async def _assistant_mp_recommendations(self, *, source: str='tmdb_trending', media_type: str='all', limit: int=20, session: str='default', cache_key: str='') -> Dict[str, Any]:
        try:
            from app.chain.recommend import RecommendChain
            from app.schemas.types import MediaType, media_type_to_agent
        except Exception as exc:
            return {'success': False, 'message': f'MP 推荐失败：当前环境缺少推荐依赖 {exc}', 'data': self._assistant_response_data(session=session, data={'action': 'mp_recommendations', 'ok': False})}
        ...
        try:
                ...
                for raw_item in (raw_results or [])[:max_limit]:
                    ...
                    if current_media_type != 'all':
                        ...
                        if enum_type and item_type != enum_type and (agent_type not in expected_types):
                            continue
                    collected.append({'index': len(collected) + 1, 'title': raw_item.get('title'), 'year': raw_item.get('year'), 'type': media_type_to_agent(item_type), 'tmdb_id': raw_item.get('tmdb_id'), 'douban_id': raw_item.get('douban_id'), 'vote_average': raw_item.get('vote_average'), 'poster_path': raw_item.get('poster_path'), 'detail_link': raw_item.get('detail_link')})
                return collected
            ...
            if source_name == 'tmdb_trending':
                ...
                return {'success': False, 'message': f'不支持的推荐来源：{source_name}', 'data': self._assistant_response_data(session=session, data={'action': 'mp_recommendations', 'ok': False})}
            items = collect_items(results)
            fallback_source = ''
            fallback_notice = ''

            async def fetch_source_items(next_source: str, next_media_type: str) -> List[Dict[str, Any]]:
                if next_source == 'tmdb_trending':
                    return collect_items(await chain.async_tmdb_trending(page=1), next_media_type)
                if next_source == 'tmdb_movies':
                    return collect_items(await chain.async_tmdb_movies(page=1), next_media_type)
                if next_source == 'tmdb_tvs':
                    return collect_items(await chain.async_tmdb_tvs(page=1), next_media_type)
                if next_source == 'douban_movie_hot':
                    return collect_items(await chain.async_douban_movie_hot(page=1, count=max_limit), next_media_type)
                if next_source == 'douban_tv_hot':
                    return collect_items(await chain.async_douban_tv_hot(page=1, count=max_limit), next_media_type)
                if next_source == 'douban_movie_showing':
                    return collect_items(await chain.async_douban_movie_showing(page=1, count=max_limit), next_media_type)
                if next_source == 'douban_movie_top250':
                    return collect_items(await chain.async_douban_movie_top250(page=1, count=max_limit), next_media_type)
                if next_source == 'douban_tv_animation':
                    return collect_items(await chain.async_douban_tv_animation(page=1, count=max_limit), next_media_type)
                if next_source == 'bangumi_calendar':
                    return collect_items(await chain.async_bangumi_calendar(page=1, count=max_limit), next_media_type)
                if next_source == 'douban_hot':
                    mixed = collect_items(await chain.async_douban_movie_hot(page=1, count=max_limit), 'movie')
                    if media_type_name in {'all', 'tv'}:
                        mixed.extend(collect_items(await chain.async_douban_tv_hot(page=1, count=max_limit), 'tv'))
                    for idx, item in enumerate(mixed, start=1):
                        item['index'] = idx
                    return mixed[:max_limit]
                return []
            fallback_candidates: List[Tuple[str, str]] = []
            if media_type_name == 'tv':
                fallback_candidates = [('douban_tv_hot', 'tv'), ('douban_tv_animation', 'tv'), ('tmdb_tvs', 'tv'), ('tmdb_trending', 'tv')]
            elif media_type_name == 'movie':
                fallback_candidates = [('douban_movie_showing', 'movie'), ('douban_movie_hot', 'movie'), ('tmdb_movies', 'movie'), ('tmdb_trending', 'movie')]
            else:
                fallback_candidates = [('douban_hot', 'all'), ('douban_movie_showing', 'movie'), ('tmdb_trending', 'all')]
            if not items:
                for candidate_source, candidate_media_type in fallback_candidates:
                    if candidate_source == source_name:
                        continue
                    candidate_items = await fetch_source_items(candidate_source, candidate_media_type)
                    if candidate_items:
                        fallback_source = candidate_source
                        fallback_notice = f'{source_name} 当前暂无结果，已自动回退 {candidate_source}。'
            ...
            if fallback_notice:
                lines.append(f'注：{fallback_notice}')
            ...
            if cache_key:
                self._save_session(cache_key, {'kind': 'assistant_mp_recommend', 'stage': 'result', 'source': fallback_source or source_name, 'requested_source': source_name, 'media_type': media_type_name, 'keyword': '', 'items': items, 'target_path': '', 'decision_summary': decision_summary, 'detail_short_command': '详情', 'plan_short_command': '计划', 'confirm_short_command': '确认', 'decision_short_command': '决策', 'pansou_short_command': '盘搜', 'mp_short_command': '原生'})
            ...
            return {'success': False, 'message': f'MP 推荐失败：{exc}', 'data': self._assistant_response_data(session=session, data={'action': 'mp_recommendations', 'ok': False})}

    async def _assistant_streaming_recommend(self, *, media_type: str='all', intent: str='hot', month: str='', window_days: int=90, session: str='default', cache_key: str='', compact: bool=False) -> Dict[str, Any]:
        """流媒体推荐：TMDB discover 直连，返回按热度+评分综合排序的推荐列表"""
        try:
            from datetime import date as _date, timedelta as _td
            from .services.streaming_recommend import StreamingRecommendService
        except Exception as exc:
            return {'success': False, 'message': f'流媒体推荐加载失败：{exc}', 'data': self._assistant_response_data(session=session, data={'action': 'streaming_recommend', 'ok': False})}
        tmdb_api_key = self._read_tmdb_api_key()
        if not tmdb_api_key:
            return {'success': False, 'message': 'TMDB API Key 未配置，无法查询流媒体推荐。请在 MoviePilot 系统设置中配置 TMDB API Key。', 'data': self._assistant_response_data(session=session, data={'action': 'streaming_recommend', 'ok': False})}
        today = _date.today()
        start_date = ''
        end_date = ''
        if month:
        ...
        result = await service.query(media_type=media_type, intent=intent, start_date=start_date, end_date=end_date, window_days=window_days, limit=10)
        if not result.get('success'):
            return {'success': False, 'message': result.get('message') or '流媒体推荐查询失败。', 'data': self._assistant_response_data(session=session, data={'action': 'streaming_recommend', 'ok': False})}
        ...
        if not items:
            ...
            return {'success': True, 'message': f'流媒体推荐（{time_desc} · {type_desc}）暂无结果。请换个条件试试。', 'data': self._assistant_response_data(session=session, data={'action': 'streaming_recommend', 'ok': True, 'items': []})}
        ...
        for item in items:
            ...
            lines.append(f'{idx}. {emoji} {title} ({year}) | 评分 {avg} | 热度 {pop}')
            lines.append(f'   上线：{display_release} | {reason}')
        if cache_key:
            self._save_session(cache_key, {'kind': 'assistant_streaming_recommend', 'stage': 'result', 'media_type': media_type, 'intent': intent, 'month': month, 'window_days': window_days, 'items': items, 'query_params': query_params})
        return {'success': True, 'message': '\n'.join(lines), 'data': self._assistant_response_data(session=session, data={'action': 'streaming_recommend', 'ok': True, 'media_type': media_type, 'intent': intent, 'month': month, 'window_days': window_days, 'items': items, 'query_params': query_params})}
    ...
    def _format_mp_recommend_item_detail_text(self, item: Dict[str, Any]) -> str:
        ...
        lines = ['MP 推荐条目详情', f'标题：{title}', f'年份：{year}', f'类型：{media_type}', f'评分：{vote}', f'TMDB：{tmdb_id}', f'豆瓣：{douban_id}']
        return '\n'.join(lines)

    def _assistant_mp_recommendation_decision_summary(self) -> Dict[str, Any]:
        return {'decision_mode': 'show_detail', 'decision_reason': '推荐列表默认先看当前榜单首项详情，再决定是否生成计划或直接确认执行。', 'preferred_command': '详情', 'fallback_command': '计划', 'compact_commands': ['详情', '计划'], 'command_policy': 'safe_read_only', 'preferred_requires_confirmation': False, 'fallback_requires_confirmation': False, 'can_auto_run_preferred': True, 'recommended_agent_behavior': 'show_only', 'detail_short_command': '详情', 'decision_short_command': '决策', 'plan_short_command': '计划', 'confirm_short_command': '确认', 'pansou_short_command': '盘搜', 'mp_short_command': '原生'}
    ...
    def _assistant_recommend_handoff_public_data(self, state: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {'source': self._clean_text(handoff.get('source')), 'requested_source': self._clean_text(handoff.get('requested_source')), 'media_type': self._clean_text(handoff.get('media_type')), 'selected_index': selected_index if selected_index > 0 else None, 'selected_title': self._clean_text(selected_item.get('title')), 'return_short_command': '回推荐', 'source_short_commands': {'pansou': '盘搜', 'mp': '原生'}}
    ...
    async def _assistant_switch_recommend_handoff_source(
        self,
        request,
        *,
        session: str,
        ...
        if not isinstance(handoff, dict) or not handoff:
            return {
                "success": False,
                "message": "当前会话没有可切换的推荐上下文，请重新发送：智能发现 热门电影。",
                "data": self._assistant_response_data(session=session, data={
                    "action": "switch_recommend_handoff_source",
                    "ok": False,
                    "error_code": "recommend_handoff_missing",
                }),
            }
        ...
        if not keyword:
            return {
                "success": False,
                "message": "当前推荐上下文缺少标题，无法切换源。请先发送：回推荐。",
                "data": self._assistant_response_data(session=session, data={
                    "action": "switch_recommend_handoff_source",
                    "ok": False,
    ...
    async def _assistant_run_recommend_source_compound(self, request, *, session: str, cache_key: str, state: Dict[str, Any], mode: str, followup_action: str, compact: bool, target_path: str) -> Dict[str, Any]:
        ...
        if mode not in {'pansou', 'mp'}:
            return {'success': False, 'message': '当前只支持 盘搜/原生 的单条详情、计划、确认命令。', 'data': self._assistant_response_data(session=session, data={'action': 'recommend_source_compound', 'ok': False, 'error_code': 'unsupported_recommend_source_compound_mode'})}
        if kind == 'assistant_mp_recommend':
            selected_index = max(1, self._safe_int(state.get('selected_index'), 0) or 1)
            source_result = await self.api_assistant_pick(_JsonRequestShim(request, {'session': session, 'session_id': cache_key, 'index': selected_index, 'mode': mode, 'path': target_path, 'compact': compact, 'apikey': self._extract_apikey(request, {})}))
            if not source_result.get('success'):
                return source_result
        else:
            kind_mode = {'assistant_pansou': 'pansou', 'assistant_mp': 'mp'}.get(kind, '')
            if kind_mode != mode:
                source_result = await self._assistant_switch_recommend_handoff_source(request, session=session, cache_key=cache_key, state=state, mode=mode)
                if not source_result.get('success'):
                    return source_result
        return await self.api_assistant_pick(_JsonRequestShim(request, {'session': session, 'session_id': cache_key, 'index': 0, 'action': followup_action, 'path': target_path, 'compact': compact, 'apikey': self._extract_apikey(request, {})}))
    ...
    async def _assistant_confirm_recommend_handoff(self, request, *, session: str, cache_key: str, state: Dict[str, Any], compact: bool, target_path: str) -> Dict[str, Any]:
        pending_plan = self._find_workflow_plan(session=session, session_id=cache_key, executed=False)
        ...
        if pending_plan:
            result = await self.api_assistant_plan_execute(_JsonRequestShim(request, {'session': session, 'session_id': cache_key, 'prefer_unexecuted': True, 'compact': compact, 'apikey': self._extract_apikey(request, {})}))
        ...
        kind = self._clean_text(state.get('kind'))
        if kind in {'assistant_pansou', 'assistant_mp'}:
            result = await self.api_assistant_pick(_JsonRequestShim(request, {'session': session, 'session_id': cache_key, 'choice': 0, 'action': 'best_execute', 'path': target_path, 'compact': compact, 'apikey': self._extract_apikey(request, {})}))
            if handoff_state.get('recommend_handoff') and (not bool(result.get('success'))):
                result_data = dict(result.get('data') or {})
                result_data.update(self._assistant_recommend_handoff_short_metadata(handoff_state))
                result_data['followup_summary'] = self._assistant_recommend_handoff_execute_failure_followup(handoff_state)
                ...
                result['data'] = result_data
            return result
        return {'success': False, 'data': self._assistant_response_data(session=session, data={'action': 'confirm_recommend_handoff', 'ok': False, 'error_code': 'recommend_handoff_pending_plan_missing', 'preferred_command': '决策', 'fallback_command': '回推荐'})}

    def _assistant_recommend_handoff_short_metadata(self, state: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        ...
        return {'recommend_handoff': dict(handoff) if handoff else {}, 'return_short_command': self._clean_text(handoff.get('return_short_command') or '回推荐') if handoff else '', 'detail_short_command': '详情', 'decision_short_command': '决策', 'plan_short_command': '计划', 'confirm_short_command': '确认', 'pansou_short_command': self._clean_text(source_short_commands.get('pansou') or '盘搜') if handoff else '', 'mp_short_command': self._clean_text(source_short_commands.get('mp') or '原生') if handoff else ''}
    ...
    def _assistant_recommend_handoff_execute_failure_followup(self, state: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        meta = self._assistant_recommend_handoff_short_metadata(state)
        return {'category': 'recommend_handoff', 'stage': 'write_failed', 'label': '执行失败，建议换路', 'preferred_action': 'return_to_smart_decision', 'decision_hint': '当前推荐条目的执行失败；优先回到统一资源决策，或回推荐后切换其它源。', 'command_policy': 'safe_read_recovery', 'preferred_requires_confirmation': False, 'fallback_requires_confirmation': False, 'can_auto_run_preferred': False, 'preferred_command': '决策', 'fallback_command': meta.get('return_short_command') or '回推荐', 'compact_commands': ['决策', meta.get('return_short_command') or '回推荐'], 'recommended_commands': ['决策', meta.get('return_short_command') or '回推荐', meta.get('mp_short_command') or '原生'], 'recommended_agent_behavior': 'show_only'}
    ...
    def _format_p115_next_actions(self, status: Optional[Dict[str, Any]]=None) -> str:
        ...
        if current.get('ready'):
            lines = ['下一步建议：', f'1. 直接发：链接 https://115cdn.com/s/xxxx path={final_path}', '2. 也可以直接贴 115 链接，不写前缀也能识别', '4. 外部搜资源可发：盘搜搜索 片名', '5. 想复查登录状态可发：115状态']
        else:
            lines = ['下一步建议：', '1. 回复：115登录', '2. 扫码确认后回复：检查115登录', '3. 登录完成后可回复：115状态', f'4. 然后可直接发 115 链接转存到 {final_path}']
        return '\n'.join(lines)
    ...
    def _assistant_recovery_public_data(self, *, session_state: Optional[Dict[str, Any]]=None, action_templates: Optional[List[Dict[str, Any]]]=None) -> Dict[str, Any]:
        ...
        if saved_plan.get('has_pending'):
            ...
            reason = '当前会话存在待执行计划'
            template = self._assistant_find_action_template(templates, ['execute_latest_plan', 'execute_plan', 'execute_session_latest_plan'])
        elif latest_plan_executed and latest_plan_id:
            mode = 'followup_executed_plan'
            reason = '当前会话最近一条计划已执行，建议先做统一后续追踪'
            template = self._assistant_action_template(name='query_execution_followup', description='按最近已执行计划自动追踪下载、订阅或入库后续状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'plan_id': latest_plan_id})
        elif pending_p115.get('has_pending'):
            mode = 'resume_pending_115'
            reason = '当前会话存在待继续的 115 任务'
            template = self._assistant_find_action_template(templates, ['resume_pending_115', 'check_115_login'])
            ...
            template = self._assistant_find_action_template(templates, ['query_mp_search_result_detail', 'pick_mp_download'])
        elif has_session and kind == 'assistant_mp':
            mode = 'continue_mp_search'
            reason = '当前会话停留在 MP 原生搜索结果列表'
            template = self._assistant_find_action_template(templates, ['query_mp_best_result_detail', 'query_mp_search_result_detail', 'pick_mp_download'])
        elif has_session and kind == 'assistant_mp_recommend':
            mode = 'continue_mp_recommend'
            reason = '当前会话停留在 MP 热门推荐列表'
            template = self._assistant_find_action_template(templates, ['pick_recommend_smart_decision', 'pick_recommend_smart_plan', 'pick_recommend_mp_search'])
        elif has_session and kind == 'assistant_mp_download_tasks':
            mode = 'continue_mp_download_tasks'
            reason = '当前会话停留在 MP 下载任务列表'
            template = self._assistant_find_action_template(templates, ['query_mp_download_history', 'pause_mp_download', 'resume_mp_download', 'delete_mp_download'])
        elif has_session and kind == 'assistant_mp_download_history':
            mode = 'continue_mp_download_history'
            reason = '当前会话停留在 MP 下载历史'
            template = self._assistant_find_action_template(templates, ['query_mp_lifecycle_status', 'start_mp_media_search'])
        elif has_session and kind == 'assistant_mp_downloaders':
            mode = 'continue_mp_downloaders'
            reason = '当前会话停留在 MP 下载器状态'
            template = self._assistant_find_action_template(templates, ['query_mp_sites', 'start_mp_media_search'])
        elif has_session and kind == 'assistant_mp_sites':
            mode = 'continue_mp_sites'
            reason = '当前会话停留在 MP 站点状态'
            template = self._assistant_find_action_template(templates, ['query_mp_downloaders', 'start_mp_media_search'])
        elif has_session and kind == 'assistant_mp_subscribes':
            mode = 'continue_mp_subscribes'
            reason = '当前会话停留在 MP 订阅列表'
            template = self._assistant_find_action_template(templates, ['start_mp_subscribe', 'search_mp_subscribe', 'start_mp_media_search'])
        elif has_session and kind == 'assistant_mp_lifecycle_status':
            mode = 'continue_mp_lifecycle_status'
            reason = '当前会话停留在 MP 生命周期追踪'
            template = self._assistant_find_action_template(templates, ['query_mp_download_history', 'start_mp_media_search'])
            ...
            template = self._assistant_find_action_template(templates, ['start_pansou_search', 'start_115_login'])
        can_resume = mode != 'start_new' and bool(template)
        return {'mode': mode, 'reason': reason, 'can_resume': can_resume, 'recommended_action': self._clean_text((template or {}).get('name')), 'recommended_tool': self._clean_text((template or {}).get('tool')), 'action_template': template or None, 'alternatives': [self._clean_text(item.get('name')) for item in templates[:6] if self._clean_text(item.get('name'))]}
    ...
    def _assistant_session_public_data(self, session: str='default') -> Dict[str, Any]:
        ...
        state = self._load_session(session_id) or {}
        if not state:
            payload = {'has_session': False, 'session': session_name, 'session_id': session_id, 'saved_plan': saved_plan, 'suggested_actions': ['execute_plan.session', 'smart_entry'] if saved_plan.get('has_pending') else ['smart_entry']}
        ...
        payload: Dict[str, Any] = {'has_session': True, 'session': session_name, 'session_id': session_id, 'kind': kind, 'stage': stage, 'updated_at': self._safe_int(state.get('updated_at'), 0), 'updated_at_text': self._format_unix_time(state.get('updated_at')), 'target_path': target_path, 'keyword': self._clean_text(state.get('keyword')), 'media_type': self._clean_text(state.get('media_type')), 'year': self._clean_text(state.get('year')), 'pending_p115': self._pending_p115_public_data(state), 'saved_plan': saved_plan, 'suggested_actions': []}
        if kind == 'assistant_pansou':
            items = state.get('items') or []
            payload.update({'result_count': len(items), 'recommend_handoff': self._assistant_recommend_handoff_public_data(state), 'items_preview': [{'index': self._safe_int(item.get('index'), idx + 1), 'channel': self._clean_text(item.get('channel')), 'title': self._clean_text(item.get('note')), 'source': self._clean_text(item.get('source'))} for idx, item in enumerate(items[:6]) if isinstance(item, dict)], 'score_summary': self._score_summary(items, limit=5), 'suggested_actions': ['smart_pick.choice', 'session_clear']})
            if payload.get('recommend_handoff'):
                payload['suggested_actions'] = ['smart_entry.text=回推荐', 'smart_entry.text=决策', 'smart_entry.text=详情', 'smart_entry.text=计划', 'smart_entry.text=确认', 'smart_entry.text=原生', *list(payload.get('suggested_actions') or [])]
        elif kind == 'assistant_mp':
            items = state.get('items') or []
            payload.update({'result_count': len(items), 'recommend_handoff': self._assistant_recommend_handoff_public_data(state), 'items_preview': [{'index': self._safe_int(item.get('index'), idx + 1), 'title': self._clean_text((item.get('torrent_info') or {}).get('title') or item.get('title')), 'site': self._clean_text((item.get('torrent_info') or {}).get('site_name')), 'seeders': (item.get('torrent_info') or {}).get('seeders'), 'volume_factor': self._clean_text((item.get('torrent_info') or {}).get('volume_factor')), 'score': (item.get('score') or {}).get('score') if isinstance(item.get('score'), dict) else None, 'score_level': (item.get('score') or {}).get('score_level') if isinstance(item.get('score'), dict) else '', 'recommended_action': (item.get('score') or {}).get('recommended_action') if isinstance(item.get('score'), dict) else '', 'risk_reasons': (item.get('score') or {}).get('risk_reasons', [])[:2] if isinstance(item.get('score'), dict) else []} for idx, item in enumerate(items[:8]) if isinstance(item, dict)], 'score_summary': self._score_summary(items, limit=5), 'suggested_actions': ['mp_download.choice', 'mp_subscribe.keyword', 'session_clear']})
            if payload.get('recommend_handoff'):
                payload['suggested_actions'] = ['smart_entry.text=回推荐', 'smart_entry.text=决策', 'smart_entry.text=详情', 'smart_entry.text=计划', 'smart_entry.text=确认', 'smart_entry.text=盘搜', *list(payload.get('suggested_actions') or [])]
        elif kind == 'assistant_mp_candidate':
            candidates = state.get('candidates') or []
            payload.update({'stage': 'candidate', 'candidate_count': len(candidates), 'page': self._safe_int(state.get('page'), 1), 'candidates_preview': [{'index': idx + 1, 'title': self._clean_text(item.get('title')), 'year': self._clean_text(item.get('year')), 'media_type': self._clean_text(item.get('media_type') or item.get('type')), 'tmdb_id': item.get('tmdb_id')} for idx, item in enumerate(candidates[:10]) if isinstance(item, dict)], 'suggested_actions': ['smart_pick.choice', 'smart_pick.action=详情', 'smart_pick.action=下一页', 'session_clear']})
        elif kind == 'assistant_mp_download_tasks':
            items = state.get('items') or []
            payload.update({'result_count': len(items), 'items_preview': [{'index': self._safe_int(item.get('index'), idx + 1), 'title': self._clean_text(item.get('title')), 'hash_short': self._clean_text(item.get('hash_short')), 'downloader': self._clean_text(item.get('downloader')), 'progress': self._clean_text(item.get('progress')), 'state': self._clean_text(item.get('state'))} for idx, item in enumerate(items[:8]) if isinstance(item, dict)], 'suggested_actions': ['mp_download_control.pause', 'mp_download_control.resume', 'mp_download_control.delete', 'session_clear'] if items else ['mp_media_search', 'mp_download_history', 'session_clear']})
        elif kind == 'assistant_mp_download_history':
            items = state.get('items') or []
            payload.update({'result_count': len(items), 'items_preview': [{'index': self._safe_int(item.get('index'), idx + 1), 'title': self._clean_text(item.get('title')), 'year': self._clean_text(item.get('year')), 'date': self._clean_text(item.get('date')), 'transfer_status_text': self._clean_text(item.get('transfer_status_text')), 'download_hash_short': self._clean_text(item.get('download_hash_short'))} for idx, item in enumerate(items[:8]) if isinstance(item, dict)], 'suggested_actions': ['mp_lifecycle_status', 'mp_media_search', 'session_clear']})
        elif kind == 'assistant_mp_downloaders':
            items = state.get('items') or []
            payload.update({'enabled_count': self._safe_int(state.get('enabled_count'), 0), 'result_count': len(items), 'items_preview': [{'name': self._clean_text(item.get('name')), 'type': self._clean_text(item.get('type')), 'enabled': bool(item.get('enabled')), 'default': bool(item.get('default'))} for item in items[:8] if isinstance(item, dict)], 'suggested_actions': ['mp_sites', 'mp_media_search', 'session_clear']})
        elif kind == 'assistant_mp_sites':
            items = state.get('items') or []
            payload.update({'status': self._clean_text(state.get('status')), 'result_count': len(items), 'items_preview': [{'index': self._safe_int(item.get('index'), idx + 1), 'name': self._clean_text(item.get('name')), 'domain': self._clean_text(item.get('domain')), 'enabled': bool(item.get('enabled')), 'has_cookie': bool(item.get('has_cookie')), 'priority': item.get('priority')} for idx, item in enumerate(items[:8]) if isinstance(item, dict)], 'suggested_actions': ['mp_downloaders', 'mp_media_search', 'session_clear']})
        elif kind == 'assistant_mp_subscribes':
            items = state.get('items') or []
            payload.update({'result_count': len(items), 'items_preview': [{'index': self._safe_int(item.get('index'), idx + 1), 'id': self._safe_int(item.get('id'), 0), 'title': self._clean_text(item.get('name')), 'year': self._clean_text(item.get('year')), 'state': self._clean_text(item.get('state')), 'lack_episode': item.get('lack_episode')} for idx, item in enumerate(items[:8]) if isinstance(item, dict)], 'suggested_actions': ['mp_subscribe_control.search', 'mp_subscribe_control.pause', 'mp_subscribe_control.resume', 'mp_subscribe_control.delete', 'session_clear'] if items else ['mp_subscribe.keyword', 'mp_media_search', 'session_clear']})
    ...
    def _assistant_session_brief_public_data(self, session_id: str, state: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        result: Dict[str, Any] = {'session': session_name, 'session_id': name or self._assistant_session_id(session_name), 'kind': self._clean_text(payload.get('kind')), 'stage': self._clean_text(payload.get('stage')), 'updated_at': self._safe_int(payload.get('updated_at'), 0), 'updated_at_text': self._format_unix_time(payload.get('updated_at')), 'keyword': self._clean_text(payload.get('keyword')), 'target_path': self._clean_text(payload.get('target_path')), 'has_pending_p115': bool(self._clean_text((payload.get('pending_p115') or {}).get('share_url'))), 'has_saved_plan': bool(saved_plan.get('has_plan')), 'has_pending_plan': bool(saved_plan.get('has_pending')), 'saved_plan': saved_plan.get('latest')}
        ...
        result['recovery'] = self._assistant_recovery_public_data(session_state={'pending_p115': self._pending_p115_public_data(payload), 'saved_plan': saved_plan}, action_templates=[self._assistant_action_template(name='execute_session_latest_plan', description='按 session_id 执行该会话最近一条待执行计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/plan/execute', tool='agent_resource_officer_execute_plan', body={'session_id': result.get('session_id'), 'prefer_unexecuted': True}), self._assistant_action_template(name='inspect_session', description='查看某个会话的详细状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/session', tool='agent_resource_officer_session_state', body={'session_id': result.get('session_id')})])
        return result
    ...
    def _assistant_action_templates(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        base_route = {'session': session_name, 'session_id': session_id}
        base_pick = {'session': session_name, 'session_id': session_id}
        base_state = {'session': session_name, 'session_id': session_id}
        templates: List[Dict[str, Any]] = []
        preference_status = self._assistant_preferences_status_brief(session=session_name)
        preference_template = self._assistant_action_template(name='preferences_save', description='保存智能体片源偏好；首次接入建议先询问用户后再保存', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/preferences', tool='agent_resource_officer_preferences', body={'preferences': self._assistant_default_preferences_template()})
        if not data.get('has_session'):
            ...
            if saved_plan.get('has_pending'):
                templates.append(self._assistant_action_template(name='execute_latest_plan', description='执行当前会话最近一条待执行计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/plan/execute', tool='agent_resource_officer_execute_plan', body={'prefer_unexecuted': True}))
            templates.extend([self._assistant_action_template(name='start_pansou_search', description='发起新的盘搜搜索', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'mode': 'pansou', 'keyword': '<关键词>'}), self._assistant_action_template(endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'keyword': '<关键词>', 'media_type': 'auto'}), self._assistant_action_template(name='start_mp_media_search', description='发起新的 MP 原生搜索，返回 PT 候选和评分摘要', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'mode': 'mp', 'keyword': '<关键词>'}), self._assistant_action_template(name='query_mp_media_detail', description='使用 MoviePilot 原生识别确认媒体信息和 TMDB/Douban/IMDB ID', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_media_detail', 'keyword': '<关键词>', 'media_type': 'auto'}), self._assistant_action_template(name='start_mp_recommendations', description='查看 MP 原生热门推荐，例如 TMDB、豆瓣或 Bangumi', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'start_mp_recommendations', 'source': 'tmdb_trending', 'media_type': 'all'}), self._assistant_action_template(name='query_mp_download_tasks', description='查看 MP 下载任务状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_download_tasks', 'status': 'downloading'}), self._assistant_action_template(name='query_mp_download_history', description='查看 MP 下载历史，并关联整理/入库状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_download_history', 'limit': 10}), self._assistant_action_template(name='query_mp_lifecycle_status', description='聚合查看 MP 下载任务、下载历史和整理/入库历史', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_lifecycle_status', 'keyword': '<关键词>', 'limit': 5}), self._assistant_action_template(name='query_mp_downloaders', description='查看 MP 下载器配置摘要', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_downloaders'}), self._assistant_action_template(name='query_mp_sites', description='查看 MP 站点启用状态和 Cookie 是否存在，不返回 Cookie 明文', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_sites', 'status': 'active', 'limit': 30}), self._assistant_action_template(name='query_mp_subscribes', description='查看 MP 订阅列表', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_subscribes', 'status': 'all', 'limit': 20}), self._assistant_action_template(name='query_mp_transfer_history', description='查看 MP 最近整理/入库历史，用于判断下载后是否已落库', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_transfer_history', 'status': 'all', 'limit': 10}), self._assistant_action_template(name='start_115_login', description='发起新的 115 扫码登录', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'action': 'p115_qrcode_start'})])
            return templates
        ...
        if preference_status.get('needs_onboarding'):
            templates.append(preference_template)
        templates.append(self._assistant_action_template(name='inspect_session_state', description='重新获取当前会话详细状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/session', tool='agent_resource_officer_session_state', body=base_state))
        if kind == 'assistant_pansou':
            templates.extend([self._assistant_action_template(name='pick_pansou_result', description='按编号选择盘搜结果继续转存', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/pick', tool='agent_resource_officer_smart_pick', body={'choice': '<1-N>', 'path': target_path or self._p115_default_path}), self._assistant_action_template(name='plan_pansou_result', description='按编号生成盘搜转存计划，不立即写入', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/pick', tool='agent_resource_officer_smart_pick', body={'choice': '<1-N>', 'action': 'plan', 'path': target_path or self._p115_default_path})])
            if isinstance(data.get('recommend_handoff'), dict) and data.get('recommend_handoff'):
                templates.extend([self._assistant_action_template(name='return_to_recommendations', description='返回当前盘搜结果对应的推荐榜单会话', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '回推荐'}), self._assistant_action_template(endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={}), self._assistant_action_template(name='handoff_pansou_decision', description='基于当前推荐条目回到统一资源决策', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '决策'}), self._assistant_action_template(name='handoff_pansou_best_detail', description='查看当前盘搜首选详情', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '详情'}), self._assistant_action_template(name='handoff_pansou_best_plan', description='为当前盘搜首选生成待确认计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '计划'}), self._assistant_action_template(name='handoff_pansou_confirm', description='优先执行当前待确认计划；如无待计划则直接执行当前盘搜首选', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '确认'}), self._assistant_action_template(name='switch_pansou_to_mp', description='基于当前推荐条目切到 MP 原生搜索结果', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '原生'})])
        elif kind == 'assistant_mp':
            templates.extend([self._assistant_action_template(name='pick_mp_best_download', description='按当前评分最高的 MP 搜索结果生成下载计划；不会静默下载', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'pick_mp_best_download'}), self._assistant_action_template(name='pick_mp_download', description='按编号为 MP 原生搜索结果生成下载计划；聊天里直接回编号会立即下载', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'pick_mp_download', 'choice': '<1-N>'}), self._assistant_action_template(name='start_mp_subscribe', description='按当前关键词生成 MP 订阅计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'start_mp_subscribe', 'keyword': data.get('keyword') or '<关键词>'})])
            if isinstance(data.get('recommend_handoff'), dict) and data.get('recommend_handoff'):
                templates.extend([self._assistant_action_template(name='return_to_recommendations', description='返回当前原生搜索结果对应的推荐榜单会话', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '回推荐'}), self._assistant_action_template(name='switch_mp_to_pansou', description='基于当前推荐条目切到盘搜结果', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '盘搜'}), self._assistant_action_template(name='handoff_mp_decision', description='基于当前推荐条目回到统一资源决策', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '决策'}), self._assistant_action_template(name='handoff_mp_best_detail', description='查看当前 MP 搜索里评分最高的候选详情', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '详情'}), self._assistant_action_template(name='handoff_mp_best_plan', description='为当前 MP 搜索里评分最高的候选生成待确认计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '计划'}), self._assistant_action_template(name='handoff_mp_confirm', description='优先执行当前待确认计划；如无待计划则直接执行当前 MP 首选', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'text': '确认'}), self._assistant_action_template(endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={})])
        elif kind == 'assistant_mp_download_tasks':
            has_items = bool(data.get('items')) or self._safe_int(data.get('result_count'), 0) > 0
            if has_items:
                templates.extend([self._assistant_action_template(name='pause_mp_download', description='按编号暂停下载任务；写入动作建议先 dry_run 生成计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'mp_download_control', 'control': 'pause', 'target': '<1-N>'}), self._assistant_action_template(name='resume_mp_download', description='按编号恢复下载任务；写入动作建议先 dry_run 生成计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'mp_download_control', 'control': 'resume', 'target': '<1-N>'}), self._assistant_action_template(name='delete_mp_download', description='按编号删除下载任务；默认不删除文件', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'mp_download_control', 'control': 'delete', 'target': '<1-N>', 'delete_files': False})])
            else:
                templates.extend([self._assistant_action_template(name='query_mp_download_history', description='当前没有下载中任务，改查下载历史和整理状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_download_history', 'limit': 10}), self._assistant_action_template(name='start_mp_media_search', description='重新发起 MP 原生搜索', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'mode': 'mp', 'keyword': '<关键词>'})])
        elif kind == 'assistant_mp_download_history':
            templates.extend([self._assistant_action_template(name='query_mp_lifecycle_status', description='按关键词聚合查看下载任务、下载历史和整理/入库状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_lifecycle_status', 'keyword': data.get('keyword') or '<关键词>', 'limit': 5}), self._assistant_action_template(name='start_mp_media_search', description='按关键词重新发起 MP 原生搜索', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'mode': 'mp', 'keyword': data.get('keyword') or '<关键词>'})])
        elif kind == 'assistant_mp_downloaders':
            templates.extend([self._assistant_action_template(name='query_mp_sites', description='查看 PT 站点启用状态和 Cookie 是否存在', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_sites', 'status': 'active', 'limit': 30}), self._assistant_action_template(name='start_mp_media_search', description='发起新的 MP 原生搜索，返回 PT 候选和评分摘要', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'mode': 'mp', 'keyword': '<关键词>'})])
        elif kind == 'assistant_mp_sites':
            templates.extend([self._assistant_action_template(name='query_mp_downloaders', description='查看 MP 下载器配置摘要', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_downloaders'}), self._assistant_action_template(name='start_mp_media_search', description='发起新的 MP 原生搜索，返回 PT 候选和评分摘要', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'mode': 'mp', 'keyword': '<关键词>'})])
        elif kind == 'assistant_mp_subscribes':
            has_items = bool(data.get('items')) or self._safe_int(data.get('result_count'), 0) > 0
            if has_items:
                templates.extend([self._assistant_action_template(name='search_mp_subscribe', description='按编号触发订阅搜索；写入动作建议先 dry_run 生成计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'mp_subscribe_control', 'control': 'search', 'target': '<1-N>'}), self._assistant_action_template(name='pause_mp_subscribe', description='按编号暂停订阅；写入动作建议先 dry_run 生成计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'mp_subscribe_control', 'control': 'pause', 'target': '<1-N>'}), self._assistant_action_template(name='resume_mp_subscribe', description='按编号恢复订阅；写入动作建议先 dry_run 生成计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'mp_subscribe_control', 'control': 'resume', 'target': '<1-N>'}), self._assistant_action_template(name='delete_mp_subscribe', description='按编号删除订阅', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'mp_subscribe_control', 'control': 'delete', 'target': '<1-N>'})])
            else:
                templates.extend([self._assistant_action_template(name='start_mp_subscribe', description='按关键词生成新的 MP 订阅计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'start_mp_subscribe', 'keyword': data.get('keyword') or '<关键词>'}), self._assistant_action_template(name='start_mp_media_search', description='按关键词重新发起 MP 原生搜索', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'mode': 'mp', 'keyword': data.get('keyword') or '<关键词>'})])
        elif kind == 'assistant_mp_lifecycle_status':
            templates.extend([self._assistant_action_template(name='query_mp_download_history', description='继续查看 MP 下载历史', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_download_history', 'title': data.get('keyword') or '', 'limit': 10}), self._assistant_action_template(name='start_mp_media_search', description='按当前关键词重新发起 MP 原生搜索', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'mode': 'mp', 'keyword': data.get('keyword') or '<关键词>'})])
        elif kind == 'assistant_mp_recommend':
            templates.extend([self._assistant_action_template(name='pick_recommend_smart_decision', description='按编号选择推荐条目并进入统一资源决策', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/pick', tool='agent_resource_officer_smart_pick', body={'choice': '<1-N>', 'mode': 'smart_decision'}), self._assistant_action_template(name='pick_recommend_smart_plan', description='按编号选择推荐条目并直接生成统一资源计划', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/pick', tool='agent_resource_officer_smart_pick', body={'choice': '<1-N>', 'mode': 'smart_plan'}), self._assistant_action_template(name='pick_recommend_smart_execute', description='按编号选择推荐条目并直接进入统一资源执行链', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/pick', tool='agent_resource_officer_smart_pick', body={'choice': '<1-N>', 'mode': 'smart_execute'}), self._assistant_action_template(name='pick_recommend_mp_search', description='按编号选择推荐条目并进入 MP 原生搜索', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/pick', tool='agent_resource_officer_smart_pick', body={'choice': '<1-N>', 'mode': 'mp'}), self._assistant_action_template(endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/pick', tool='agent_resource_officer_smart_pick', body={'choice': '<1-N>'}), self._assistant_action_template(name='pick_recommend_pansou_search', description='按编号选择推荐条目并进入盘搜搜索', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/pick', tool='agent_resource_officer_smart_pick', body={'choice': '<1-N>', 'mode': 'pansou'})])
        elif kind == 'assistant_p115_login':
            templates.extend([self._assistant_action_template(name='check_115_login', description='检查 115 扫码是否已确认，并在成功后自动继续待任务', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'action': 'p115_qrcode_check'}), self._assistant_action_template(name='show_115_status', description='查看当前 115 状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'action': 'p115_status'})])
        if pending.get('has_pending'):
            templates.extend([self._assistant_action_template(name='resume_pending_115', description='继续当前会话里待处理的 115 任务', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'action': 'p115_resume'}), self._assistant_action_template(name='cancel_pending_115', description='取消当前会话里待处理的 115 任务', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'action': 'p115_cancel'})])
        templates.append(self._assistant_action_template(name='clear_current_session', description='清理当前会话缓存', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/session/clear', tool='agent_resource_officer_session_clear', body=base_state))
        return templates
    ...
    def _assistant_plan_execute_followup(self, *, workflow: str, session: str, session_id: str, session_state: Optional[Dict[str, Any]]=None, ok: bool, plan_id: str='') -> Dict[str, Any]:
        ...
        if workflow_name == 'smart_resource_plan':
            inferred_source = self._clean_text(state.get('source_type') or (state.get('best_candidate') or {}).get('source_type') or plan_execute_body.get('mode')).lower()
            if inferred_source == 'mp':
                inferred_source = 'mp_pt'
            if inferred_source == 'mp_pt':
                workflow_name = 'mp_best_download'
        base_route = {'session': session_name, 'session_id': session_cache}
        ...
        if workflow_name in {'mp_best_download', 'mp_download', 'mp_search_download', 'mp_download_control'}:
            ...
            templates = [self._assistant_action_template(name='query_execution_followup', description='按最近已执行计划自动查询下载后的统一后续状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_execution_followup'}), self._assistant_action_template(name='query_mp_ingest_status', description='按关键词判断当前处于下载、整理、入库还是失败阶段', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_ingest_status', 'keyword': keyword_value, 'limit': 5}), self._assistant_action_template(name='query_mp_download_history', description='查看下载历史，并继续追踪整理/入库状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_download_history', 'keyword': keyword_value, 'limit': 10}), self._assistant_action_template(name='query_mp_lifecycle_status', description='按关键词聚合查看下载任务、下载历史和整理/入库状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_lifecycle_status', 'keyword': keyword_value, 'limit': 5}), self._assistant_action_template(name='query_mp_local_diagnose', description='汇总失败线索并给出本地/PT 入库诊断建议', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_local_diagnose', 'keyword': keyword_value, 'limit': 5})]
        elif workflow_name in {'mp_subscribe', 'mp_subscribe_control'}:
            ...
            templates = [self._assistant_action_template(name='query_execution_followup', description='按最近已执行计划自动查询订阅后的统一后续状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_execution_followup'}), self._assistant_action_template(name='query_mp_subscribes', description='查看订阅列表，确认当前订阅是否已生效', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_subscribes', 'status': 'all', 'keyword': keyword, 'limit': 20}), self._assistant_action_template(name='query_mp_ingest_status', description='按关键词判断当前处于下载、整理、入库还是失败阶段', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_ingest_status', 'keyword': keyword_value, 'limit': 5}), self._assistant_action_template(name='start_mp_media_search', description='按当前关键词重新发起 MP 原生搜索', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'mode': 'mp', 'keyword': keyword_value})]
            ...
            templates = []
            if keyword:
                templates.extend([self._assistant_action_template(name='query_mp_local_diagnose', description='查看这次二次识别关联的本地/PT 入库诊断', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_local_diagnose', 'keyword': keyword_value, 'limit': 5}), self._assistant_action_template(name='query_mp_ingest_status', description='按目标标题查看当前是否已重新识别、整理或入库', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_mp_ingest_status', 'keyword': keyword_value, 'limit': 5})])
            templates.extend([self._assistant_action_template(name='query_ai_sample_worklist', description='回看 AI 工作清单，继续处理剩余失败样本', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_ai_sample_worklist', 'keyword': keyword}), self._assistant_action_template(name='query_ai_failed_samples', description='回看 AI 原始失败样本，确认当前仍有哪些标题或路径失败', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_ai_failed_samples', 'keyword': keyword}), self._assistant_action_template(name='query_ai_sample_insights', description='查看 AI 样本洞察，确认失败原因是否仍然集中', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'name': 'query_ai_sample_insights', 'keyword': keyword})])
        ...
        followup_summary = self._assistant_followup_summary(category=category, stage=self._clean_text(workflow_stage), recommended_action=next_actions[0] if next_actions else '', follow_up_hint=follow_up_hint, next_actions=next_actions, action_templates=templates, keyword=keyword)
        return {'next_actions': next_actions, 'action_templates': templates, 'recommended_action': next_actions[0] if next_actions else '', 'follow_up_hint': follow_up_hint, 'followup_summary': followup_summary}
    ...
    def _assistant_compact_command_summary(payload: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        ...
        candidates = [('error_summary', data.get('error_summary') if isinstance(data.get('error_summary'), dict) else {}), ('followup_summary', data.get('followup_summary') if isinstance(data.get('followup_summary'), dict) else {}), ('decision_summary', data.get('decision_summary') if isinstance(data.get('decision_summary'), dict) else {}), ('score_summary', score_summary.get('decision') if isinstance(score_summary.get('decision'), dict) else {})]
        for source, summary in candidates:
            if not isinstance(summary, dict) or not summary:
                continue
            compact_commands = [AgentResourceOfficer._clean_text(item) for item in summary.get('compact_commands') or summary.get('recommended_commands') or [] if AgentResourceOfficer._clean_text(item)]
    ...
    def _assistant_single_action_compact_response(self, name: str, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        data = dict(response.get('data') or {})
        session_state = dict(data.get('session_state') or {})
        payload = {'protocol_version': 'assistant.v1', 'action': self._clean_text(data.get('action')) or self._clean_text(name) or 'execute_action', 'ok': bool(data.get('ok')) if 'ok' in data else bool(response.get('success')), 'compact': True, 'write_effect': data.get('write_effect') or self._assistant_write_effect_for_action(self._clean_text(data.get('action')) or self._clean_text(name)), 'error_code': self._clean_text(data.get('error_code')) or ('' if response.get('success') else 'assistant_error'), 'name': self._clean_text(name), 'session': self._clean_text(data.get('session') or session_state.get('session')) or 'default', 'session_id': self._clean_text(data.get('session_id') or session_state.get('session_id')), 'message_head': self._assistant_result_message_head(response.get('message')), 'kind': self._clean_text(session_state.get('kind')), 'stage': self._clean_text(session_state.get('stage')), 'next_actions': data.get('next_actions') or session_state.get('suggested_actions') or [], 'action_templates': data.get('action_templates') or []}
        error_summary = self._assistant_error_summary(error_code=payload.get('error_code'), recommended_action=self._clean_text(data.get('recommended_action')), message_head=payload.get('message_head'), next_actions=payload.get('next_actions'), action_templates=payload.get('action_templates'), keyword=self._clean_text(session_state.get('keyword')), hash_value=self._clean_text(session_state.get('hash')), target=self._clean_text(data.get('target')))
        if error_summary:
            payload['error_summary'] = error_summary
        ...
        if command_summary:
            payload.update(command_summary)
        return {'success': bool(response.get('success')), 'message': response.get('message') or '', 'data': payload}
    ...
    def _format_assistant_session_summary(self, session: str='default') -> str:
        ...
        if not data.get('has_session'):
            return '\n'.join(['当前没有活跃会话。', '可直接调用 smart_entry 发起新操作，例如：', '1. text=盘搜搜索 大君夫人', '3. text=链接 https://115cdn.com/s/xxxx path=/待整理'])
        lines = ['当前会话状态', f'会话：{data.get('session')}', f'类型：{data.get('kind') or '-'}', f'阶段：{data.get('stage') or '-'}']
        if data.get('keyword'):
    ...
    def _format_p115_help_text(self) -> str:
        ...
        lines = ['115 使用帮助', f'当前状态：{('可用' if status.get('ready') else '待登录/待修复')}', f'默认目录：{final_path}', '如果 115 转存因登录问题失败，我会记住这次任务；扫码成功后回复 检查115登录，会自动继续执行。', '常用示例：', f'1. 链接 https://115cdn.com/s/xxxx path={final_path}', '3. 盘搜搜索 大君夫人', '4. 115登录', '5. 检查115登录', '6. 115状态', '7. 继续115任务 / 取消115任务', self._format_p115_next_actions(status)]
        return '\n'.join(lines)

    def _format_assistant_help_text(self, session: str='default') -> str:
        session_name = self._clean_text(session) or 'default'
        lines = ['Agent影视助手 使用帮助', f'当前会话：{session_name}', '推荐优先使用原生 Tool：agent_resource_officer_smart_entry 与 agent_resource_officer_smart_pick。', 'smart_entry 常用示例：', '1. text=盘搜搜索 大君夫人', '2. text=搜索 大君夫人 默认走当前启用源顺序', '4. text=MP搜索 蜘蛛侠 或 PT搜索 蜘蛛侠', '5. text=115登录', '6. text=检查115登录', '7. text=链接 https://115cdn.com/s/xxxx path=/待整理', '8. text=链接 https://pan.quark.cn/s/xxxx 位置=分享', '10. text=下载任务；暂停下载 1 / 恢复下载 1 / 删除下载 1 会先生成计划', '11. text=站点状态；下载器状态 用于排查 PT 搜索/下载环境', '12. text=记录 片名 用于判断资源是否提交过下载并进入整理流程', '13. text=状态 片名 一次查看下载任务、下载历史和入库历史', '14. text=流媒体推荐 5月上新的大作 / 本月热门电影 / 近期热门剧', '15. text=识别 片名 使用 MoviePilot 原生识别确认 TMDB/Douban/IMDB 信息', '16. text=订阅列表；刷新订阅 1 / 暂停订阅 1 / 恢复订阅 1 / 删除订阅 1 会先生成计划', '17. text=入库记录；入库失败 片名 用于判断下载后是否已经整理落库', '18. text=执行计划 执行当前会话最近待执行计划；text=执行 plan-xxxx 精确执行指定计划', '20. text=后续 / 最近 / 入库 片名 / 诊断 片名 是更省 token 的本地/PT 跟踪短命令', '21. text=跟进 / 跟进 片名 是统一跟进入口：有已执行计划时自动跟执行后状态，有片名时直接看生命周期', 'smart_pick 常用示例：', '1. choice=1', '3. action=下一页', 'MP 搜索结果里，choice=1 / text=下载1 / text=计划1 都会直接下载；如需三方案下载请直接发 text=下载 片名。', 'MP 搜索结果里，action=最佳 仅用于评分推荐；普通聊天不要把 PT 编号改写成详情。', 'MP 搜索结果里，text=下载最佳 会按当前最高分候选生成下载计划，不会静默下载。', '说明：同一个 session 会自动串起候选列表、资源列表、115 待任务与扫码续跑。', self._format_p115_next_actions(self._p115_status_snapshot())]
    ...
    def _format_assistant_capabilities_text(self) -> str:
        ...
        defaults = data.get('defaults') or {}
        lines = ['Agent影视助手 能力说明', f'版本：{data.get('version')}', '推荐上层调用顺序：', '1. 先看 capabilities 或 assistant/startup', '2. 如需恢复会话，可先看 assistant/sessions', '3. 再调用 smart_entry', '4. 之后用 assistant/session 或 session_state 判断下一步', '5. 最后再调用 smart_pick 或 session_clear', '默认目录：', f'- 115：{defaults.get('p115_path')}', f'- 夸克：{defaults.get('quark_path')}', f'- 115 客户端：{defaults.get('p115_client_type')}', '搜索源总开关：', f'- 盘搜：{('开启' if defaults.get('pansou_enabled') else '关闭')}', f'- MP/PT：{('开启' if defaults.get('mp_pt_enabled') else '关闭')}', '默认评分策略：', f'- PT 最低做种数：{defaults.get('pt_min_seeders')}', f'- 建议确认分数线：{defaults.get('confirm_score_threshold')}', f'- 自动入库：{('开启' if defaults.get('auto_ingest_enabled') else '关闭')}；自动入库分数线：{defaults.get('auto_ingest_score_threshold')}', '启动聚合包：assistant/startup，一次返回 pulse、自检、核心工具、端点和恢复建议，适合外部智能体开场调用', '轻量启动探针：assistant/pulse，返回版本、关键服务状态与最佳恢复建议，适合外部智能体每次开场调用', '轻量工具清单：assistant/toolbox，返回推荐工具、端点、工作流和命令示例，适合外部智能体初始化系统提示', '轻量协议自检：assistant/selfcheck，返回 compact 模板、布尔解析和低 token 入口健康状态', '启动探针：assistant/readiness，可直接判断外部智能体是否可以开始调用；compact=true 可减少嵌套回执', '执行历史：assistant/history，可查看最近 action/workflow 的成功状态和摘要；compact=true 可减少嵌套回执', 'smart_entry 结构化字段：session / session_id / path / mode / keyword / url / access_code / media_type / year / client_type / action / plan_id', 'smart_entry 动作：assistant_help / p115_qrcode_start / p115_qrcode_check / p115_status / p115_help / p115_pending / p115_resume / p115_cancel', 'smart_entry 与 smart_pick 支持 compact=true，可减少搜索与选择链路嵌套回执', 'smart_pick 字段：session / session_id / choice / action / path / compact', 'smart_pick 动作：detail / next_page', '动作执行入口：assistant/action，可直接执行 action_templates 里的 name + body；compact=true 可减少嵌套回执', '批量动作入口：assistant/actions，可一次执行多步 action_body；compact=true 可减少嵌套回执', '计划执行入口：assistant/plan/execute，可执行 dry_run 返回的 plan_id；compact=true 可减少嵌套回执', '自然语言计划确认：smart_entry 支持“执行计划”和“执行 plan-xxxx”，用于确认已生成的下载、订阅或控制计划', '计划管理入口：assistant/plans 与 assistant/plans/clear，可查询或清理 dry_run 保存的计划；compact=true 可减少嵌套回执', '单入口恢复：assistant/recover，可自动选择最值得恢复的会话或计划；execute=true 时直接执行推荐动作；compact=true 可减少回执字段', '统一回执字段：protocol_version / action / ok / session / session_id / session_state / next_actions / action_templates']
        return '\n'.join(lines)
    ...
    def _assistant_readiness_public_data(self) -> Dict[str, Any]:
        ...
        if not self._pansou_enabled:
            warnings.append('盘搜已关闭，普通搜索和盘搜补查不会再走盘搜')
        if not self._mp_pt_enabled:
            warnings.append('MP/PT 原生搜索/下载已关闭，外部智能体不应再调用 MP搜索、PT搜索或下载链')
    ...
    def _format_assistant_readiness_text(self) -> str:
        ...
        lines = ['Agent影视助手 启动就绪', f'版本：{data.get('version')}', f'插件：{('已启用' if data.get('enabled') else '未启用')}', f'外部智能体：{('可以启动' if data.get('can_start') else '暂不可启动')}', f'115：{('可用' if p115.get('ready') else '不可用')}', f'夸克：{('已配置' if quark.get('configured') else '未配置')}', f'活跃会话：{(data.get('active_sessions') or {}).get('total') or 0}', f'待执行计划：{(data.get('saved_plans') or {}).get('pending') or 0}', '推荐入口：assistant/workflow 或 assistant/actions']
    ...
    def _assistant_pulse_public_data(self) -> Dict[str, Any]:
        ...
        recovery_data.update({'action': 'recover', 'ok': True, 'execute_requested': False, 'executed': False})
        ...
        return {'protocol_version': 'assistant.v1', 'action': 'pulse', 'ok': bool(self._enabled), 'version': self.plugin_version, 'enabled': self._enabled, 'can_start': bool(self._enabled), 'services': {'p115_ready': bool(p115_status.get('ready')), 'p115_direct_ready': bool(p115_status.get('direct_ready')), 'quark_configured': bool(self._quark_cookie)}, 'warnings': warnings, 'session': recovery_compact.get('session'), 'session_id': recovery_compact.get('session_id'), 'recovery': recovery_compact.get('recovery') or {}, 'selected_session': recovery_compact.get('selected_session'), 'next_actions': recovery_compact.get('next_actions') or [], 'action_templates': recovery_compact.get('action_templates') or [], 'recommended_endpoints': {'startup': '/api/v1/plugin/AgentResourceOfficer/assistant/startup', 'selfcheck': '/api/v1/plugin/AgentResourceOfficer/assistant/selfcheck', 'toolbox': '/api/v1/plugin/AgentResourceOfficer/assistant/toolbox', 'recover': '/api/v1/plugin/AgentResourceOfficer/assistant/recover?compact=true', 'workflow': '/api/v1/plugin/AgentResourceOfficer/assistant/workflow', 'actions': '/api/v1/plugin/AgentResourceOfficer/assistant/actions'}}

    def _format_assistant_pulse_text(self) -> str:
        data = self._assistant_pulse_public_data()
        services = data.get('services') or {}
        recovery = data.get('recovery') or {}
        lines = ['Agent影视助手 轻量启动状态', f'版本：{data.get('version')}', f'插件：{('已启用' if data.get('enabled') else '未启用')}', f'115：{('可用' if services.get('p115_ready') else '不可用')}', f'夸克：{('已配置' if services.get('quark_configured') else '未配置')}', f'恢复模式：{recovery.get('mode') or 'unknown'}']
        if recovery.get('recommended_action'):
            lines.append(f'推荐动作：{recovery.get('recommended_action')}')
        warnings = data.get('warnings') or []
        if warnings:
    ...
    def _format_assistant_startup_text(self) -> str:
        ...
        lines = ['Agent影视助手 启动聚合包', f'版本：{data.get('version')}', f'可启动：{('是' if data.get('ok') else '否')}', f'自检：{('通过' if not failed else '失败 ' + ', '.join(failed))}', f'恢复模式：{recovery.get('mode') or 'unknown'}', f'可执行模板：{len(data.get('action_templates') or [])} 个', '状态：活跃会话 {active_sessions}；过期会话 {stale_sessions}；保存计划 {saved_plans_total}；待执行计划 {saved_plans_pending}；已执行计划 {saved_plans_executed}'.format(**data.get('maintenance') or {}), '下一步：优先按 recovery 建议执行；没有待恢复任务时使用 workflow 或 smart_entry']
        warnings = data.get('warnings') or []
        if warnings:
            lines.append('提示：' + '；'.join((str(item) for item in warnings if item)))
    ...
    def _assistant_request_templates_response_data(self, limit: int=100, names: Any=None, recipe: Any=None, include_templates: bool=True) -> Dict[str, Any]:
        ...
        templates = {name: all_templates[name] for name in selected_names if name in all_templates} if selected_names else all_templates
        confirmation_required = [name for name, item in templates.items() if bool((item or {}).get('requires_confirmation'))]
        safe_without_confirmation = [name for name, item in templates.items() if not bool((item or {}).get('requires_confirmation'))]
        write_side_effects = [name for name, item in templates.items() if self._clean_text((item or {}).get('side_effect')) in {'write', 'depends_on_action', 'depends_on_session', 'depends_on_text'}]
        cacheable_templates = [name for name, item in templates.items() if self._safe_int((item or {}).get('cache_ttl_seconds'), 0) > 0]
        non_cacheable_templates = [name for name, item in templates.items() if self._safe_int((item or {}).get('cache_ttl_seconds'), 0) <= 0]
        auth = {'mode': 'query_apikey', 'query_param': 'apikey', 'url_template': '{base_url}{endpoint}?apikey={MP_API_TOKEN}', 'description': '调用插件 HTTP 接口时推荐使用 ?apikey=你的MP_API_TOKEN；MP Tool 调用不需要此参数。'}
        external_agent_execution_policy_contract = {'auto_continue': '直接执行 auto_run_command。', 'auto_continue_then_wait_confirmation': '先执行 auto_run_command，再停止并向用户展示 confirm_command。', 'wait_user_confirmation': '不要自动执行；先向用户展示 confirm_command 或 display_command。', 'show_only': '只展示 display_command，不要自动继续。', 'stop': '当前没有适合自动继续的命令，不要继续执行。'}
        external_agent_execution_loop_contract = [{'step': 'startup', 'template': 'startup_probe', 'when': '新会话开始时先读取启动聚合包。'}, {'step': 'decide', 'template': 'startup_probe', 'when': '结合恢复信息和推荐 recipe，确定先继续会话还是开始新流程。'}, {'step': 'route', 'template': 'route_text', 'when': '处理用户的自然语言资源指令。'}, {'step': 'policy', 'template': '', 'when': '读取 recommended_agent_behavior、auto_run_command、confirm_command 决定自动继续、确认或停止。'}, {'step': 'followup', 'template': 'execution_followup', 'when': '执行计划后继续追踪下载、入库或失败诊断。'}]
        entry_patterns = {'external_agent': {'label': '外部智能体', 'client_role': '客户端调度层', 'start_with': 'startup', 'decide_with': 'decide --summary-only', 'route_with': 'route --summary-only', 'followup_with': 'followup --summary-only', 'read_fields': ['recommended_agent_behavior', 'auto_run_command', 'confirm_command', 'display_command', 'preferred_command', 'compact_commands'], 'notes': '适用于 WorkBuddy、Hermes、OpenClaw（小龙虾）等外部智能体；优先使用 Skill/helper。'}, 'mp_builtin_agent': {'label': 'MP 内置智能体', 'client_role': '客户端调度层', 'start_with': 'assistant/request_templates', 'decide_with': 'agent_resource_officer_request_templates', 'route_with': 'agent_resource_officer_smart_entry', 'followup_with': 'agent_resource_officer_execution_followup', 'read_fields': ['recommended_recipe_detail', 'recommended_agent_behavior', 'preferred_command', 'compact_commands'], 'notes': '优先走 Agent Tool / request_templates，不在模型侧直拼底层资源 API。'}, 'feishu_channel': {'label': '飞书入口', 'client_role': '客户端消息入口', 'start_with': 'feishu message -> route', 'decide_with': '插件内置命令解析', 'route_with': 'route/pick/followup', 'followup_with': 'followup / smart_followup', 'read_fields': ['recommended_agent_behavior', 'preferred_command', 'fallback_command', 'compact_commands'], 'notes': '飞书只负责把消息送进同一套 assistant 协议；确认策略与外部智能体保持一致。'}}
        orchestration_contract = {'service_role': 'Agent影视助手 / AgentResourceOfficer 负责服务端能力执行。', 'client_role': '外部智能体、MP 内置智能体、飞书入口负责客户端调度与展示。', 'recommended_first_call': 'startup', 'recommended_decision_call': 'decide --summary-only', 'recommended_route_call': 'route --summary-only', 'recommended_followup_call': 'followup --summary-only', 'recommended_read_fields': ['recommended_agent_behavior', 'auto_run_command', 'confirm_command', 'display_command', 'preferred_command', 'compact_commands'], 'confirmation_rule': '写入动作默认确认制；只有明确标记可自动继续的只读步骤才自动续跑。'}
        entry_playbooks = {'external_agent': {'label': '外部智能体最小执行流', 'transport': 'skill_helper_or_http', 'steps': [{'step': 'startup', 'helper_command': 'python3 scripts/aro_request.py startup', 'http_call': {'method': 'GET', 'endpoint': '/api/v1/plugin/AgentResourceOfficer/assistant/startup', 'url_template': '{base_url}/api/v1/plugin/AgentResourceOfficer/assistant/startup?apikey={MP_API_TOKEN}'}, 'purpose': '读取启动状态、恢复建议和推荐 recipe。', 'read_fields': ['recommended_request_templates', 'recovery', 'services']}, {'step': 'decide', 'helper_command': 'python3 scripts/aro_request.py decide --summary-only', 'http_call': {'method': 'GET', 'endpoint': '/api/v1/plugin/AgentResourceOfficer/assistant/startup', 'url_template': '{base_url}/api/v1/plugin/AgentResourceOfficer/assistant/startup?apikey={MP_API_TOKEN}'}, 'purpose': '决定继续会话、初始化偏好还是直接进入 route。', 'read_fields': ['recommended_agent_behavior', 'auto_run_command', 'confirm_command']}, {'step': 'route', 'helper_command': "python3 scripts/aro_request.py route '<用户原始指令>' --session 'agent:<会话ID>' --summary-only", 'http_call': {'method': 'POST', 'endpoint': '/api/v1/plugin/AgentResourceOfficer/assistant/route', 'url_template': '{base_url}/api/v1/plugin/AgentResourceOfficer/assistant/route?apikey={MP_API_TOKEN}'}, 'purpose': '执行自然语言主入口。', 'read_fields': ['recommended_agent_behavior', 'auto_run_command', 'confirm_command', 'preferred_command', 'compact_commands']}, {'step': 'followup', 'helper_command': "python3 scripts/aro_request.py followup --session 'agent:<会话ID>' --summary-only", 'http_call': {'method': 'POST', 'endpoint': '/api/v1/plugin/AgentResourceOfficer/assistant/action', 'url_template': '{base_url}/api/v1/plugin/AgentResourceOfficer/assistant/action?apikey={MP_API_TOKEN}'}, 'purpose': '执行计划后继续追踪下载、入库或失败诊断。', 'read_fields': ['recommended_agent_behavior', 'auto_run_command', 'followup_summary']}], 'auto_rule': '优先读取 recommended_agent_behavior；只读步骤可自动续跑，写入步骤默认确认。'}, 'mp_builtin_agent': {'label': 'MP 内置智能体最小执行流', 'transport': 'moviepilot_agent_tool', 'steps': [{'step': 'request_templates', 'tool': 'agent_resource_officer_request_templates', 'tool_args': {'recipe': 'external_agent', 'compact': True}, 'purpose': '读取最小流程、确认策略和推荐入口。', 'read_fields': ['orchestration_contract', 'entry_patterns', 'recommended_recipe_detail']}, {'step': 'route', 'tool': 'agent_resource_officer_smart_entry', 'tool_args': {'text': '<用户原始指令>'}, 'purpose': '处理搜索、链接、登录状态等主入口。', 'read_fields': ['recommended_agent_behavior', 'preferred_command', 'compact_commands']}, {'step': 'followup', 'tool': 'agent_resource_officer_execution_followup', 'tool_args': {}, 'purpose': '执行计划后继续查看下载、入库和失败状态。', 'read_fields': ['followup_summary', 'preferred_command', 'compact_commands']}], 'auto_rule': '依然遵守服务端 compact 协议；不要在模型侧拼底层资源站点 API。'}, 'feishu_channel': {'label': '飞书入口最小执行流', 'transport': 'feishu_channel', 'steps': [{'step': 'message_in', 'channel': 'feishu', 'purpose': '用户消息进入内置 Channel。', 'read_fields': ['session', 'text', 'reply_target']}, {'step': 'route', 'internal': 'route / pick / followup', 'purpose': '复用同一套 assistant 协议，不维护单独状态机。', 'read_fields': ['recommended_agent_behavior', 'preferred_command', 'compact_commands']}, {'step': 'reply', 'channel': 'feishu', 'purpose': '按确认策略回消息、展示编号或提示下一步。', 'read_fields': ['display_command', 'confirm_command', 'message']}], 'auto_rule': '飞书只负责消息承载；写入动作依然由插件服务端统一确认。'}}
        recommended_sequence = [{'step': 'bootstrap', 'template': 'startup_probe', 'when': '每次新会话开始时先读取启动聚合包。'}, {'step': 'healthcheck', 'template': 'selfcheck_probe', 'when': '当外部智能体需要确认协议健康或怀疑环境变化时执行。'}, {'step': 'maintenance_preview', 'template': 'maintain_preview', 'when': '长会话或多轮执行前先看是否有低风险维护建议。'}, {'step': 'plan', 'template': 'workflow_dry_run', 'when': '需要先生成计划、等待用户确认或减少重复大 JSON 时使用。'}, {'step': 'execute_saved_plan', 'template': 'saved_plan_execute', 'when': '已确认 dry_run 计划后执行。'}, {'step': 'post_execute_followup', 'template': 'execution_followup', 'when': '计划执行成功后，自动决定先查下载历史、生命周期、订阅或入库历史。'}, {'step': 'continue_session', 'template': 'pick_continue'}, {'step': 'preferences_onboarding', 'template': 'preferences_get', 'when': '外部智能体首次接入时，先读取偏好和评分策略，再保存用户片源偏好。'}, {'step': 'mp_pt_mainline', 'template': 'mp_search', 'when': '需要 MP 原生 PT 搜索、评分、下载计划、订阅或任务追踪时使用。'}, {'step': 'mp_recommendation', 'template': 'mp_recommend', 'when': '需要 TMDB、豆瓣、Bangumi 等 MP 原生推荐，并继续搜索时使用。'}]
        recipes = [{'name': 'safe_bootstrap', 'description': '新会话安全启动：先拿启动聚合包，再自检，再看维护建议。', 'templates': recipe_templates_map['safe_bootstrap']}, {'name': 'plan_then_confirm', 'description': '先生成计划，等待用户确认后再执行保存计划。', 'templates': recipe_templates_map['plan_then_confirm']}, {'name': 'post_execute_followup', 'description': '执行计划后的统一只读跟踪：由插件自动选择最合适的后续查询动作。', 'templates': recipe_templates_map['post_execute_followup']}, {'name': 'continue_existing_session', 'templates': recipe_templates_map['continue_existing_session']}, {'name': 'maintenance_cycle', 'description': '先预览维护建议，再在确认后执行低风险维护。', 'templates': recipe_templates_map['maintenance_cycle']}, {'name': 'external_agent_quickstart', 'description': '外部智能体接入：启动探测后，把用户文本交给统一入口，再按编号继续。', 'templates': recipe_templates_map['external_agent_quickstart']}, {'name': 'preferences_onboarding', 'description': '首次接入时先读取偏好、评分策略，再保存用户的片源偏好画像。', 'templates': recipe_templates_map['preferences_onboarding']}, {'name': 'smart_decision', 'description': '统一搜索并返回明确的下一步决策：查看详情、生成计划或直接执行。', 'templates': recipe_templates_map['smart_decision']}, {'name': 'smart_search_plan', 'description': '统一搜索决策后，直接为当前首选生成待确认计划；仍需后续执行计划才会真正写入。', 'templates': recipe_templates_map['smart_search_plan']}, {'name': 'smart_search_execute', 'description': '统一搜索决策后，直接执行当前首选写入动作；只适用于用户明确要求立即执行的场景。', 'templates': recipe_templates_map['smart_search_execute']}, {'name': 'mp_pt_mainline', 'description': 'MP 原生 PT 主线：识别、搜索、评分、下载计划、任务、订阅、站点和入库追踪。', 'templates': recipe_templates_map['mp_pt_mainline']}, {'name': 'mp_recommendation', 'templates': recipe_templates_map['mp_recommendation']}]
        recipe_summaries: List[Dict[str, Any]] = []
        for recipe in recipes:
            template_names = [self._clean_text(item) for item in recipe.get('templates') or [] if self._clean_text(item)]
            current_templates = [templates[name] for name in template_names if isinstance(templates.get(name), dict)]
            recipe_summaries.append({'requires_confirmation': any((bool(item.get('requires_confirmation')) for item in current_templates)), 'has_write_effect': any((self._clean_text(item.get('side_effect')) in {'write', 'depends_on_action', 'depends_on_session', 'depends_on_text'} for item in current_templates)), 'cache_ttl_seconds': min([self._safe_int(item.get('cache_ttl_seconds'), 0) for item in current_templates] or [0])})
        ...
        recommended_recipe_detail = next((item for item in recipe_summaries if item.get('name') == recommended_recipe), {})
        recommended_recipe_templates = [name for name in recommended_recipe_detail.get('templates') or [] if name in all_templates]
    ...
    def _format_assistant_toolbox_text(self) -> str:
        ...
        lines = ['Agent影视助手 轻量工具清单', f'版本：{data.get('version')}', '推荐启动顺序：' + ' -> '.join((str(item) for item in (data.get('startup_order') or [])[:5])), '常用工作流：' + ' / '.join((str(item.get('name')) for item in workflows if item.get('name')))]
        return '\n'.join(lines)

    def _assistant_selfcheck_public_data(self) -> Dict[str, Any]:
        mp_media_search_params = inspect.signature(self._assistant_mp_media_search).parameters
        format_mp_search_params = inspect.signature(self._format_mp_search_text).parameters
        mp_search_signature_ok = 'result_filter' in mp_media_search_params
        mp_search_formatter_signature_ok = {'result_filter', 'latest_episode', 'episode_filter'}.issubset(format_mp_search_params)
        action_template = self._assistant_action_template(name='show_115_status', description='自检模板：查看 115 状态', endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/action', tool='agent_resource_officer_execute_action', body={'session': 'selfcheck'})
        route_template = self._assistant_action_template(endpoint='/api/v1/plugin/AgentResourceOfficer/assistant/route', tool='agent_resource_officer_smart_entry', body={'session': 'selfcheck', 'keyword': '蜘蛛侠', 'media_type': 'movie'})
        bool_cases = {'true': self._parse_bool_value('true', False), 'false': self._parse_bool_value('false', True), 'one': self._parse_bool_value('1', False), 'zero': self._parse_bool_value('0', True), 'off': self._parse_bool_value('off', True), 'default_true': self._parse_bool_value(None, True), 'default_false': self._parse_bool_value(None, False)}
        compact_templates_ok = all([(action_template.get('body') or {}).get('compact') is True, (action_template.get('action_body') or {}).get('compact') is True, (route_template.get('body') or {}).get('compact') is True, (route_template.get('action_body') or {}).get('compact') is True])
        bool_parse_ok = bool_cases['true'] is True and bool_cases['false'] is False and (bool_cases['one'] is True) and (bool_cases['zero'] is False) and (bool_cases['off'] is False) and (bool_cases['default_true'] is True) and (bool_cases['default_false'] is False)
        pulse = self._assistant_pulse_public_data()
        toolbox = self._assistant_toolbox_public_data()
        startup_request_templates = self._assistant_recommended_request_templates_data()
        startup_continue_request_templates = self._assistant_recommended_request_templates_data(recipe='continue', reason='selfcheck')
        maintain = self._assistant_maintain_public_data(execute=False)
        request_templates = self._assistant_request_templates_public_data(limit=100)
        filtered_request_templates = self._assistant_request_templates_response_data(limit=5, names='maintain_execute,missing_template')
        recipe_request_templates = self._assistant_request_templates_response_data(limit=5, names='startup_probe,maintain_preview,workflow_dry_run,saved_plan_execute,pick_continue,maintain_execute', include_templates=False)
        recipe_filtered_request_templates = self._assistant_request_templates_response_data(limit=5, recipe='plan', include_templates=False)
        external_recipe_request_templates = self._assistant_request_templates_response_data(limit=5, recipe='external_agent', include_templates=False)
        policy_only_request_templates = self._assistant_request_templates_response_data(limit=5, names='maintain_execute', include_templates=False)
        maintenance_templates = maintain.get('action_templates') or []
        maintenance_template_names = {self._clean_text(item.get('name')) for item in maintenance_templates if isinstance(item, dict)}
        maintenance_templates_compact_ok = all(((item.get('body') or {}).get('compact') is True and (item.get('action_body') or {}).get('compact') is True for item in maintenance_templates if isinstance(item, dict)))
        maintain_dry_run_ok = maintain.get('action') == 'maintain' and maintain.get('execute_requested') is False and (maintain.get('executed') is False) and {'clear_stale_sessions', 'clear_executed_plans'}.issubset(maintenance_template_names)
        protocol_ok = pulse.get('protocol_version') == 'assistant.v1' and toolbox.get('protocol_version') == 'assistant.v1' and (pulse.get('action') == 'pulse') and (toolbox.get('action') == 'toolbox') and (maintain.get('protocol_version') == 'assistant.v1')
        startup_request_templates_ok = startup_request_templates.get('recipe') == 'bootstrap' and startup_request_templates.get('include_templates') is False and (startup_request_templates.get('tool') == 'agent_resource_officer_request_templates') and ('{MP_API_TOKEN}' in self._clean_text(startup_request_templates.get('url_template'))) and (startup_continue_request_templates.get('recipe') == 'continue') and ((startup_continue_request_templates.get('tool_args') or {}).get('recipe') == 'continue') and bool(self._clean_text(startup_continue_request_templates.get('reason')))
        request_templates_ok = all((isinstance(request_templates.get(name), dict) and self._clean_text((request_templates.get(name) or {}).get('endpoint')) and self._clean_text((request_templates.get(name) or {}).get('method')) and self._clean_text((request_templates.get(name) or {}).get('tool')) and self._clean_text((request_templates.get(name) or {}).get('description')) and self._clean_text((request_templates.get(name) or {}).get('side_effect')) and isinstance((request_templates.get(name) or {}).get('requires_confirmation'), bool) and self._clean_text((request_templates.get(name) or {}).get('cache_scope')) and isinstance((request_templates.get(name) or {}).get('cache_ttl_seconds'), int) and isinstance((request_templates.get(name) or {}).get('tool_args'), dict) for name in ['startup_probe', 'selfcheck_probe', 'maintain_preview', 'maintain_execute', 'workflow_dry_run', 'saved_plan_execute', 'action_execute', 'route_text', 'pick_continue']))
        request_templates_filter_ok = list((filtered_request_templates.get('request_templates') or {}).keys()) == ['maintain_execute'] and filtered_request_templates.get('selected_names') == ['maintain_execute', 'missing_template'] and (filtered_request_templates.get('invalid_names') == ['missing_template']) and ((((filtered_request_templates.get('request_templates') or {}).get('maintain_execute') or {}).get('body') or {}).get('limit') == 5)
        request_templates_policy_ok = 'maintain_execute' in ((filtered_request_templates.get('execution_policy') or {}).get('confirmation_required') or []) and 'maintain_execute' in ((filtered_request_templates.get('execution_policy') or {}).get('write_side_effects') or [])
        request_templates_schema_ok = filtered_request_templates.get('schema_version') == self.request_templates_schema_version
        request_templates_cache_ok = ((filtered_request_templates.get('request_templates') or {}).get('maintain_execute') or {}).get('cache_scope') == 'no_cache' and ((filtered_request_templates.get('request_templates') or {}).get('maintain_execute') or {}).get('cache_ttl_seconds') == 0
        request_templates_sequence_ok = isinstance(filtered_request_templates.get('recommended_sequence'), list) and any((isinstance(item, dict) and self._clean_text(item.get('template')) == 'startup_probe' for item in filtered_request_templates.get('recommended_sequence') or [])) and any((isinstance(item, dict) and self._clean_text(item.get('template')) == 'saved_plan_execute' for item in filtered_request_templates.get('recommended_sequence') or []))
        request_templates_recipes_ok = isinstance(recipe_request_templates.get('recipes'), list) and any((isinstance(item, dict) and self._clean_text(item.get('name')) == 'safe_bootstrap' for item in recipe_request_templates.get('recipes') or [])) and any((isinstance(item, dict) and self._clean_text(item.get('name')) == 'plan_then_confirm' for item in recipe_request_templates.get('recipes') or []))
        request_templates_recipe_summary_ok = any((isinstance(item, dict) and self._clean_text(item.get('name')) == 'safe_bootstrap' and (item.get('requires_confirmation') is False) and (item.get('has_write_effect') is False) for item in recipe_request_templates.get('recipes') or [])) and any((isinstance(item, dict) and self._clean_text(item.get('name')) == 'plan_then_confirm' and (item.get('requires_confirmation') is True) and (item.get('has_write_effect') is True) for item in recipe_request_templates.get('recipes') or []))
        request_templates_recommended_recipe_ok = filtered_request_templates.get('recommended_recipe') == 'maintenance_cycle' and bool(self._clean_text(filtered_request_templates.get('recommended_recipe_reason'))) and (recipe_request_templates.get('recommended_recipe') == 'plan_then_confirm')
        request_templates_recipe_filter_ok = recipe_filtered_request_templates.get('requested_recipe') == 'plan' and recipe_filtered_request_templates.get('selected_recipe') == 'plan_then_confirm' and (recipe_filtered_request_templates.get('selected_names') == ['workflow_dry_run', 'saved_plan_execute']) and (recipe_filtered_request_templates.get('recommended_recipe') == 'plan_then_confirm') and (recipe_filtered_request_templates.get('templates_included') is False) and ('plan_then_confirm' in (recipe_filtered_request_templates.get('available_recipes') or [])) and ((recipe_filtered_request_templates.get('recipe_aliases') or {}).get('plan') == 'plan_then_confirm')
    ...
    def _merge_assistant_structured_input(self, body: Dict[str, Any], parsed: Dict[str, str]) -> Dict[str, str]:
        ...
        if year:
            merged['year'] = year
        cloud_provider = self._clean_text(body.get('cloud_provider') or body.get('provider')).lower()
        if cloud_provider in {'115', 'quark'}:
            merged['cloud_provider'] = cloud_provider
        ...
        if result_filter in {'latest_episode', 'latest', 'latest_episodes'}:
            ...
            episode_value = self._safe_int(result_filter.split(':', 1)[1], 0)
            if episode_value > 0:
                merged['result_filter'] = f'episode:{episode_value}'
    ...
    def _parse_assistant_text(text: str) -> Dict[str, str]:
        ...
        plan_match = re.search('\\bplan-[a-zA-Z0-9]+\\b', raw)
        options: Dict[str, str] = {'text': raw, 'url': share_url, 'access_code': '', 'path': '', 'mode': mode, 'keyword': query or remain, 'result_filter': '', 'source_order_text': '', 'cloud_provider': '', 'type': '', 'year': '', 'action': '', 'client_type': '', 'status': '', 'hash': '', 'plan_id': plan_match.group(0) if plan_match else '', 'decision_intent': '', 'sample_index': '', 'remove_if_resolved': 'true', 'streaming_intent': '', 'streaming_month': '', 'streaming_window': '', 'streaming_media_type': ''}
        ...
        cloud_search_match = re.match('^\\s*(?:云盘搜索|云盘搜)\\s*(.*)$', raw)
        if cloud_search_match:
            options['action'] = 'cloud_search_removed'
            options['mode'] = ''
            options['keyword'] = AgentResourceOfficer._clean_text(cloud_search_match.group(1))
        transfer_provider_prefixes = [('夸克转存资源', 'quark'), ('夸克转存', 'quark'), ('115转存资源', '115'), ('115转存', '115')]
        for prefix, provider in transfer_provider_prefixes:
            if raw == prefix:
                options['action'] = 'cloud_transfer_removed'
                options['mode'] = ''
                options['keyword'] = ''
            ...
            if raw.startswith(prefix + ' '):
                ...
                options['cloud_provider'] = provider
                break
        replay_match = re.match('^\\s*(重放样本|重识别样本|重跑样本)\\s*(\\d+)?(?:\\s+(.*))?$', raw)
        if replay_match:
        ...
        _today = _date.today()
        streaming_match = re.match('^\\s*流媒体推荐\\s*(.*?)$', raw, re.IGNORECASE)
        if streaming_match:
            _streaming_text = AgentResourceOfficer._clean_text(streaming_match.group(1))
            _nl_compact = re.sub('\\s+', '', _streaming_text)
            ...
            month_match = re.search('(\\d{1,2})\\s*月', _streaming_text)
            if month_match:
            ...
            elif re.search('本月|这个月', _streaming_text):
                options['streaming_month'] = f'{_today.year}-{_today.month:02d}'
                options['streaming_window'] = ''
            elif re.search('上月|上个月', _streaming_text):
                ...
                options['streaming_window'] = ''
            elif re.search('近期|最近|近来', _streaming_text):
                options['streaming_month'] = ''
                options['streaming_window'] = '90'
            else:
                options['streaming_month'] = f'{_today.year}-{_today.month:02d}'
        ...
        if options.get('plan_id') and compact.startswith(('执行plan-', '确认plan-', 'executeplan-')):
            ...
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'帮助', '使用帮助', '命令帮助', 'help', 'agenthelp', 'arohelp', '插件帮助'}:
            options['action'] = 'assistant_help'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'执行计划', '执行最新计划', '确认计划', '确认执行计划', '执行plan', '执行最新plan', 'executeplan', 'executelatestplan'}:
            options['action'] = 'execute_plan'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'计划列表', '查看计划', '待执行计划', '保存计划', 'plans', 'listplans'}:
            options['action'] = 'plans_list'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'偏好', '片源偏好', '查看偏好', '偏好设置', '智能体偏好', 'preferences', 'getpreferences'}:
            options['action'] = 'preferences_get'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'重置偏好', '清除偏好', '重设偏好', '恢复默认偏好', 'resetpreferences'}:
            options['action'] = 'preferences_reset'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'评分策略', '评分规则', '自动化规则', 'scoringpolicy', 'scoringrules'}:
            options['action'] = 'scoring_policy'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'取消计划', '清理计划', '删除计划', 'cancelplan', 'clearplan', 'deleteplan'}:
            options['action'] = 'plans_clear'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'115登录', '115扫码', '扫码115', '登录115', '115login', '115qrcode', 'p115login', 'p115qrcode'}:
            options['action'] = 'p115_qrcode_start'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'检查115登录', '检查115扫码', '检查扫码', '115check', 'check115login', 'p115check'}:
            options['action'] = 'p115_qrcode_check'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'115登录状态', '115状态', '查看115状态', '115健康', '115status', 'p115status'}:
            options['action'] = 'p115_status'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'115帮助', '115命令', '115使用', '115help', 'p115help'}:
            options['action'] = 'p115_help'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'115任务', '待处理115', '待继续115', '115pending', 'p115pending'}:
            options['action'] = 'p115_pending'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'清空夸克默认目录', '清空夸克默认转存目录', '清空夸克转存目录', '清空夸克目录'}:
            options['action'] = 'quark_clear_default_dir'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'清空115默认目录', '清空115默认转存目录', '清空115转存目录', '清空115目录', '清空115网盘转存目录', '清空115网盘目录'}:
            options['action'] = 'p115_clear_default_dir'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'继续115任务', '重试115任务', '继续115转存', '重试115转存', 'continue115', 'resume115'}:
            options['action'] = 'p115_resume'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'取消115任务', '取消115转存', '清除115任务', 'cancel115', 'clear115'}:
            options['action'] = 'p115_cancel'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'资源决策', '智能决策', 'decision', 'smartdecision'}:
            options['mode'] = 'smart_decision'
            options['keyword'] = ''
        elif compact in {'后续', '执行后追踪', '继续追踪', 'followup', 'postexecute'}:
            options['action'] = 'execution_followup'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'跟进', '继续跟进', '查看进展', '进展', 'smartfollowup'}:
            options['action'] = 'smart_followup'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'继续决策', '继续资源决策', 'decisioncontinue'}:
            options['action'] = 'smart_decision_adjust'
            options['mode'] = ''
            options['keyword'] = ''
            options['decision_adjust'] = 'decision_continue'
        elif compact in {'换盘搜', '切换盘搜', '走盘搜', '用盘搜', 'decisionpansou'}:
            options['action'] = 'smart_decision_adjust'
            options['mode'] = ''
            options['keyword'] = ''
            options['decision_adjust'] = 'decision_pansou'
        elif compact in {'换pt', '换原生', '换mp', '切换pt', '切换原生', '切换mp', 'decisionmppt'}:
            options['action'] = 'smart_decision_adjust'
            options['mode'] = ''
            options['keyword'] = ''
            options['decision_adjust'] = 'decision_mp_pt'
        elif compact in {'保守一点', '更保守', '保守模式', 'decisionconservative'}:
            options['action'] = 'smart_decision_adjust'
            options['mode'] = ''
            options['keyword'] = ''
            options['decision_adjust'] = 'decision_conservative'
        elif compact in {'激进一点', '更激进', '激进模式', 'decisionaggressive'}:
            options['action'] = 'smart_decision_adjust'
            options['mode'] = ''
            options['keyword'] = ''
            options['decision_adjust'] = 'decision_aggressive'
        elif compact in {'下载任务', 'downloadtasks'}:
            options['action'] = 'mp_download_tasks'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'下载最佳', '下载推荐', '下载最好', '下载最佳片源', '下载推荐片源', 'downloadbest'}:
            options['action'] = 'mp_download_best'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'下载历史', '下载记录', '记录', '历史下载', 'downloadhistory'}:
            options['action'] = 'mp_download_history'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'最近下载', 'recentdownloads'}:
            options['action'] = 'mp_recent_activity'
            options['mode'] = ''
            options['keyword'] = ''
            options['download_only'] = 'true'
        elif compact in {'最近', '最近动态', '动态', 'recentactivity'}:
            options['action'] = 'mp_recent_activity'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'追踪', '资源追踪', '下载追踪', '媒体状态', '落库状态', '状态', '进度', 'lifecyclestatus'}:
            options['action'] = 'mp_lifecycle_status'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'入库状态', '本地入库', '入库', 'ingeststatus'}:
            options['action'] = 'mp_ingest_status'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'识别', '媒体识别', '媒体详情', 'mp识别', 'mp媒体识别', 'mpmediadetail'}:
            options['action'] = 'mp_media_detail'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'下载器状态', 'downloaders'}:
            options['action'] = 'mp_downloaders'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'站点状态', 'sites'}:
            options['action'] = 'mp_sites'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'订阅列表', 'subscribes'}:
            options['action'] = 'mp_subscribes'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'入库历史', '整理历史', '转移历史', '入库记录', '最近整理', 'transferhistory'}:
            options['action'] = 'mp_transfer_history'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'最近入库', 'recentingest'}:
            options['action'] = 'mp_recent_activity'
            options['mode'] = ''
            options['keyword'] = ''
            options['transfer_only'] = 'true'
        elif compact in {'入库失败', '整理失败', '失败入库', '失败整理', 'transferfailed'}:
            options['action'] = 'mp_ingest_failures'
            options['mode'] = ''
            options['keyword'] = ''
            options['status'] = 'failed'
        elif compact in {'入库成功', '整理成功', '成功入库', '成功整理', 'transfersuccess'}:
            options['action'] = 'mp_transfer_history'
            options['mode'] = ''
            options['keyword'] = ''
            options['status'] = 'success'
        elif compact in {'本地诊断', '诊断', '失败原因', 'localdiagnose'}:
            options['action'] = 'mp_local_diagnose'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'失败样本', '识别样本', 'ai失败样本', 'aifailedsamples'}:
            options['action'] = 'ai_failed_samples'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'工作清单', '识别工作清单', '样本清单', 'sampleworklist'}:
            options['action'] = 'ai_sample_worklist'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'样本洞察', '识别洞察', '失败洞察', 'sampleinsights'}:
            options['action'] = 'ai_sample_insights'
            options['mode'] = ''
            options['keyword'] = ''
        elif compact in {'重放样本', '重识别样本', '重跑样本', 'aireplay', 'replaysample'}:
            options['action'] = 'ai_replay_failed_sample'
            options['mode'] = ''
            options['keyword'] = ''
        else:
            for prefix, action in [('执行计划', 'execute_plan'), ('执行', 'execute_plan'), ('确认计划', 'execute_plan'), ('确认', 'execute_plan'), ('查看计划', 'plans_list'), ('计划列表', 'plans_list'), ('取消计划', 'plans_clear'), ('清理计划', 'plans_clear'), ('删除计划', 'plans_clear'), ('保存偏好', 'preferences_save'), ('设置偏好', 'preferences_save'), ('更新偏好', 'preferences_save'), ('偏好设置', 'preferences_save'), ('偏好', 'preferences_save'), ('查看偏好', 'preferences_get'), ('片源偏好', 'preferences_get'), ('重置偏好', 'preferences_reset'), ('清除偏好', 'preferences_reset'), ('评分策略', 'scoring_policy'), ('评分规则', 'scoring_policy'), ('自动化规则', 'scoring_policy')]:
                if raw.startswith(prefix + ' ') or raw.startswith(prefix + '：') or raw.startswith(prefix + ':'):
                    ...
                    if action in {'plans_list', 'preferences_get', 'preferences_reset'} or options.get('plan_id') or remain_text:
                        ...
                        if remain_text and (not options.get('plan_id')):
                            match = re.search('\\bplan-[a-zA-Z0-9]+\\b', remain_text)
                            if match:
                                options['plan_id'] = match.group(0)
                        break
            for prefix, control in [('暂停下载', 'pause'), ('停止下载', 'pause'), ('恢复下载', 'resume'), ('继续下载', 'resume'), ('开始下载', 'resume'), ('删除下载', 'delete'), ('移除下载', 'delete')]:
            ...
            if not options.get('action'):
                for prefix in ['下载状态', '正在下载', '下载列表', '查看下载', '下载进度', '站点', '站点列表', 'PT站点', 'pt站点', 'pt站点状态', '下载器', '下载器列表', '查看下载器']:
                    prefix_match = AgentResourceOfficer._match_command_prefix(raw, [prefix])
                    if prefix_match:
                        options['action'] = 'command_alias_removed'
                        options['mode'] = ''
                        options['keyword'] = prefix_match[1]
                        options['type'] = prefix
                        break
            if not options.get('action'):
                prefix_match = AgentResourceOfficer._match_command_prefix(raw, ['下载任务'])
                if prefix_match:
                    options['action'] = 'mp_download_tasks'
                    options['mode'] = ''
                    options['keyword'] = prefix_match[1]
            if not options.get('action') and (not options.get('mode')):
                prefix_match = AgentResourceOfficer._match_command_prefix(raw, ['资源决策', '智能决策', '热门推荐', '推荐', '智能发现', '热门发现'])
                if prefix_match:
                    options['action'] = 'advanced_command_removed'
                    options['mode'] = ''
                    options['keyword'] = prefix_match[1]
            if not options.get('action') and (not options.get('mode')):
            ...
            if not options.get('action'):
                ...
                if prefix_match:
                    ...
                    options['keyword'] = prefix_match[1]
            if not options.get('action'):
                prefix_match = AgentResourceOfficer._match_command_prefix(raw, ['站点状态'])
                if prefix_match:
                    options['action'] = 'mp_sites'
                    options['mode'] = ''
                    options['keyword'] = prefix_match[1]
            if not options.get('action'):
                for prefix in ['搜索订阅', '移除订阅', '订阅状态', '查看订阅', 'MP订阅', 'mp订阅']:
                    prefix_match = AgentResourceOfficer._match_command_prefix(raw, [prefix])
                    if prefix_match:
                        options['action'] = 'command_alias_removed'
                        options['mode'] = ''
                        ...
                        break
            if not options.get('action'):
                for prefix, control in [('刷新订阅', 'search'), ('暂停订阅', 'pause'), ('恢复订阅', 'resume'), ('删除订阅', 'delete'), ('移除订阅', 'delete')]:
                    prefix_match = AgentResourceOfficer._match_command_prefix(raw, [prefix])
                    if prefix_match:
                        target_text = prefix_match[1]
                        if target_text:
                        ...
                        break
            if not options.get('action'):
                prefix_match = AgentResourceOfficer._match_command_prefix(raw, ['订阅列表'])
                if prefix_match:
                    options['action'] = 'mp_subscribes'
                    options['mode'] = ''
                    options['keyword'] = prefix_match[1]
            if not options.get('action'):
                for prefix, status_name in [('入库失败', 'failed'), ('整理失败', 'failed'), ('失败入库', 'failed'), ('失败整理', 'failed'), ('入库成功', 'success'), ('整理成功', 'success'), ('成功入库', 'success'), ('成功整理', 'success'), ('入库历史', 'all'), ('入库记录', 'all'), ('整理历史', 'all'), ('转移历史', 'all'), ('最近入库', 'all'), ('最近整理', 'all')]:
                    prefix_match = AgentResourceOfficer._match_command_prefix(raw, [prefix])
                    if prefix_match:
            ...
            if not options.get('action'):
                ...
                if prefix_match:
                    ...
                    if '保留样本' in remain_text or '不移除' in remain_text:
                        options['remove_if_resolved'] = 'false'
            if not options.get('action'):
                for prefix in ['转存资源', '下载资源', '订阅并搜索', '订阅搜索', '订阅媒体']:
                    prefix_match = AgentResourceOfficer._match_command_prefix(raw, [prefix])
                    if prefix_match:
                        options['action'] = 'command_alias_removed'
                        options['mode'] = ''
                        ...
                        break
            if not options.get('action'):
                for prefix, action in [('转存', 'cloud_transfer'), ('下载', 'mp_download'), ('订阅', 'mp_subscribe')]:
                    if raw == prefix:
                        options['action'] = action
                        options['mode'] = ''
                        options['keyword'] = ''
                    ...
                    if raw.startswith(prefix + ' '):
                        ...
                        if action == 'mp_download':
                            download_match = re.fullmatch('[#＃]?\\s*(\\d+)', remain_text)
                            if download_match:
                                options['action'] = action
                                options['mode'] = ''
                    ...
                    if raw.startswith(prefix):
                        ...
                        options['keyword'] = remain_text
                        break
        if options.get('mode') in {'search', 'mp', 'mp_download_title'} and options.get('keyword'):
            filtered_keyword, extracted_filter = AgentResourceOfficer._extract_mp_result_filter_intent(options.get('keyword') or '')
            if extracted_filter:
                options['keyword'] = filtered_keyword.strip()
                options['result_filter'] = extracted_filter
        ...
        if share_url and (AgentResourceOfficer._is_quark_url(share_url) or AgentResourceOfficer._is_115_url(share_url)):
            if options.get('action') in {'cloud_transfer_removed', 'command_alias_removed'}:
                ...
                options['type'] = ''
        if not options.get('action') and (not options.get('mode')):
            check_match = re.match('^\\s*检查\\s*(.+)$', raw)
            if check_match:
                remain = AgentResourceOfficer._clean_text(check_match.group(1))
                if remain and (not re.match('^[0-9a-zA-Z]', remain)):
                    options['mode'] = 'smart'
    ...
    @staticmethod
    def _extract_size_text(text: str) -> str:
        match = re.search(r"(\d+(?:\.\d+)?)\s*(gb|g|mb|m)\b", str(text or ""), flags=re.IGNORECASE)
        if not match:
            return ""
        value = match.group(1)
    ...
    def _pansou_item_brief_summary(self, item: Dict[str, Any]) -> str:
        text = self._score_text_blob(item)
        parts: List[str] = []
        if "2160" in text or "4k" in text or "uhd" in text:
            parts.append("4K")
        ...
        if self._score_has_any(text, ["dolby vision", "dovi", "dv", "杜比视界"]):
            ...
            parts.append("HDR")
        if self._score_has_any(text, ["中字", "中文字幕", "简中", "繁中", "双语", "官中", "内封简繁"]):
            parts.append("中字")
        elif self._score_has_any(text, ["无中字", "无字幕"]):
            parts.append("无中字")
        progress = self._extract_series_progress(text)
        if progress.get("episode_count", 0) >= max(3, progress.get("max_episode", 0)):
            parts.append(f"E01-E{progress['max_episode']:02d}")
        elif progress.get("max_episode", 0) > 0:
            parts.append(f"更新到E{progress['max_episode']:02d}")
        size_text = self._extract_size_text(text) or self._extract_size_text(self._list_text(item.get("share_size") or item.get("size")))
        if size_text:
            parts.append(size_text)
        return " | ".join(parts[:5])

    @staticmethod
    def _format_markdown_link(title: Any, url: Any) -> str:
        clean_title = str(title or "").strip()
        ...
        return f"[{safe_title}]({clean_url})"

    @staticmethod
    def _parse_simple_cjk_number(text: str) -> Optional[int]:
        raw = str(text or "").strip()
        if not raw:
            return None
        if raw.isdigit():
        ...
        if "十" in raw:
            ...
            left_value = 1 if left == "" else mapping.get(left)
            if left_value is None:
                return None
            right_value = 0 if right == "" else mapping.get(right)
    ...
    @classmethod
    def _normalize_simple_cjk_numbers(cls, text: str) -> str:
        raw = str(text or "")
        if not raw:
            return ""
        pattern = re.compile(r"[零〇一二两三四五六七八九十]{1,3}")
        def repl(match: re.Match[str]) -> str:
            value = cls._parse_simple_cjk_number(match.group(0))
            return str(value) if value is not None else match.group(0)
        return pattern.sub(repl, raw)

    @classmethod
    def _build_keyword_variants(cls, keyword: str) -> List[str]:
        raw = cls._clean_text(keyword)
        if not raw:
            return []
        variants: List[str] = []
        ...
        add(raw)
        trimmed = re.split(r"[：:（(【\[\-_/|]", raw, 1)[0].strip()
        add(trimmed)
        if "的" in raw:
            add(raw.split("的", 1)[0].strip())
        base_candidates = list(variants)
        for base in base_candidates:
            numeric = cls._normalize_simple_cjk_numbers(base)
            add(numeric)
            add(numeric.replace("个月", "ヶ月").replace("個月", "ヶ月"))
            add(numeric.replace("个月", "个月").replace("個月", "个月"))
            add(base.replace("三个月", "3个月").replace("三個月", "3个月"))
            add(base.replace("三个月", "3ヶ月").replace("三個月", "3ヶ月"))
            add(re.sub(r"\s+", "", base))
        return variants

    def _pansou_payload_item_count(self, payload: Dict[str, Any]) -> int:
        data = payload.get("data") or {}
        merged = data.get("merged_by_type") or {}
        total = 0
        for channel in ("115", "quark"):
            items = merged.get(channel) or []
            if isinstance(items, list):
                total += len(items)
        return total

    def _call_pansou_search_with_variants(self, keyword: str) -> Tuple[bool, Dict[str, Any], str, str]:
        variants = self._build_keyword_variants(keyword)
        last_error = ""
        last_payload: Dict[str, Any] = {}
        for variant in variants:
            ok, payload, message = self._call_pansou_search(variant)
            if ok and self._pansou_payload_item_count(payload) > 0:
                return True, payload, message, variant
            last_error = message
            last_payload = payload if isinstance(payload, dict) else {}
        return False, last_payload, last_error or "盘搜暂无结果", self._clean_text(keyword)

    def _assistant_pansou_entry_summary(self, items: List[Dict[str, Any]]) -> Dict[str, Any]:
        best = self._best_scored_source_item(items)
        index = self._safe_int((best or {}).get("index") or (best or {}).get("pick_index"), 0)
        if index <= 0:
            return {
                "stage": "pansou_result",
                "label": "先看条目再选编号",
        ...
        return {
            "stage": "pansou_result",
            "label": "盘搜列表已返回",
            "decision_hint": "默认直接回编号即可转存；想先确认可回复“选择 编号 详情”。只有明确要生成计划时才发“计划选择 编号”。",
            "preferred_command": str(index),
            "fallback_command": f"选择 {index} 详情",
            "compact_commands": [str(index), f"选择 {index} 详情"],
            "preferred_requires_confirmation": True,
            "fallback_requires_confirmation": False,
            "can_auto_run_preferred": False,
            "recommended_agent_behavior": "show_only",
    ...
    def _mp_episode_plan_priority(self, item: Dict[str, Any], target_episode: int) -> Tuple[int, int, int]:
        target = self._safe_int(target_episode, 0)
        if target <= 0 or not isinstance(item, dict):
            return (9, 9, 9)
        progress = item.get("_series_progress") if isinstance(item.get("_series_progress"), dict) else self._extract_series_progress(self._score_text_blob(item))
        max_episode = self._safe_int(progress.get("max_episode"), 0)
        episode_count = self._safe_int(progress.get("episode_count"), 0)
        if max_episode <= 0:
            return (9, 9, 9)
        count = max(1, episode_count or 1)
        start_episode = max(1, max_episode - count + 1)
        if not (start_episode <= target <= max_episode):
            return (9, 9, 9)
        exact_single = count == 1 and max_episode == target
        at_edge = target == start_episode or target == max_episode
        if exact_single:
            return (0, 0, count)
        if count <= 2 and at_edge:
            return (1, 0, count)
        if count <= 3:
            return (2 if at_edge else 3, 0 if at_edge else 1, count)
        if count <= 6:
            return (4 if at_edge else 5, 0 if at_edge else 1, count)
        return (6 if at_edge else 7, 0 if at_edge else 1, count)

    def _sort_mp_episode_filtered_items(self, items: List[Dict[str, Any]], target_episode: int) -> List[Dict[str, Any]]:
        ranked = [
            dict(item or {})
            for item in (items or [])
            if isinstance(item, dict)
        ...
        ranked.sort(
            key=lambda item: (
                self._mp_episode_plan_priority(item, target),
                -self._safe_int((item.get("score") or {}).get("score"), 0),
                -self._score_quality_rank(item),
                -self._safe_int(((item.get("torrent_info") or {}).get("seeders")), 0),
                self._safe_int(item.get("index"), 0),
    ...
    @staticmethod
    def _format_mp_candidate_lines(candidates: List[Dict[str, Any]], page: int = 1, page_size: int = 10) -> str:
        if not candidates:
            return "MP 搜索候选：0 个"
        safe_page_size = max(1, int(page_size or 10))
        total_pages = max(1, (len(candidates) + safe_page_size - 1) // safe_page_size)
    ...
    def _resource_points_text(item: Dict[str, Any]) -> str:
        ...
        if points in (None, "", "未知"):
            return "免费" if AgentResourceOfficer._resource_has_free_marker(item) else "未标积分"
        return f"{points}分"
    ...
    async def tool_route_share(self, share_url: str, access_code: str = "", target_path: str = "") -> str:
        ...
        if self._is_quark_url(share_url):
            ...
            if not ok:
                return self._format_quark_transfer_failure(
                    detail=message,
                    target_path=self._clean_text(target_path) or self._quark_default_path,
                )
    ...
    async def tool_assistant_readiness(self, compact: bool = True) -> str:
        ...
        data = self._assistant_readiness_public_data()
        if compact:
            compact_data = self._assistant_readiness_compact_data(data)
            services = compact_data.get("services") or {}
            return (
                f"就绪：{'是' if compact_data.get('can_start') else '否'}；"
                f"115：{'可用' if services.get('p115_ready') else '不可用'}；"
                f"夸克：{'已配' if services.get('quark_configured') else '未配'}；"
                f"待计划：{compact_data.get('saved_plans_pending') or 0}"
            )
...
<body>
  ...
  <script>
    ...
    async def api_assistant_cookie_update(self, request: Request):
        body: Dict[str, Any] = {}
        try:
            body = await request.json()
        except Exception:
        ...
        if not self._enabled:
            return {'success': False, 'message': '插件未启用'}
        kind = self._clean_text(body.get('kind') or body.get('provider') or body.get('target') or body.get('name')).lower()
        cookie = self._clean_text(body.get('cookie') or body.get('cookie_header'))
        if not cookie:
            return {'success': False, 'message': 'Cookie 不能为空'}
        aliases = {'quark': 'quark', '夸克': 'quark'}
        target = aliases.get(kind, '')
        if target == 'quark':
            self._quark_cookie = cookie
            config = self._build_config({'quark_cookie': cookie})
            self.update_config(config)
            self.init_plugin(config)
            return {'success': True, 'message': '夸克 Cookie 已写回 Agent影视助手。', 'data': {'kind': 'quark', 'field': 'quark_cookie', 'length': len(cookie), 'write_effect': 'config'}}
    ...
    async def api_assistant_route(self, request: Request):
        ...
        if not self._enabled:
            return {'success': False, 'message': '插件未启用'}
        session, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or body.get('chat_id') or body.get('user_id') or body.get('conversation_id') or 'default', session_id=body.get('session_id'))
        ...
        if pending_plan_group and (numeric_confirmation > 0 or self._is_pending_plan_confirmation_text(text)):
            pending_multi_plan = self._find_pending_multi_plan(session=session, session_id=cache_key, rank=numeric_confirmation if numeric_confirmation > 0 else 1, group_id=pending_plan_group)
        pending_plan = pending_multi_plan or self._find_workflow_plan(session=session, session_id=cache_key, executed=False)
        if pending_plan and (self._is_pending_plan_confirmation_text(text) or self._is_pending_plan_numeric_confirmation(text, pending_plan)) and (not self._clean_text(body.get('plan_id'))):
            if isinstance(state.get('recommend_handoff'), dict) and state.get('recommend_handoff'):
                return finish(await self._assistant_confirm_recommend_handoff(request, session=session, cache_key=cache_key, state=state, compact=compact, target_path=self._resolve_pan_path_value(self._clean_text(body.get('path') or body.get('target_path')))))
            return finish(await self.api_assistant_plan_execute(_JsonRequestShim(request, {'session': session, 'session_id': cache_key, 'plan_id': self._clean_text(pending_plan.get('plan_id')) if pending_multi_plan else '', 'prefer_unexecuted': True, 'stop_on_error': self._parse_bool_value(body.get('stop_on_error'), True), 'include_raw_results': self._parse_bool_value(body.get('include_raw_results'), False), 'compact': compact, 'apikey': self._extract_apikey(request, body)})))
        recommend_direct_intent = self._normalize_mp_recommend_direct_intent(text)
        if recommend_direct_intent:
            source_name, inferred_media_type = self._normalize_mp_recommend_request(recommend_direct_intent.get('keyword') or 'tmdb_trending')
            recommend_result = await self._assistant_mp_recommendations(source=source_name, media_type=self._clean_text(body.get('media_type') or body.get('type') or inferred_media_type or 'all'), limit=self._safe_int(body.get('limit'), 20), session=session, cache_key=cache_key)
            if not recommend_result.get('success'):
                return finish(recommend_result)
            return finish(await self.api_assistant_pick(_JsonRequestShim(request, {'session': session, 'session_id': cache_key, 'index': recommend_direct_intent.get('index') or 1, 'action': recommend_direct_intent.get('action'), 'mode': recommend_direct_intent.get('mode'), 'path': self._resolve_pan_path_value(self._clean_text(body.get('path') or body.get('target_path'))), 'compact': compact, 'apikey': self._extract_apikey(request, body)})))
        recommend_short_direct = self._normalize_mp_recommend_short_action(text, state=state)
        if recommend_short_direct:
            return finish(await self.api_assistant_pick(_JsonRequestShim(request, {'session': session, 'session_id': cache_key, 'index': recommend_short_direct.get('index'), 'action': recommend_short_direct.get('action'), 'mode': recommend_short_direct.get('mode'), 'path': self._resolve_pan_path_value(self._clean_text(body.get('path') or body.get('target_path'))), 'compact': compact, 'apikey': self._extract_apikey(request, body)})))
        recommend_source_compound = self._normalize_recommend_source_compound_action(text, state=state)
        if recommend_source_compound:
            return finish(await self._assistant_run_recommend_source_compound(request, session=session, cache_key=cache_key, state=state, mode=self._clean_text(recommend_source_compound.get('mode')), followup_action=self._clean_text(recommend_source_compound.get('followup_action')), compact=compact, target_path=self._resolve_pan_path_value(self._clean_text(body.get('path') or body.get('target_path')))))
        ...
        if preparsed_action not in {'ai_replay_failed_sample'} and recommend_short:
            pick_result = await self.api_assistant_pick(_JsonRequestShim(request, {'session': session, 'index': recommend_short.get('index'), 'action': recommend_short.get('action'), 'mode': recommend_short.get('mode'), 'path': target_path, 'compact': compact, 'apikey': self._extract_apikey(request, body)}))
            return pick_result
        ...
        if not has_recommend_followup and preparsed_action not in {'ai_replay_failed_sample'} and (pick_index > 0 or pick_action):
            pick_result = await self.api_assistant_pick(_JsonRequestShim(request, {'session': session, 'index': pick_index, 'action': pick_action, 'mode': pick_mode, 'path': target_path or pick_path, 'compact': compact, 'apikey': self._extract_apikey(request, body)}))
            return pick_result
        if not has_recommend_followup and preparsed_action not in {'ai_replay_failed_sample'}:
            ...
            if route_action:
                pick_result = await self.api_assistant_pick(_JsonRequestShim(request, {'session': session, 'index': 0, 'action': route_action, 'path': target_path, 'compact': compact, 'apikey': self._extract_apikey(request, body)}))
                return pick_result
        if not text and (not any((parsed.get(key) for key in ['mode', 'keyword', 'url', 'action']))):
            ...
            return finish({'success': True, 'message': summary, 'data': self._assistant_response_data(session=session, data={'action': 'assistant_help', 'ok': True, 'status_summary': summary})})
        ...
        if assistant_action == 'assistant_help':
            summary = self._format_assistant_help_text(session=session)
            return finish({'success': True, 'message': summary, 'data': self._assistant_response_data(session=session, data={'action': 'assistant_help', 'ok': True, 'status_summary': summary})})
        if assistant_action == 'cloud_search_removed':
            lines = ['“云盘搜索”命令已取消。', '现在请按来源明确搜索，结果更可控：']
            if keyword:
                lines.append(f'- 盘搜搜索 {keyword}')
                lines.append(f'- PT搜索 {keyword}')
            else:
                lines.append('- 盘搜搜索 片名')
                lines.append('- PT搜索 片名')
            return finish({'success': False, 'message': '\n'.join(lines), 'data': self._assistant_response_data(session=session, data={'action': 'cloud_search_removed', 'ok': False, 'keyword': keyword, 'recommended_commands': [f'盘搜搜索 {keyword}' if keyword else '盘搜搜索 片名', f'PT搜索 {keyword}' if keyword else 'PT搜索 片名']})})
        if assistant_action == 'command_alias_removed':
            removed_command = self._clean_text(parsed.get('type'))
            lines = [f'“{removed_command}”这个旧别名已取消。', '请改用更直接的主线命令：']
            if removed_command == '下载资源':
                lines.append(f'- 下载 {keyword}' if keyword else '- 下载 片名')
                lines.append('- 或者先 MP搜索 / PT搜索，再回编号')
            elif removed_command in {'转存资源'}:
                lines.append(f'- 盘搜搜索 {keyword}' if keyword else '- 盘搜搜索 片名')
                lines.append('- 然后从结果里直接回编号继续')
            elif removed_command in {'订阅并搜索', '订阅搜索'}:
                lines.append(f'- 订阅 {keyword}' if keyword else '- 订阅 片名')
                lines.append('- 需要立刻看资源时，再手动发搜索命令')
            elif removed_command == '订阅媒体':
                lines.append(f'- 订阅 {keyword}' if keyword else '- 订阅 片名')
            elif removed_command == '搜索订阅':
                lines.append(f'- 刷新订阅 {keyword}' if keyword else '- 刷新订阅 1')
            elif removed_command == '移除订阅':
                lines.append(f'- 删除订阅 {keyword}' if keyword else '- 删除订阅 1')
            elif removed_command in {'订阅状态', '查看订阅', 'MP订阅', 'mp订阅'}:
                lines.append('- 订阅列表')
            elif removed_command in {'下载状态', '正在下载', '下载列表', '查看下载', '下载进度'}:
                lines.append('- 下载任务')
            elif removed_command in {'站点', '站点列表', 'PT站点', 'pt站点', 'pt站点状态'}:
                lines.append('- 站点状态')
            elif removed_command in {'下载器', '下载器列表', '查看下载器'}:
                lines.append('- 下载器状态')
            return finish({'success': False, 'message': '\n'.join(lines), 'data': self._assistant_response_data(session=session, data={'action': 'command_alias_removed', 'ok': False, 'keyword': keyword, 'removed_command': removed_command})})
        if assistant_action == 'cloud_transfer_removed':
            provider = self._clean_text(parsed.get('cloud_provider')).lower()
            command_name = '转存'
            if provider == 'quark':
                command_name = '夸克转存'
            elif provider == '115':
                command_name = '115转存'
            lines = [f'“{command_name} 片名”命令已取消。', '现在请先按来源搜索，再从结果里直接回编号继续处理：']
            if keyword:
                lines.append(f'- 盘搜搜索 {keyword}')
                lines.append(f'- PT搜索 {keyword}')
            else:
                lines.append('- 盘搜搜索 片名')
                lines.append('- PT搜索 片名')
            lines.append('看到资源列表后，直接回编号即可继续转存或下载。')
            return finish({'success': False, 'message': '\n'.join(lines), 'data': self._assistant_response_data(session=session, data={'action': 'cloud_transfer_removed', 'ok': False, 'keyword': keyword, 'cloud_provider': provider})})
        if assistant_action == 'advanced_command_removed':
            lines = ['这组高级命令已下线。', '现在请直接用主线命令，结果更稳，也更不容易被外部智能体二次改写：']
            if keyword:
                lines.append(f'- 搜索 {keyword}')
                lines.append(f'- MP搜索 {keyword}')
                lines.append(f'- PT搜索 {keyword}')
                lines.append(f'- 盘搜搜索 {keyword}')
                ...
                lines.append('- 下载 片名')
                lines.append('- 搜索 片名')
            return finish({'success': False, 'message': '\n'.join(lines), 'data': self._assistant_response_data(session=session, data={'action': 'advanced_command_removed', 'ok': False, 'keyword': keyword})})
        ...
        if assistant_action == 'plans_list':
            ...
            plans_data = self._assistant_plans_public_data(session=session, session_id=cache_key, executed=executed, include_actions=include_actions, limit=limit)
            return finish({'success': True, 'message': self._format_assistant_plans_text(session=session, session_id=cache_key, executed=executed, include_actions=include_actions, limit=limit), 'data': self._assistant_response_data(session=session, data={'action': 'plans_list', 'ok': True})})
        if assistant_action == 'plans_clear':
            plan_id = self._clean_text(parsed.get('plan_id'))
            if not plan_id:
                return finish({'success': False, 'message': '取消或清理计划需要指定 plan_id，例如：取消计划 plan-xxxx。', 'data': self._assistant_response_data(session=session, data={'action': 'plans_clear', 'ok': False, 'error_code': 'missing_plan_id'})})
            clear_result = self._clear_workflow_plans(plan_id=plan_id, limit=1)
            return finish({'success': bool(clear_result.get('ok')), 'message': str(clear_result.get('message') or '计划清理完成'), 'data': self._assistant_response_data(session=session, data={'action': 'plans_clear', 'ok': bool(clear_result.get('ok'))})})
        if assistant_action in {'preferences_get', 'preferences_save', 'preferences_reset'}:
            if assistant_action == 'preferences_save':
                ...
                if not preferences:
                    return finish({'success': False, 'data': self._assistant_response_data(session=session, data={'action': 'preferences_save', 'ok': False, 'error_code': 'missing_preferences'})})
                payload = {'session': session, 'user_key': self._clean_text(body.get('user_key')), 'preferences': preferences, 'compact': compact, 'apikey': self._extract_apikey(request, body)}
            ...
            if assistant_action == 'preferences_reset':
                return finish(await self.api_assistant_preferences(_JsonRequestShim(request, {'session': session, 'user_key': self._clean_text(body.get('user_key')), 'compact': compact, 'apikey': self._extract_apikey(request, body)}, method='DELETE')))
            return finish(await self.api_assistant_preferences(_JsonRequestShim(request, {'session': session, 'user_key': self._clean_text(body.get('user_key')), 'compact': compact, 'apikey': self._extract_apikey(request, body)}, method='GET')))
        if assistant_action == 'scoring_policy':
            return finish({'success': True, 'message': '评分策略已返回。云盘与 PT 使用不同规则；自动化决策以硬风险和 score_summary 为准。', 'data': self._assistant_response_data(session=session, data={'action': 'scoring_policy', 'ok': True, 'scoring_policy': self._assistant_scoring_policy_public_data()})})
        if assistant_action == 'mp_media_detail':
            if not keyword:
                return finish({'success': False, 'message': '媒体识别失败：缺少片名。用法：识别 蜘蛛侠', 'data': self._assistant_response_data(session=session, data={'action': 'mp_media_detail', 'ok': False, 'error_code': 'missing_keyword'})})
            return finish(await self._assistant_mp_media_detail(keyword=keyword, session=session, cache_key=cache_key, media_type=self._clean_text(body.get('media_type') or body.get('type') or parsed.get('type') or 'auto'), year=self._clean_text(body.get('year') or parsed.get('year'))))
        ...
        if assistant_action == 'mp_sites':
            return finish(await self._assistant_mp_sites(session=session, cache_key=cache_key, status=self._clean_text(body.get('status') or parsed.get('status') or 'active'), name=keyword, limit=self._safe_int(body.get('limit'), 30)))
        if assistant_action == 'mp_subscribes':
            return finish(await self._assistant_mp_subscribes(session=session, cache_key=cache_key, status=self._clean_text(body.get('status') or parsed.get('status') or 'all'), media_type=self._clean_text(body.get('media_type') or body.get('type') or parsed.get('type') or 'all'), name=keyword, limit=self._safe_int(body.get('limit'), 20)))
        if assistant_action == 'mp_transfer_history':
            return finish(await self._assistant_mp_transfer_history(session=session, cache_key=cache_key, title=keyword, status=self._clean_text(body.get('status') or parsed.get('status') or 'all'), limit=self._safe_int(body.get('limit'), 10), page=self._safe_int(body.get('page'), 1)))
        if assistant_action == 'mp_ingest_failures':
            return finish(await self._assistant_mp_ingest_failures(session=session, cache_key=cache_key, title=keyword, limit=self._safe_int(body.get('limit'), 10), page=self._safe_int(body.get('page'), 1)))
        if assistant_action == 'ai_failed_samples':
            return finish(await self._assistant_ai_failed_samples(session=session, cache_key=cache_key, keyword=keyword, limit=self._safe_int(body.get('limit'), 10)))
        if assistant_action == 'ai_sample_worklist':
            return finish(await self._assistant_ai_sample_worklist(session=session, cache_key=cache_key, keyword=keyword, limit=self._safe_int(body.get('limit'), 10)))
        if assistant_action == 'ai_sample_insights':
            return finish(await self._assistant_ai_sample_insights(session=session, cache_key=cache_key, keyword=keyword, limit=self._safe_int(body.get('limit'), 20), top=self._safe_int(body.get('top'), 5)))
        if assistant_action == 'ai_replay_failed_sample':
            ...
            return finish(self._assistant_ai_replay_sample_plan_response(sample_index=sample_index, session=session, cache_key=cache_key, remove_if_resolved=remove_if_resolved))
        if assistant_action == 'streaming_recommend':
            return finish(await self._assistant_streaming_recommend(media_type=parsed.get('streaming_media_type') or 'all', intent=parsed.get('streaming_intent') or 'hot', month=parsed.get('streaming_month') or '', window_days=self._safe_int(parsed.get('streaming_window') or body.get('window_days'), 90), session=session, cache_key=cache_key, compact=compact))
        if assistant_action == 'mp_subscribe_control':
            ...
            control_aliases = {'搜索': 'search', '刷新': 'search', 'search': 'search', 'run': 'search', '暂停': 'pause', '停止': 'pause', 'pause': 'pause', 'stop': 'pause', '恢复': 'resume', '继续': 'resume', 'resume': 'resume', 'start': 'resume', '删除': 'delete', '移除': 'delete', 'delete': 'delete', 'remove': 'delete'}
            ...
            if control not in {'search', 'pause', 'resume', 'delete'} or not target:
                return finish({'success': False, 'message': '用法：先发“订阅列表”，再发“刷新订阅 1”“暂停订阅 1”“恢复订阅 1”或“删除订阅 1”。', 'data': self._assistant_response_data(session=session, data={'action': 'mp_subscribe_control', 'ok': False, 'error_code': 'invalid_subscribe_control_args'})})
            if not self._resolve_mp_subscribe_target(target=target, cache_key=cache_key, allow_raw_id=allow_raw_subscribe_id):
                return finish({'success': False, 'message': '未找到可操作的订阅。请先发送“订阅列表”获取列表，再按编号操作；也可以直接传订阅 ID。', 'data': self._assistant_response_data(session=session, data={'action': 'mp_subscribe_control', 'ok': False, 'error_code': 'subscribe_target_not_found', 'target': target})})
            if not self._parse_bool_value(body.get('confirmed') or body.get('execute'), False):
                return finish(immediate(self._assistant_mp_subscribe_control_plan_response(control=control, target=target, session=session, cache_key=cache_key, allow_raw_id=allow_raw_subscribe_id)))
            return finish(await self._assistant_mp_subscribe_control(session=session, cache_key=cache_key, control=control, target=target, allow_raw_id=allow_raw_subscribe_id))
        if assistant_action == 'mp_download_tasks':
            return finish(await self._assistant_mp_download_tasks(session=session, cache_key=cache_key, status=self._clean_text(body.get('status') or parsed.get('status') or 'downloading'), title=keyword, hash_value=self._clean_text(body.get('hash') or body.get('hash_value')), downloader=self._clean_text(body.get('downloader')), limit=self._safe_int(body.get('limit'), 10)))
        if assistant_action == 'mp_download_history':
            return finish(await self._assistant_mp_download_history(session=session, cache_key=cache_key, title=keyword, hash_value=self._clean_text(body.get('hash') or body.get('hash_value') or parsed.get('hash')), limit=self._safe_int(body.get('limit'), 10), page=self._safe_int(body.get('page'), 1)))
        if assistant_action == 'mp_lifecycle_status':
            return finish(await self._assistant_mp_lifecycle_status(session=session, cache_key=cache_key, title=keyword, hash_value=self._clean_text(body.get('hash') or body.get('hash_value') or parsed.get('hash')), limit=self._safe_int(body.get('limit'), 5)))
        if assistant_action == 'mp_ingest_status':
            return finish(await self._assistant_mp_ingest_status(session=session, cache_key=cache_key, title=keyword, hash_value=self._clean_text(body.get('hash') or body.get('hash_value') or parsed.get('hash')), limit=self._safe_int(body.get('limit'), 5)))
        if assistant_action == 'mp_recent_activity':
            return finish(await self._assistant_mp_recent_activity(session=session, cache_key=cache_key, limit=self._safe_int(body.get('limit'), 10), download_only=self._parse_bool_value(body.get('download_only') or parsed.get('download_only'), False), transfer_only=self._parse_bool_value(body.get('transfer_only') or parsed.get('transfer_only'), False)))
        if assistant_action == 'smart_followup':
            return finish(await self._assistant_smart_followup(request, session=session, session_id=cache_key, keyword=keyword, hash_value=self._clean_text(body.get('hash') or body.get('hash_value') or parsed.get('hash')), limit=self._safe_int(body.get('limit'), 5)))
        if assistant_action == 'smart_decision_adjust':
            return finish(await self._assistant_smart_resource_decision_adjust(request, session=session, cache_key=cache_key, state=state, adjust_action=self._clean_text(parsed.get('decision_adjust') or body.get('decision_adjust') or 'decision_continue')))
        if assistant_action == 'execution_followup':
            return finish(await self._assistant_execution_followup(request, session=session, session_id=cache_key, plan_id=self._clean_text(body.get('plan_id') or parsed.get('plan_id'))))
        if assistant_action == 'mp_local_diagnose':
            return finish(await self._assistant_mp_local_diagnose(session=session, cache_key=cache_key, title=keyword, hash_value=self._clean_text(body.get('hash') or body.get('hash_value') or parsed.get('hash')), limit=self._safe_int(body.get('limit'), 5)))
        if assistant_action == 'mp_download_control':
            ...
            control_aliases = {'暂停': 'pause', '停止': 'pause', 'pause': 'pause', 'stop': 'pause', '恢复': 'resume', '继续': 'resume', '开始': 'resume', 'resume': 'resume', 'start': 'resume', '删除': 'delete', '移除': 'delete', 'delete': 'delete', 'remove': 'delete'}
            control = control_aliases.get(control, control)
            ...
            if control not in {'pause', 'resume', 'delete'} or not target:
                return finish({'success': False, 'message': '用法：先发“下载任务”，再发“暂停下载 1”“恢复下载 1”或“删除下载 1”。', 'data': self._assistant_response_data(session=session, data={'action': 'mp_download_control', 'ok': False, 'error_code': 'invalid_download_control_args'})})
            if not self._resolve_mp_download_task_target(target=target, cache_key=cache_key):
                return finish({'success': False, 'message': '未找到可操作的下载任务。请先发送“下载任务”获取列表，再按编号操作；也可以直接传 40 位任务 hash。', 'data': self._assistant_response_data(session=session, data={'action': 'mp_download_control', 'ok': False, 'error_code': 'download_task_target_not_found', 'target': target})})
            if not self._parse_bool_value(body.get('confirmed') or body.get('execute'), False):
                return finish(immediate(self._assistant_mp_download_control_plan_response(control=control, target=target, session=session, cache_key=cache_key, downloader=self._clean_text(body.get('downloader')), delete_files=self._parse_bool_value(body.get('delete_files'), False))))
            return finish(await self._assistant_mp_download_control(session=session, cache_key=cache_key, control=control, target=target, downloader=self._clean_text(body.get('downloader')), delete_files=self._parse_bool_value(body.get('delete_files'), False)))
        if assistant_action == 'mp_download_best':
            ...
            return finish(await self._assistant_mp_best_download_plan(session=session, cache_key=cache_key, preferences=preferences))
        if assistant_action == 'mp_download':
            choice = self._safe_int(parsed.get('keyword') or body.get('choice') or body.get('index'), 0)
            if choice <= 0:
                return finish({'success': False, 'message': '用法：下载 1', 'data': self._assistant_response_data(session=session, data={'action': 'mp_download', 'ok': False})})
            preferences = self._normalize_assistant_preferences((self._assistant_preferences or {}).get(self._normalize_preference_key(session=session)))
            current_state = self._load_session(cache_key)
            current_kind = self._clean_text(current_state.get('kind')) if isinstance(current_state, dict) else ''
            direct_from_pt_list = current_kind == 'assistant_mp'
            if not direct_from_pt_list and (not self._parse_bool_value(body.get('confirmed') or body.get('execute'), False)):
                return finish(immediate(self._assistant_mp_download_plan_response(choice=choice, session=session, cache_key=cache_key, preferences=preferences)))
            return finish(await self._assistant_mp_download(choice=choice, session=session, cache_key=cache_key, preferences=preferences))
        if assistant_action == 'mp_subscribe_search':
            return finish({'success': False, 'data': self._assistant_response_data(session=session, data={'action': 'mp_subscribe_search_removed', 'ok': False})})
        if assistant_action == 'mp_subscribe':
            if not keyword:
                return finish({'success': False, 'message': '用法：订阅 片名', 'data': self._assistant_response_data(session=session, data={'action': assistant_action, 'ok': False})})
            if not self._parse_bool_value(body.get('confirmed') or body.get('execute'), False):
                return finish(immediate(self._assistant_mp_subscribe_plan_response(keyword=keyword, session=session, cache_key=cache_key, immediate_search=False)))
            return finish(await self._assistant_mp_subscribe(keyword=keyword, session=session, immediate_search=False))
        if assistant_action == 'mp_recommendations':
            source, inferred_media_type = self._normalize_mp_recommend_request(body.get('source') or parsed.get('keyword') or text or 'tmdb_trending')
            return finish(await self._assistant_mp_recommendations(source=source, media_type=self._clean_text(body.get('media_type') or body.get('type') or parsed.get('type') or inferred_media_type or 'all'), limit=self._safe_int(body.get('limit'), 20), session=session, cache_key=cache_key))
        if assistant_action == 'return_to_recommend':
            return finish(await self._assistant_restore_mp_recommendation_handoff(session=session, cache_key=cache_key, state=state, limit=self._safe_int(body.get('limit'), 20)))
        if assistant_action == 'switch_recommend_handoff_source':
            return finish(await self._assistant_switch_recommend_handoff_source(request, session=session, cache_key=cache_key, state=state, mode=self._clean_text(parsed.get('mode'))))
        if assistant_action == 'return_to_smart_decision':
            return finish(await self._assistant_route_recommend_handoff_to_smart_decision(request, session=session, cache_key=cache_key, state=state))
        if assistant_action == 'confirm_recommend_handoff':
            return finish(await self._assistant_confirm_recommend_handoff(request, session=session, cache_key=cache_key, state=state, compact=compact, target_path=target_path))
        if assistant_action == 'p115_help':
            summary = self._format_p115_help_text()
            pending_summary = self._pending_p115_summary(state)
            if pending_summary:
                summary = f'{summary}\n{pending_summary}'
            return {'success': True, 'message': summary, 'data': self._assistant_response_data(session=session, data={'action': 'p115_help', 'ok': True, 'status_summary': summary, 'status': self._p115_status_snapshot()})}
        if assistant_action == 'p115_status':
            summary = self._format_p115_status_summary()
            pending_summary = self._pending_p115_summary(state)
            if pending_summary:
                summary = f'{summary}\n{pending_summary}'
            return finish({'success': True, 'message': summary, 'data': self._assistant_response_data(session=session, data={'action': 'p115_status', 'ok': True, 'status_summary': summary, 'status': self._p115_status_snapshot()})})
        if assistant_action == 'p115_pending':
            pending_summary = self._pending_p115_summary(state)
            if not pending_summary:
                return {'success': True, 'message': '当前没有待继续的 115 任务。', 'data': self._assistant_response_data(session=session, data={'action': 'p115_pending', 'ok': True})}
            return {'success': True, 'message': pending_summary, 'data': self._assistant_response_data(session=session, data={'action': 'p115_pending', 'ok': True})}
        if assistant_action == 'quark_clear_default_dir':
            bridge_result = self._assistant_quarkdisk_clear_directory(self._quark_default_path)
            ...
            if not clear_ok:
                ...
                lines = [f'清空夸克默认目录失败：{clear_message or '未知错误'}', f'目录：{target_path}']
                if removed_count:
                ...
                if isinstance(failed_items, list) and failed_items:
                    names = [self._clean_text(item.get('name') if isinstance(item, dict) else item) for item in failed_items[:5]]
                    names = [name for name in names if name]
                    if names:
                        lines.append('失败项：' + '、'.join(names))
                return finish({'success': False, 'message': '\n'.join(lines), 'data': self._assistant_response_data(session=session, data={'action': 'quark_clear_default_dir', 'ok': False, 'target_path': target_path, 'result': clear_result, 'write_effect': 'cloud_drive'})})
            removed_count = self._safe_int((clear_result or {}).get('removed_count'), 0)
            file_count = self._safe_int((clear_result or {}).get('file_count'), 0)
            folder_count = self._safe_int((clear_result or {}).get('folder_count'), 0)
            if removed_count <= 0:
                message_text = f'夸克默认目录当前层已是空目录\n目录：{target_path}'
            else:
                message_text = f'夸克默认目录已清空当前层\n目录：{target_path}\n已删除：{removed_count} 项（文件 {file_count} / 文件夹 {folder_count}）'
            return finish({'success': True, 'message': message_text, 'data': self._assistant_response_data(session=session, data={'action': 'quark_clear_default_dir', 'ok': True, 'target_path': target_path, 'removed_count': removed_count, 'file_count': file_count, 'folder_count': folder_count, 'result': clear_result, 'write_effect': 'cloud_drive'})})
        if assistant_action == 'p115_clear_default_dir':
            p115_service = self._ensure_p115_service()
            clear_ok, clear_result, clear_message = p115_service.clear_directory(self._p115_default_path)
            target_path = self._clean_text((clear_result or {}).get('path')) or self._p115_default_path
            if not clear_ok:
                file_count = self._safe_int((clear_result or {}).get('file_count'), 0)
                folder_count = self._safe_int((clear_result or {}).get('folder_count'), 0)
                lines = [f'清空115默认目录失败：{clear_message or '未知错误'}', f'目录：{target_path}']
                if file_count or folder_count:
                    lines.append(f'当前层项目：文件 {file_count} / 文件夹 {folder_count}')
                return finish({'success': False, 'message': '\n'.join(lines), 'data': self._assistant_response_data(session=session, data={'action': 'p115_clear_default_dir', 'ok': False, 'target_path': target_path, 'result': clear_result, 'write_effect': 'cloud_drive'})})
            removed_count = self._safe_int((clear_result or {}).get('removed_count'), 0)
            file_count = self._safe_int((clear_result or {}).get('file_count'), 0)
            folder_count = self._safe_int((clear_result or {}).get('folder_count'), 0)
            if removed_count <= 0:
                message_text = f'115 默认目录当前层已是空目录\n目录：{target_path}'
            else:
                message_text = f'115 默认目录已清空当前层\n目录：{target_path}\n已删除：{removed_count} 项（文件 {file_count} / 文件夹 {folder_count}）'
            return finish({'success': True, 'message': message_text, 'data': self._assistant_response_data(session=session, data={'action': 'p115_clear_default_dir', 'ok': True, 'target_path': target_path, 'removed_count': removed_count, 'file_count': file_count, 'folder_count': folder_count, 'result': clear_result, 'write_effect': 'cloud_drive'})})
        if assistant_action == 'p115_resume':
            pending_summary = self._pending_p115_summary(state)
            if not pending_summary:
                summary = self._format_p115_status_summary()
                return {'success': False, 'message': f'当前没有待继续的 115 任务。\n{summary}', 'data': self._assistant_response_data(session=session, data={'action': 'p115_resume', 'ok': False})}
            if not self._p115_status_snapshot().get('ready'):
                return {'success': False, 'message': f'{pending_summary}\n当前 115 还不可用，请先回复：115登录', 'data': self._assistant_response_data(session=session, data={'action': 'p115_resume', 'ok': False})}
            resume_ok, resume_message, resume_data = await self._resume_pending_p115_share(request, body, session_id=cache_key, state=state)
            message_text = '已手动继续 115 任务'
            if resume_message:
                message_text = f'{message_text}\n{resume_message}'
            if not resume_ok:
                message_text = f'{message_text}\n任务仍未成功，保留待继续状态。'
            return {'success': resume_ok, 'message': message_text, 'data': self._assistant_response_data(session=session, data={'action': 'p115_resume', 'ok': resume_ok, 'result': resume_data})}
        if assistant_action == 'p115_cancel':
            pending_summary = self._pending_p115_summary(state)
            if not pending_summary:
                return {'success': True, 'message': '当前没有待取消的 115 任务。', 'data': self._assistant_response_data(session=session, data={'action': 'p115_cancel', 'ok': True})}
            self._clear_pending_p115_share(cache_key)
            return {'success': True, 'message': f'{pending_summary}\n已取消并清除这次待继续的 115 任务。', 'data': self._assistant_response_data(session=session, data={'action': 'p115_cancel', 'ok': True})}
        if assistant_action == 'p115_qrcode_start':
            previous_state = state
            client_type = P115TransferService.normalize_qrcode_client_type(parsed.get('client_type') or self._p115_client_type)
            ...
            self._save_session(cache_key, {'kind': 'assistant_p115_login', 'stage': 'qrcode', 'client_type': client_type, 'uid': self._clean_text(data.get('uid')), 'time': self._clean_text(data.get('time')), 'sign': self._clean_text(data.get('sign'))})
            pending_text = ''
            if (previous_state.get('pending_p115') or {}).get('share_url'):
                pending_text = '\n检测到有待继续的 115 任务，登录成功后我会自动继续执行。'
            return {'success': True, 'message': f'115 扫码二维码已生成\n客户端：{client_type}\n请使用 115 App 扫码确认后，再回复：检查115登录{pending_text}', 'data': self._assistant_response_data(session=session, data={'action': 'p115_qrcode_start', 'ok': True, 'qrcode': data.get('qrcode'), 'uid': data.get('uid'), 'time': data.get('time'), 'sign': data.get('sign'), 'client_type': client_type})}
        if assistant_action == 'p115_qrcode_check':
            if not state or str(state.get('kind') or '').strip() != 'assistant_p115_login':
                ...
                if pending_summary and self._p115_status_snapshot().get('ready'):
                    ...
                    if not resume_ok:
                        message_text = f'{message_text}\n任务仍未成功，继续保留待处理状态。'
                    return {'success': resume_ok, 'message': message_text, 'data': self._assistant_response_data(session=session, data={'action': 'p115_qrcode_check', 'ok': resume_ok, 'result': resume_data})}
                summary = self._format_p115_status_summary()
                if pending_summary:
                    summary = f'{summary}\n{pending_summary}'
                return {'success': True, 'message': f'没有待检查的 115 登录会话。\n{summary}\n如需重新扫码登录，请回复：115登录', 'data': {}}
            client_type = P115TransferService.normalize_qrcode_client_type(state.get('client_type') or parsed.get('client_type') or self._p115_client_type)
            qr_ok, data, qr_message = self._ensure_p115_service().check_qrcode_login(uid=self._clean_text(state.get('uid')), time_value=self._clean_text(state.get('time')), sign=self._clean_text(state.get('sign')), client_type=client_type)
            if qr_ok and data.get('status') == 'success':
                cookie = self._clean_text(data.pop('cookie'))
                if cookie:
                    self._p115_cookie = cookie
                    self._p115_client_type = client_type
                    self._apply_runtime_config({'p115_cookie': cookie, 'p115_client_type': client_type})
                    data['cookie_saved'] = True
                    data['cookie_mode'] = 'client_cookie'
                    self._save_session(cache_key, {'stage': 'success', 'client_type': client_type})
            if not qr_ok:
                return {'success': False, 'message': f'115 扫码状态：{qr_message}', 'data': self._assistant_response_data(session=session, data={'action': 'p115_qrcode_check', 'ok': False})}
            status = self._clean_text(data.get('status'))
            lines = ['115 扫码状态', f'状态：{status or 'unknown'}', f'结果：{qr_message}']
            if data.get('cookie_saved'):
                lines.append(self._format_p115_status_summary(title='115 登录完成'))
                resume_ok, resume_message, resume_data = await self._resume_pending_p115_share(request, body, session_id=cache_key, state=state)
                if resume_message:
            ...
            message_text = '\n'.join((line for line in lines if line)).strip()
            return {'success': True, 'message': message_text, 'data': self._assistant_response_data(session=session, data={'action': 'p115_qrcode_check', 'ok': True})}
        if parsed.get('url'):
            provider = 'quark' if self._is_quark_url(parsed['url']) else '115' if self._is_115_url(parsed['url']) else 'unknown'
            result = await self.api_share_route(_JsonRequestShim(request, {'url': parsed['url'], 'access_code': parsed.get('access_code') or '', 'path': target_path, 'trigger': 'Agent影视助手 智能入口', 'apikey': self._extract_apikey(request, body)}))
            if provider == '115':
                if result.get('success'):
                    self._clear_pending_p115_share(cache_key)
                else:
                    self._save_pending_p115_share(cache_key, share_url=parsed['url'], access_code=parsed.get('access_code') or '', target_path=target_path or self._p115_default_path, source='assistant_link', last_error=str(result.get('message') or ''))
            return finish({'success': bool(result.get('success')), 'message': f'{('夸克' if provider == 'quark' else '115' if provider == '115' else '分享')}转存已完成\n目录：{((result.get('data') or {}).get('result') or {}).get('target_path') or ((result.get('data') or {}).get('result') or {}).get('path') or target_path or '-'}' if result.get('success') else f'{str(result.get('message') or '处理失败')}\n{self._format_p115_resume_hint()}' if provider == '115' else str(result.get('message') or '处理失败'), 'data': self._assistant_response_data(session=session, data={'action': 'share_route', 'ok': bool(result.get('success')), 'provider': provider, 'result': result.get('data') or {}})})
        ...
        if not source_order:
            parsed_source_order_text = self._clean_text(parsed.get('source_order_text'))
            if parsed_source_order_text:
                source_order = [self._clean_text(item).lower() for item in parsed_source_order_text.split(',') if self._clean_text(item)]
        ...
        session_preference_overrides: Dict[str, Any] = {}
        cloud_provider = self._clean_text(parsed.get('cloud_provider') or body.get('cloud_provider') or body.get('provider')).lower()
        if cloud_provider == 'quark':
            session_preference_overrides.update({'has_quark': True, 'has_115': False, 'prefer_cloud_provider': 'quark'})
        elif cloud_provider == '115':
            session_preference_overrides.update({'has_quark': False, 'has_115': True, 'prefer_cloud_provider': '115'})
        preferences = self._normalize_assistant_preferences((self._assistant_preferences or {}).get(self._normalize_preference_key(session=session)))
        ...
        if mode == 'mp_download_title':
            if not keyword:
                return finish({'success': False, 'message': '用法：下载 片名', 'data': self._assistant_response_data(session=session, data={'action': 'mp_download_title', 'ok': False, 'error_code': 'missing_keyword'})})
            if not self._keyword_has_explicit_year(keyword, year):
                candidate_result = await self._assistant_mp_candidate_search(keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, pending_action={'mode': 'mp_download_title', 'label': '生成待确认下载计划', 'result_filter': result_filter}, target_path=target_path)
            ...
            preferences = self._normalize_assistant_preferences((self._assistant_preferences or {}).get(self._normalize_preference_key(session=session)))
            result = await self._assistant_mp_media_search(keyword=keyword, session=session, cache_key=cache_key, preferences=preferences, result_filter=result_filter)
            return finish(await self._assistant_attach_download_plan_choices(result, session=session, cache_key=cache_key, preferences=preferences))
        if mode == 'smart':
            if not keyword:
                return finish({'success': False, 'message': '用法：智能搜索 片名', 'data': self._assistant_response_data(session=session, data={'action': 'smart_resource_search', 'ok': False, 'error_code': 'missing_keyword'})})
            if decision_intent == 'make_plan':
                return finish(await self._assistant_smart_resource_plan(request, keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, source_order=source_order, target_path=target_path, origin=origin))
            if decision_intent == 'show_detail':
                return finish(await self._assistant_smart_decision_followup_detail(request, keyword=keyword, session=session, cache_key=cache_key, compact=compact, apikey=apikey, media_type=media_type, year=year, source_order=source_order, target_path=target_path))
            if decision_intent == 'execute_now':
                return finish(await self._assistant_smart_resource_execute(request, keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, source_order=source_order, target_path=target_path, origin=origin))
            return finish(await self._assistant_smart_resource_search(request, keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, source_order=source_order, target_path=target_path, session_preference_overrides=session_preference_overrides, origin=origin))
        if mode == 'cloud_transfer_execute':
            if not keyword:
                return finish({'success': False, 'message': '用法：转存 片名；也支持：夸克转存 片名 / 115转存 片名。', 'data': self._assistant_response_data(session=session, data={'action': 'cloud_transfer_execute', 'ok': False, 'error_code': 'missing_keyword'})})
            if not self._keyword_has_explicit_year(keyword, year):
                pending_label = '云盘转存'
                if cloud_provider == 'quark':
                    pending_label = '夸克转存'
                    ...
                    pending_label = '115转存'
                candidate_result = await self._assistant_mp_candidate_search(keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, pending_action={'mode': 'cloud_transfer_execute', 'cloud_provider': cloud_provider, 'label': pending_label}, target_path=target_path)
                candidates = (candidate_result.get('data') or {}).get('candidates') or []
                if len(candidates) > 1:
                    return finish(candidate_result)
                if len(candidates) == 1:
                    ...
                    media_type = self._clean_text(candidate.get('media_type') or media_type or 'auto').lower() or 'auto'
                    year = candidate_year or year
            return finish(await self._assistant_cloud_transfer_execute(request, keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, source_order=source_order or ['pansou'], target_path=target_path, session_preference_overrides=session_preference_overrides, origin=origin or cloud_provider or 'cloud_transfer'))
        if mode == 'smart_decision':
            if not keyword:
                return finish({'success': False, 'message': '用法：资源决策 片名；也支持：智能决策 片名。', 'data': self._assistant_response_data(session=session, data={'action': 'smart_resource_decision', 'ok': False, 'error_code': 'missing_keyword'})})
            if decision_intent == 'make_plan':
                return finish(await self._assistant_smart_resource_plan(request, keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, source_order=source_order, target_path=target_path, origin=origin))
            if decision_intent == 'show_detail':
                return finish(await self._assistant_smart_decision_followup_detail(request, keyword=keyword, session=session, cache_key=cache_key, compact=compact, apikey=apikey, media_type=media_type, year=year, source_order=source_order, target_path=target_path, decision_profile=self._clean_text(body.get('decision_profile') or parsed.get('decision_profile'))))
            if decision_intent == 'execute_now':
                return finish(await self._assistant_smart_resource_execute(request, keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, source_order=source_order, target_path=target_path, origin=origin))
            return finish(await self._assistant_smart_resource_decision(request, keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, source_order=source_order, target_path=target_path, decision_profile=self._clean_text(body.get('decision_profile') or parsed.get('decision_profile')), origin=origin))
        if mode == 'smart_plan':
            if not keyword and self._clean_text((state or {}).get('kind')) != 'assistant_smart_search':
                return finish({'success': False, 'message': '用法：智能计划 片名；也可以先做“智能搜索 片名”，再回复“计划最佳”。', 'data': self._assistant_response_data(session=session, data={'action': 'smart_resource_plan', 'ok': False, 'error_code': 'missing_keyword'})})
            return finish(await self._assistant_smart_resource_plan(request, keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, source_order=source_order, target_path=target_path, origin=origin))
        if mode == 'smart_execute':
            if not keyword and self._clean_text((state or {}).get('kind')) != 'assistant_smart_search':
                return finish({'success': False, 'message': '用法：智能执行 片名；也可以先做“智能搜索 片名”，再回复“执行最佳”。', 'data': self._assistant_response_data(session=session, data={'action': 'smart_resource_execute', 'ok': False, 'error_code': 'missing_keyword'})})
            return finish(await self._assistant_smart_resource_execute(request, keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year, source_order=source_order, target_path=target_path, origin=origin))
        if mode == 'search':
            preferences = self._normalize_assistant_preferences((self._assistant_preferences or {}).get(self._normalize_preference_key(session=session)))
            pansou_enabled = self._assistant_source_enabled(preferences, 'pansou')
            mp_pt_enabled = self._assistant_source_enabled(preferences, 'mp_pt')
            if not keyword:
                return finish({'success': False, 'message': '用法：搜索 片名', 'data': self._assistant_response_data(session=session, data={'action': 'media_search', 'ok': False})})
            next_mode = ''
            episode_query = self._keyword_prefers_mp_pt_search(keyword)
            if episode_query:
                allowed, disabled = self._ensure_mp_pt_enabled()
                if not allowed:
                ...
                next_mode = 'pansou'
            if not next_mode:
                return finish(self._all_search_sources_disabled_response())
            return finish(await self.api_assistant_route(_JsonRequestShim(request, {'session': session, 'session_id': cache_key, 'mode': next_mode, 'keyword': keyword, 'result_filter': result_filter, 'media_type': media_type, 'year': year, 'path': target_path, 'compact': compact, 'apikey': self._extract_apikey(request, body)})))
        if mode == 'mp':
            allowed, disabled = self._ensure_mp_pt_enabled()
            if not allowed:
                return finish(disabled)
            if not keyword:
                return finish({'success': False, 'message': '用法：MP搜索 片名', 'data': self._assistant_response_data(session=session, data={'action': 'media_search', 'ok': False})})
            preferences = self._normalize_assistant_preferences((self._assistant_preferences or {}).get(self._normalize_preference_key(session=session)))
            if not self._keyword_has_explicit_year(keyword, year):
                candidate_result = await self._assistant_mp_candidate_search(keyword=keyword, session=session, cache_key=cache_key, media_type=media_type, year=year)
                candidates = (candidate_result.get('data') or {}).get('candidates') or []
                if len(candidates) > 1:
                    return finish(candidate_result)
                if len(candidates) == 1:
            ...
            if (not result.get('success') or not mp_items) and keyword:
                episode_origin = origin in {'episode_search', 'update_episode_search'} or self._keyword_prefers_mp_pt_search(keyword)
                next_commands = []
                if episode_origin:
                    next_commands.append(f'PT搜索 {keyword}')
                elif self._assistant_source_enabled(preferences, 'pansou'):
                    next_commands.append(f'盘搜搜索 {keyword}')
                lines = [self._clean_text(result.get('message')) or f'MP/PT 暂无可用结果：{keyword}']
                if episode_origin:
                    lines.append(f'如需继续 PT 侧尝试，可以调整片名或年份后回复：PT搜索 {keyword}')
                elif next_commands:
                    lines.append('如果想继续找云盘资源，可以回复：' + ' / '.join(next_commands))
                result['success'] = False
                result['message'] = '\n'.join((line for line in lines if line)).strip()
                result_data = dict(result.get('data') or {})
                result_data.update({'suggested_commands': next_commands, 'decision_summary': {'stage': 'mp_no_result', 'label': 'MP/PT 暂无结果', 'preferred_command': next_commands[0] if next_commands else '', 'fallback_command': next_commands[1] if len(next_commands) > 1 else '', 'compact_commands': next_commands, 'recommended_agent_behavior': 'ask_user', 'can_auto_run_preferred': False}})
                result['data'] = self._assistant_response_data(session=session, data=result_data)
            if result.get('success') and recommend_handoff:
                current_state = self._load_session(cache_key) or {}
                self._save_session(cache_key, {'recommend_handoff': dict(recommend_handoff)})
                result_data = dict(result.get('data') or {})
        ...
        if mode == 'pansou' or mode == 'cloud':
            preferences = self._normalize_assistant_preferences((self._assistant_preferences or {}).get(self._normalize_preference_key(session=session)))
            pansou_enabled = self._assistant_source_enabled(preferences, 'pansou')
            mp_pt_enabled = self._assistant_source_enabled(preferences, 'mp_pt')
            if mode == 'pansou' and (not pansou_enabled):
                if self._is_explicit_pansou_command(text):
                    return finish(self._pansou_disabled_response())
                if mp_pt_enabled:
                    return finish(await self.api_assistant_route(_JsonRequestShim(request, {'session': session, 'session_id': cache_key, 'mode': 'mp', 'keyword': keyword, 'media_type': media_type, 'year': year, 'path': target_path, 'compact': compact, 'apikey': self._extract_apikey(request, body)})))
                return finish(self._all_search_sources_disabled_response() if self._is_generic_search_command(text) else self._pansou_disabled_response())
            search_ok, payload, search_message, used_keyword = self._call_pansou_search_with_variants(keyword)
            if not search_ok:
                if mode == 'pansou':
                    return finish(self._cloud_no_result_suggestion_response(keyword=keyword, session=session, primary_source='盘搜', mp_pt_enabled=mp_pt_enabled, detail=search_message))
                search_ok = False
                payload = {}
            ...
            for item in channel_115 + channel_quark:
                items.append({'index': len(items) + 1})
            if mode == 'cloud' and (not items):
                return finish(self._assistant_finalize_cloud_result(session=session, cache_key=cache_key, keyword=keyword, pansou_items=[], pansou_total=int(data.get('total') or 0), lead_note=f'已自动改用关键词“{used_keyword}”补查。' if used_keyword and used_keyword != self._clean_text(keyword) else ''))
            if not items and mode == 'pansou':
                return finish(self._cloud_no_result_suggestion_response(keyword=keyword, session=session, primary_source='盘搜', mp_pt_enabled=mp_pt_enabled))
            if items and mode == 'cloud':
                items = self._attach_cloud_scores(items, preferences=preferences, source_type='pansou')
                items = self._rank_pansou_items(items, limit_per_channel=self._assistant_result_page_size)
                return finish(self._assistant_finalize_cloud_result(session=session, cache_key=cache_key, keyword=keyword, pansou_items=items, pansou_total=int(data.get('total') or len(items)), lead_note=f'已自动改用关键词“{used_keyword}”补查。' if used_keyword and used_keyword != self._clean_text(keyword) else ''))
            if items:
                preferences = self._normalize_assistant_preferences((self._assistant_preferences or {}).get(self._normalize_preference_key(session=session)))
                items = self._attach_cloud_scores(items, preferences=preferences, source_type='pansou')
                items = self._rank_pansou_items(items, limit_per_channel=self._assistant_result_page_size)
                return finish(self._assistant_finalize_pansou_result(session=session, cache_key=cache_key, keyword=keyword, items=items, total=int(data.get('total') or len(items)), action_name='pansou_search' if mode == 'pansou' else 'cloud_search', recommend_handoff=recommend_handoff, lead_note=f'已自动改用关键词“{used_keyword}”补查。' if used_keyword and used_keyword != self._clean_text(keyword) else ''))
        return finish({
            "success": False,
            "message": f"不支持的搜索模式：{mode or 'unknown'}",
            "data": self._assistant_response_data(
                session=session,
                data={"action": "assistant_route", "ok": False, "error_code": "unsupported_mode"},
            ),
        })
    ...
    async def api_assistant_action(self, request: Request):
        ...
        async def immediate(result: Dict[str, Any]) -> Dict[str, Any]:
            return result
        route_payload = {'session': body.get('session'), 'session_id': body.get('session_id'), 'path': body.get('path') or body.get('target_path'), 'apikey': self._extract_apikey(request, body)}
        pick_payload = {'session': body.get('session'), 'session_id': body.get('session_id'), 'path': body.get('path') or body.get('target_path'), 'apikey': self._extract_apikey(request, body)}
        if name == 'start_pansou_search':
            route_payload.update({'mode': 'pansou', 'keyword': body.get('keyword')})
            return await finish(self.api_assistant_route(_JsonRequestShim(request, route_payload)))
        if name == 'start_smart_resource_search':
            route_payload.update({'mode': 'smart', 'keyword': body.get('keyword'), 'media_type': body.get('media_type') or 'auto', 'year': body.get('year'), 'source_order': body.get('source_order') if isinstance(body.get('source_order'), list) else None})
            return await finish(self.api_assistant_route(_JsonRequestShim(request, route_payload)))
        if name == 'start_smart_resource_decision':
            route_payload.update({'mode': 'smart_decision', 'keyword': body.get('keyword'), 'media_type': body.get('media_type') or 'auto', 'year': body.get('year'), 'source_order': body.get('source_order') if isinstance(body.get('source_order'), list) else None, 'decision_profile': body.get('decision_profile')})
            return await finish(self.api_assistant_route(_JsonRequestShim(request, route_payload)))
        if name == 'start_smart_resource_plan':
            route_payload.update({'mode': 'smart_plan', 'keyword': body.get('keyword'), 'media_type': body.get('media_type') or 'auto', 'year': body.get('year'), 'source_order': body.get('source_order') if isinstance(body.get('source_order'), list) else None})
            return await finish(self.api_assistant_route(_JsonRequestShim(request, route_payload)))
        if name == 'start_smart_resource_execute':
            route_payload.update({'mode': 'smart_execute', 'keyword': body.get('keyword'), 'media_type': body.get('media_type') or 'auto', 'year': body.get('year'), 'source_order': body.get('source_order') if isinstance(body.get('source_order'), list) else None})
            return await finish(self.api_assistant_route(_JsonRequestShim(request, route_payload)))
        if name == 'start_mp_media_search':
            route_payload.update({'mode': 'mp', 'keyword': body.get('keyword')})
            return await finish(self.api_assistant_route(_JsonRequestShim(request, route_payload)))
        if name == 'query_mp_media_detail':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_mp_media_detail(keyword=self._clean_text(body.get('keyword') or body.get('title')), session=session_name, cache_key=cache_key, media_type=self._clean_text(body.get('media_type') or body.get('type') or 'auto'), year=self._clean_text(body.get('year'))))
        if name == 'query_mp_search_result_detail':
            ...
            return await finish(self._assistant_mp_result_detail(choice=self._safe_int(body.get('choice') or body.get('index'), 0), session=session_name, cache_key=cache_key, preferences=prefs))
        if name == 'query_mp_best_result_detail':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            prefs = self._assistant_preferences_public_data(session=session_name).get('preferences') or self._default_assistant_preferences()
            return await finish(self._assistant_mp_best_result_detail(session=session_name, cache_key=cache_key, preferences=prefs))
        if name == 'pick_mp_best_download':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            prefs = self._assistant_preferences_public_data(session=session_name).get('preferences') or self._default_assistant_preferences()
            return await finish(self._assistant_mp_best_download_plan(session=session_name, cache_key=cache_key, preferences=prefs))
        if name == 'pick_mp_download':
            ...
            if not execute_requested:
                return await finish(immediate(self._assistant_mp_download_plan_response(choice=choice, session=session_name, cache_key=cache_key, preferences=prefs)))
            return await finish(self._assistant_mp_download(choice=choice, session=session_name, cache_key=cache_key, preferences=prefs))
        if name == 'query_mp_download_tasks':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_mp_download_tasks(session=session_name, cache_key=cache_key, status=self._clean_text(body.get('status') or 'downloading'), title=self._clean_text(body.get('title') or body.get('keyword')), hash_value=self._clean_text(body.get('hash') or body.get('hash_value')), downloader=self._clean_text(body.get('downloader')), limit=self._safe_int(body.get('limit'), 10)))
        if name == 'query_mp_download_history':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_mp_download_history(session=session_name, cache_key=cache_key, title=self._clean_text(body.get('title') or body.get('keyword')), hash_value=self._clean_text(body.get('hash') or body.get('hash_value')), limit=self._safe_int(body.get('limit'), 10), page=self._safe_int(body.get('page'), 1)))
        if name == 'query_mp_lifecycle_status':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_mp_lifecycle_status(session=session_name, cache_key=cache_key, title=self._clean_text(body.get('title') or body.get('keyword')), hash_value=self._clean_text(body.get('hash') or body.get('hash_value')), limit=self._safe_int(body.get('limit'), 5)))
        if name == 'query_mp_ingest_status':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_mp_ingest_status(session=session_name, cache_key=cache_key, title=self._clean_text(body.get('title') or body.get('keyword')), hash_value=self._clean_text(body.get('hash') or body.get('hash_value')), limit=self._safe_int(body.get('limit'), 5)))
        ...
        if name == 'query_mp_sites':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_mp_sites(session=session_name, cache_key=cache_key, status=self._clean_text(body.get('status') or 'active'), name=self._clean_text(body.get('site_name') or body.get('keyword') or body.get('title')), limit=self._safe_int(body.get('limit'), 30)))
        if name == 'query_mp_subscribes':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_mp_subscribes(session=session_name, cache_key=cache_key, status=self._clean_text(body.get('status') or 'all'), media_type=self._clean_text(body.get('media_type') or body.get('type') or 'all'), name=self._clean_text(body.get('subscribe_name') or body.get('keyword') or body.get('title')), limit=self._safe_int(body.get('limit'), 20)))
        if name == 'query_mp_ingest_failures':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_mp_ingest_failures(session=session_name, cache_key=cache_key, title=self._clean_text(body.get('title') or body.get('keyword')), limit=self._safe_int(body.get('limit'), 10), page=self._safe_int(body.get('page'), 1)))
        if name == 'query_ai_failed_samples':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_ai_failed_samples(session=session_name, cache_key=cache_key, keyword=self._clean_text(body.get('title') or body.get('keyword')), limit=self._safe_int(body.get('limit'), 10)))
        if name == 'query_ai_sample_worklist':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_ai_sample_worklist(session=session_name, cache_key=cache_key, keyword=self._clean_text(body.get('title') or body.get('keyword')), limit=self._safe_int(body.get('limit'), 10)))
        if name == 'query_ai_sample_insights':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_ai_sample_insights(session=session_name, cache_key=cache_key, keyword=self._clean_text(body.get('title') or body.get('keyword')), limit=self._safe_int(body.get('limit'), 20), top=self._safe_int(body.get('top'), 5)))
        if name == 'replay_ai_failed_sample':
            ...
            execute_requested = self._parse_bool_value(body.get('execute') or body.get('confirmed'), False)
            if not execute_requested:
                return await finish(immediate(self._assistant_ai_replay_sample_plan_response(sample_index=sample_index, session=session_name, cache_key=cache_key, remove_if_resolved=remove_if_resolved)))
            return await finish(self._assistant_ai_replay_failed_sample(sample_index=sample_index, session=session_name, cache_key=cache_key, remove_if_resolved=remove_if_resolved))
        if name == 'query_mp_transfer_history':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_mp_transfer_history(session=session_name, cache_key=cache_key, title=self._clean_text(body.get('title') or body.get('keyword')), status=self._clean_text(body.get('status') or 'all'), limit=self._safe_int(body.get('limit'), 10), page=self._safe_int(body.get('page'), 1)))
        if name == 'query_mp_recent_activity':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_mp_recent_activity(session=session_name, cache_key=cache_key, limit=self._safe_int(body.get('limit'), 10), download_only=self._parse_bool_value(body.get('download_only'), False), transfer_only=self._parse_bool_value(body.get('transfer_only'), False)))
        if name == 'query_mp_local_diagnose':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_mp_local_diagnose(session=session_name, cache_key=cache_key, title=self._clean_text(body.get('title') or body.get('keyword')), hash_value=self._clean_text(body.get('hash') or body.get('hash_value')), limit=self._safe_int(body.get('limit'), 5)))
        if name == 'query_execution_followup':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_execution_followup(request, session=session_name, session_id=cache_key, plan_id=self._clean_text(body.get('plan_id'))))
        if name == 'query_smart_followup':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            return await finish(self._assistant_smart_followup(request, session=session_name, session_id=cache_key, keyword=self._clean_text(body.get('title') or body.get('keyword')), hash_value=self._clean_text(body.get('hash') or body.get('hash_value')), limit=self._safe_int(body.get('limit'), 5)))
        if name == 'mp_subscribe_control':
            ...
            execute_requested = self._parse_bool_value(body.get('execute') or body.get('confirmed'), False)
            if not execute_requested:
                return await finish(immediate(self._assistant_mp_subscribe_control_plan_response(control=control, target=target, session=session_name, cache_key=cache_key, allow_raw_id=allow_raw_subscribe_id)))
            return await finish(self._assistant_mp_subscribe_control(session=session_name, cache_key=cache_key, control=control, target=target, allow_raw_id=allow_raw_subscribe_id))
        if name == 'mp_download_control':
            ...
            execute_requested = self._parse_bool_value(body.get('execute') or body.get('confirmed'), False)
            if not execute_requested:
                return await finish(immediate(self._assistant_mp_download_control_plan_response(control=control, target=target, session=session_name, cache_key=cache_key, downloader=downloader, delete_files=delete_files)))
            return await finish(self._assistant_mp_download_control(session=session_name, cache_key=cache_key, control=control, target=target, downloader=downloader, delete_files=delete_files))
        if name == 'start_mp_subscribe':
            session_name, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or 'default', session_id=body.get('session_id'))
            keyword = self._clean_text(body.get('keyword') or body.get('title'))
            if not keyword:
                state = self._load_session(cache_key) or {}
            ...
            execute_requested = self._parse_bool_value(body.get('execute') or body.get('confirmed'), False)
            if not execute_requested:
                return await finish(immediate(self._assistant_mp_subscribe_plan_response(keyword=keyword, session=session_name, cache_key=cache_key, immediate_search=False)))
            return await finish(self._assistant_mp_subscribe(keyword=keyword, session=session_name, immediate_search=False))
        if name == 'start_mp_recommendations':
            recommend_session, recommend_cache_key = self._normalize_assistant_session_ref(session=self._clean_text(body.get('session')) or 'default', session_id=body.get('session_id'))
            source_name, inferred_media_type = self._normalize_mp_recommend_request(body.get('source') or 'tmdb_trending')
            return await finish(self._assistant_mp_recommendations(source=source_name, media_type=self._clean_text(body.get('media_type') or body.get('type') or inferred_media_type) or 'all', limit=self._safe_int(body.get('limit'), 20), session=recommend_session, cache_key=recommend_cache_key))
        if name == 'pick_recommend_search':
            pick_payload.update({'choice': body.get('choice') or body.get('index'), 'mode': body.get('mode') or body.get('search_mode') or 'mp'})
        ...
        if name in {'preferences_get', 'preferences_save', 'preferences_reset'}:
            ...
            payload = {'session': body.get('session'), 'session_id': body.get('session_id'), 'user_key': body.get('user_key'), 'preferences': body.get('preferences') or {}, 'compact': body.get('compact', True), 'apikey': self._extract_apikey(request, body)}
        ...
        if name in {'scoring_policy', 'query_scoring_policy'}:
            ...
            return await finish({'success': True, 'message': '评分策略已返回。云盘与 PT 使用不同规则；自动化决策以硬风险和 score_summary 为准。', 'data': self._assistant_response_data(session=session_name, data={'action': 'scoring_policy', 'ok': True, 'scoring_policy': self._assistant_scoring_policy_public_data()})})
        if name == 'start_115_login':
            route_payload.update({'action': 'p115_qrcode_start', 'client_type': body.get('client_type')})
            return await finish(self.api_assistant_route(_JsonRequestShim(request, route_payload)))
        if name == 'route_share':
            route_payload.update({'url': body.get('url') or body.get('share_url'), 'access_code': body.get('access_code')})
            return await finish(self.api_assistant_route(_JsonRequestShim(request, route_payload)))
        if name == 'inspect_session_state':
            return await finish(self.api_assistant_session_state(_JsonRequestShim(request, {'session': body.get('session'), 'session_id': body.get('session_id'), 'apikey': self._extract_apikey(request, body)})))
        if name in {'execute_latest_plan', 'execute_plan', 'execute_session_latest_plan'}:
            return await finish(self.api_assistant_plan_execute(_JsonRequestShim(request, {'plan_id': body.get('plan_id'), 'session': body.get('session'), 'session_id': body.get('session_id'), 'prefer_unexecuted': body.get('prefer_unexecuted', True), 'stop_on_error': body.get('stop_on_error', True), 'include_raw_results': body.get('include_raw_results', False), 'apikey': self._extract_apikey(request, body)})))
        ...
        if name == 'clear_current_session':
            return await finish(self.api_assistant_session_clear(_JsonRequestShim(request, {'session': body.get('session'), 'session_id': body.get('session_id'), 'apikey': self._extract_apikey(request, body)})))
        if name == 'inspect_session':
            return await finish(self.api_assistant_session_state(_JsonRequestShim(request, {'session': body.get('session'), 'session_id': body.get('session_id'), 'apikey': self._extract_apikey(request, body)})))
        if name == 'clear_session_by_id':
            return await finish(self.api_assistant_sessions_clear(_JsonRequestShim(request, {'session_id': body.get('session_id'), 'apikey': self._extract_apikey(request, body)})))
        if name == 'clear_stale_sessions':
            return await finish(self.api_assistant_sessions_clear(_JsonRequestShim(request, {'stale_only': True, 'limit': body.get('limit') or 100, 'apikey': self._extract_apikey(request, body)})))
        if name == 'clear_executed_plans':
            return await finish(self.api_assistant_plans_clear(_JsonRequestShim(request, {'executed': True, 'limit': body.get('limit') or 100, 'apikey': self._extract_apikey(request, body)})))
    ...
    def _assistant_workflow_actions(self, name: str, body: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], str]:
        ...
        if workflow_name == 'smart_resource_search':
            ...
            return ([base({'name': 'start_smart_resource_search', 'keyword': keyword, 'media_type': media_type, 'year': year, 'source_order': source_order})], '')
        if workflow_name == 'smart_resource_decision':
            if not keyword:
                return ([], 'smart_resource_decision 缺少 keyword')
            source_order = body.get('source_order') if isinstance(body.get('source_order'), list) else []
            return ([base({'name': 'start_smart_resource_decision', 'keyword': keyword, 'media_type': media_type, 'year': year, 'source_order': source_order, 'decision_profile': body.get('decision_profile')})], '')
        if workflow_name == 'smart_resource_plan':
            source_order = body.get('source_order') if isinstance(body.get('source_order'), list) else []
            return ([base({'name': 'start_smart_resource_plan', 'keyword': keyword, 'media_type': media_type, 'year': year, 'source_order': source_order})], '')
        if workflow_name == 'smart_resource_execute':
            source_order = body.get('source_order') if isinstance(body.get('source_order'), list) else []
            return ([base({'name': 'start_smart_resource_execute', 'keyword': keyword, 'media_type': media_type, 'year': year, 'source_order': source_order})], '')
        if workflow_name == 'pansou_transfer':
            if not keyword:
                return ([], 'pansou_transfer 缺少 keyword')
            choice = self._safe_int(body.get('choice'), 1)
            return ([base({'name': 'start_pansou_search', 'keyword': keyword}), base({'name': 'pick_pansou_result', 'choice': max(1, choice)})], '')
        ...
        if workflow_name == 'mp_media_detail':
            ...
            return ([base({'name': 'query_mp_media_detail', 'keyword': keyword, 'media_type': media_type, 'year': year})], '')
        if workflow_name == 'mp_search_detail':
            if not keyword:
                return ([], 'mp_search_detail 缺少 keyword')
            choice = self._safe_int(body.get('choice'), 1)
            return ([base({'name': 'start_mp_media_search', 'keyword': keyword}), base({'name': 'query_mp_search_result_detail', 'choice': max(1, choice)})], '')
        if workflow_name == 'mp_search_best':
            if not keyword:
                return ([], 'mp_search_best 缺少 keyword')
            return ([base({'name': 'start_mp_media_search', 'keyword': keyword}), base({'name': 'query_mp_best_result_detail'})], '')
        if workflow_name == 'mp_search_download':
            if not keyword:
                return ([], 'mp_search_download 缺少 keyword')
            choice = self._safe_int(body.get('choice'), 1)
            return ([base({'name': 'start_mp_media_search', 'keyword': keyword}), base({'name': 'pick_mp_download', 'choice': max(1, choice)})], '')
        if workflow_name == 'mp_download_tasks':
            return ([base({'name': 'query_mp_download_tasks', 'status': self._clean_text(body.get('status')) or 'downloading', 'title': self._clean_text(body.get('title') or body.get('keyword')), 'hash': self._clean_text(body.get('hash') or body.get('hash_value')), 'downloader': self._clean_text(body.get('downloader')), 'limit': self._safe_int(body.get('limit'), 10)})], '')
        if workflow_name == 'mp_download_history':
            return ([base({'name': 'query_mp_download_history', 'keyword': keyword, 'hash': self._clean_text(body.get('hash') or body.get('hash_value')), 'limit': self._safe_int(body.get('limit'), 10), 'page': self._safe_int(body.get('page'), 1)})], '')
        if workflow_name == 'mp_lifecycle_status':
            return ([base({'name': 'query_mp_lifecycle_status', 'keyword': keyword, 'hash': self._clean_text(body.get('hash') or body.get('hash_value')), 'limit': self._safe_int(body.get('limit'), 5)})], '')
        if workflow_name == 'mp_ingest_status':
            return ([base({'name': 'query_mp_ingest_status', 'keyword': keyword, 'hash': self._clean_text(body.get('hash') or body.get('hash_value')), 'limit': self._safe_int(body.get('limit'), 5)})], '')
        if workflow_name == 'mp_downloaders':
            return ([base({'name': 'query_mp_downloaders'})], '')
        if workflow_name == 'mp_sites':
            return ([base({'name': 'query_mp_sites', 'status': self._clean_text(body.get('status')) or 'active', 'keyword': keyword, 'limit': self._safe_int(body.get('limit'), 30)})], '')
        if workflow_name == 'mp_download_control':
            control = self._clean_text(body.get('control') or body.get('download_control') or body.get('operation'))
            target = self._clean_text(body.get('target') or body.get('hash') or body.get('index') or body.get('choice'))
            if not control or not target:
                return ([], 'mp_download_control 需要 control 和 target')
            return ([base({'name': 'mp_download_control', 'control': control, 'target': target, 'downloader': self._clean_text(body.get('downloader')), 'delete_files': self._parse_bool_value(body.get('delete_files'), False)})], '')
        ...
        if workflow_name == 'mp_subscribes':
            return ([base({'name': 'query_mp_subscribes', 'status': self._clean_text(body.get('status')) or 'all', 'media_type': self._clean_text(body.get('media_type') or body.get('type')) or 'all', 'keyword': keyword, 'limit': self._safe_int(body.get('limit'), 20)})], '')
        if workflow_name == 'mp_subscribe_control':
            control = self._clean_text(body.get('control') or body.get('subscribe_control') or body.get('operation'))
            target = self._clean_text(body.get('target') or body.get('subscribe_id') or body.get('index') or body.get('choice'))
            if not control or not target:
                return ([], 'mp_subscribe_control 需要 control 和 target')
            return ([base({'name': 'mp_subscribe_control', 'control': control, 'target': target})], '')
        if workflow_name == 'mp_transfer_history':
            return ([base({'name': 'query_mp_transfer_history', 'keyword': keyword, 'status': self._clean_text(body.get('status')) or 'all', 'limit': self._safe_int(body.get('limit'), 10), 'page': self._safe_int(body.get('page'), 1)})], '')
        if workflow_name == 'mp_ingest_failures':
            return ([base({'name': 'query_mp_ingest_failures', 'keyword': keyword, 'limit': self._safe_int(body.get('limit'), 10), 'page': self._safe_int(body.get('page'), 1)})], '')
        if workflow_name == 'ai_failed_samples':
            return ([base({'name': 'query_ai_failed_samples', 'keyword': keyword, 'limit': self._safe_int(body.get('limit'), 10)})], '')
        if workflow_name == 'ai_sample_worklist':
            return ([base({'name': 'query_ai_sample_worklist', 'keyword': keyword, 'limit': self._safe_int(body.get('limit'), 10)})], '')
        if workflow_name == 'ai_sample_insights':
            return ([base({'name': 'query_ai_sample_insights', 'keyword': keyword, 'limit': self._safe_int(body.get('limit'), 20), 'top': self._safe_int(body.get('top'), 5)})], '')
        if workflow_name == 'ai_replay_failed_sample':
            sample_index = self._safe_int(body.get('sample_index') or body.get('index'), 0)
            if sample_index <= 0:
                return ([], 'ai_replay_failed_sample 需要 sample_index')
            return ([base({'name': 'replay_ai_failed_sample', 'sample_index': sample_index, 'remove_if_resolved': self._parse_bool_value(body.get('remove_if_resolved'), True)})], '')
        if workflow_name == 'mp_recent_activity':
            return ([base({'name': 'query_mp_recent_activity', 'limit': self._safe_int(body.get('limit'), 10), 'download_only': self._parse_bool_value(body.get('download_only'), False), 'transfer_only': self._parse_bool_value(body.get('transfer_only'), False)})], '')
        if workflow_name == 'mp_local_diagnose':
            return ([base({'name': 'query_mp_local_diagnose', 'keyword': keyword, 'hash': self._clean_text(body.get('hash') or body.get('hash_value')), 'limit': self._safe_int(body.get('limit'), 5)})], '')
        if workflow_name == 'execution_followup':
            return ([base({'name': 'query_execution_followup', 'plan_id': self._clean_text(body.get('plan_id'))})], '')
        if workflow_name == 'smart_followup':
            return ([base({'name': 'query_smart_followup', 'keyword': keyword, 'hash': self._clean_text(body.get('hash') or body.get('hash_value')), 'limit': self._safe_int(body.get('limit'), 5)})], '')
        if workflow_name == 'mp_recommend':
            source_name, inferred_media_type = self._normalize_mp_recommend_request(source)
            return ([base({'name': 'start_mp_recommendations', 'source': source_name, 'media_type': self._clean_text(body.get('media_type')) or inferred_media_type or 'all', 'limit': max(1, min(50, limit))})], '')
        ...
        if workflow_name == 'mp_recommend_search':
            if keyword:
                return ([base({'name': 'start_mp_media_search', 'keyword': keyword})], '')
            source_name, inferred_media_type = self._normalize_mp_recommend_request(source)
            actions = [base({'name': 'start_mp_recommendations', 'source': source_name, 'media_type': self._clean_text(body.get('media_type')) or inferred_media_type or 'all', 'limit': max(1, min(50, limit))})]
            choice = self._safe_int(body.get('choice'), 0)
            if choice > 0:
                actions.append(base({'name': 'pick_recommend_search', 'choice': choice, 'mode': self._clean_text(body.get('mode')) or 'mp'}))
            return (actions, '')
        if workflow_name == 'share_transfer':
            share_url = self._clean_text(body.get('url') or body.get('share_url'))
            if not share_url:
                return ([], 'share_transfer 缺少 url')
            return ([base({'name': 'route_share', 'url': share_url, 'access_code': self._clean_text(body.get('access_code'))})], '')
        if workflow_name == 'p115_login_start':
            return ([base({'name': 'start_115_login', 'client_type': self._clean_text(body.get('client_type'))})], '')
    ...
    async def api_assistant_workflow(self, request: Request):
        if request.method.upper() == 'GET':
            ...
            if not ok:
                return {'success': False, 'message': message}
            return {'success': True, 'message': 'Agent影视助手 预设工作流目录', 'data': self._assistant_response_data(session='default', data={'action': 'workflow_catalog', 'ok': True})}
        body = await request.json()
        ok, message = self._check_api_access(request, body)
        if not ok:
            return {'success': False, 'message': message}
        ...
        session = self._clean_text(body.get('session')) or 'default'
        write_workflows = {'pansou_transfer', 'share_transfer', 'ai_replay_failed_sample', 'mp_search_download', 'mp_download_control', 'mp_subscribe', 'mp_subscribe_control'}
        dry_run = self._parse_bool_value(body.get('dry_run'), self._clean_text(workflow_name).lower() in write_workflows)
        if dry_run:
            if workflow_name == 'mp_download_control':
                control = self._clean_text(body.get('control') or body.get('download_control') or body.get('operation'))
                target = self._clean_text(body.get('target') or body.get('hash') or body.get('index') or body.get('choice'))
                result = self._assistant_mp_download_control_plan_response(control=control, target=target, session=session, cache_key=self._clean_text(body.get('session_id')), downloader=self._clean_text(body.get('downloader')), delete_files=self._parse_bool_value(body.get('delete_files'), False))
                if not result.get('success'):
                    return result
            if workflow_name == 'mp_subscribe_control':
                control = self._clean_text(body.get('control') or body.get('subscribe_control') or body.get('operation'))
                ...
                result = self._assistant_mp_subscribe_control_plan_response(control=control, target=target, session=session, cache_key=self._clean_text(body.get('session_id')), allow_raw_id=allow_raw_id)
                if not result.get('success'):
                    return result
            if workflow_name == 'ai_replay_failed_sample':
                result = self._assistant_ai_replay_sample_plan_response(sample_index=self._safe_int(body.get('sample_index') or body.get('index'), 0), session=session, cache_key=self._clean_text(body.get('session_id')), remove_if_resolved=self._parse_bool_value(body.get('remove_if_resolved'), True))
                if not result.get('success'):
                    return result
                return self._assistant_workflow_plan_response_compact(result) if compact else result
            execute_body = {'dry_run': False}
            plan = self._save_workflow_plan(workflow=workflow_name, session=session, session_id=self._clean_text(body.get('session_id')), actions=actions, execute_body=execute_body)
            full_data = self._assistant_response_data(session=session, data={'action': 'workflow_plan', 'ok': True, 'plan_id': plan.get('plan_id'), 'workflow': workflow_name, 'dry_run': True, 'workflow_actions': actions, 'estimated_steps': len(actions), 'ready_to_execute': True, 'execute_endpoint': '/api/v1/plugin/AgentResourceOfficer/assistant/workflow', 'execute_plan_endpoint': '/api/v1/plugin/AgentResourceOfficer/assistant/plan/execute', 'execute_plan_body': {'plan_id': plan.get('plan_id')}, 'execute_body': execute_body, 'plan_created_at': plan.get('created_at'), 'plan_created_at_text': plan.get('created_at_text')})
            return {'success': True, 'message': f'工作流 {workflow_name} 计划已生成：{plan.get('plan_id')}，共 {len(actions)} 步，未实际执行。', 'data': self._assistant_workflow_plan_compact_data(full_data) if compact else full_data}
        result = await self.api_assistant_actions(_JsonRequestShim(request, {'actions': actions, 'workflow': workflow_name, 'session': session, 'session_id': self._clean_text(body.get('session_id')), 'execute': True, 'stop_on_error': self._parse_bool_value(body.get('stop_on_error'), True), 'include_raw_results': self._parse_bool_value(body.get('include_raw_results'), False), 'compact': compact, 'apikey': self._extract_apikey(request, body)}))
        data = dict(result.get('data') or {})
        data['workflow'] = workflow_name
        if not compact:
            data['workflow_actions'] = actions
        return {'success': bool(result.get('success')), 'message': f'工作流 {workflow_name} 执行完成\n{result.get('message') or ''}'.strip(), 'data': data}
    ...
    async def api_assistant_pick(self, request: Request):
        ...
        if not self._enabled:
            return {'success': False, 'message': '插件未启用'}
        session, cache_key = self._normalize_assistant_session_ref(session=body.get('session') or body.get('chat_id') or body.get('user_id') or body.get('conversation_id') or 'default', session_id=body.get('session_id'))
        index = self._safe_int(body.get('index') or body.get('choice') or body.get('selection') or body.get('number'), 0)
        ...
        if kind == "assistant_cloud":
            ...
            if action == "next_page":
                ...
                self._save_session(cache_key, {**state, "page": next_page, "page_size": page_size})
                return finish({
                    "success": True,
                    "message": self._format_cloud_text(
                        keyword=self._clean_text(state.get("keyword")),
                        pansou_items=pansou_items,
                        pansou_total=self._safe_int(state.get("pansou_total"), total_count),
                        page=next_page,
                        page_size=page_size,
                    ),
                    "data": self._assistant_response_data(
                        session=session,
                        data={"action": "cloud_search_next_page", "ok": True, "page": next_page, "page_size": page_size, "total_pages": total_pages},
                    ),
                })
            if action == "best" and pansou_items:
                delegate_state = {
                    ...
                    "keyword": state.get("keyword"),
                    "items": pansou_items,
                    "total": self._safe_int(state.get("pansou_total"), total_count),
                    "page": current_page,
                    "page_size": page_size,
                }
                self._save_session(cache_key, delegate_state)
                try:
                    return finish(await self.api_assistant_pick(_JsonRequestShim(request, {"session": session, "session_id": cache_key, "choice": 0, "action": "best", "path": target_path, "compact": compact, "apikey": self._extract_apikey(request, body)})))
                finally:
                    self._save_session(cache_key, state)
            if index <= 0:
                return {"success": False, "message": "请选择有效序号，例如：选择 1"}
            if index > total_count:
                return {"success": False, "message": f"序号超出范围，请输入 1 到 {total_count} 之间的数字。"}
            delegate_state = {
                "kind": "assistant_pansou",
                "stage": "result",
                "keyword": state.get("keyword"),
            ...
            self._save_session(cache_key, delegate_state)
            try:
                return finish(await self.api_assistant_pick(_JsonRequestShim(request, {"session": session, "session_id": cache_key, "choice": index, "action": action, "path": target_path, "compact": compact, "apikey": self._extract_apikey(request, body)})))
            finally:
                self._save_session(cache_key, state)
        if kind == 'assistant_pansou':
            items = state.get('items') or []
            ...
            if action == 'next_page':
                ...
                if current_page >= total_pages:
                    return {'success': False, 'message': '已经是最后一页了，可以直接回复编号继续选择。'}
                next_page = current_page + 1
                updated_state = {'page': next_page, 'page_size': page_size}
            ...
            if action == 'best':
                best = self._best_scored_source_item(items)
                if not best:
                    return finish({'success': False, 'message': '当前盘搜结果没有可评分条目，请直接回复编号选择。', 'data': self._assistant_response_data(session=session, data={'action': 'pansou_best_detail', 'ok': False, 'error_code': 'best_item_not_found'})})
                return finish({'success': True, 'message': self._format_cloud_item_detail_text(best, title='盘搜当前最佳候选'), 'data': self._assistant_response_data(session=session, data={'action': 'pansou_best_detail', 'ok': True, 'item': best, 'score_summary': self._score_summary([best], limit=1)})} if not state.get('recommend_handoff') else {'success': True, 'message': self._format_cloud_item_detail_text(best, title='盘搜当前最佳候选'), 'data': self._assistant_response_data(session=session, data={'action': 'pansou_best_detail', 'ok': True, 'item': best, 'score_summary': self._score_summary([best], limit=1), 'decision_summary': self._assistant_recommend_handoff_detail_summary(state)})})
            if action == 'best_execute':
                best = self._best_scored_source_item(items)
                if not best:
                    return finish({'success': False, 'message': '当前盘搜结果没有可评分条目，请先查看列表后再执行最佳。', 'data': self._assistant_response_data(session=session, data={'action': 'pansou_best_execute', 'ok': False, 'error_code': 'best_item_not_found'})})
                ...
                final_path = target_path or (self._p115_default_path if self._is_115_url(share_url) else self._quark_default_path)
                plan_result = self._save_assistant_pick_plan_response(workflow='pansou_best_execute', session=session, session_id=cache_key, actions=[{'name': 'route_share', 'session': session, 'session_id': cache_key, 'url': share_url, 'access_code': access_code, 'path': final_path}], execute_body={'workflow': 'pansou_best_execute', 'session': session, 'session_id': cache_key, 'choice': index, 'path': final_path}, message='盘搜最佳候选计划已生成', score_items=[selected], extra_data={'choice': index, 'provider': '115' if self._is_115_url(share_url) else 'quark' if self._is_quark_url(share_url) else selected.get('channel'), 'target_path': final_path, 'selected_item': selected})
                return finish(await self._assistant_execute_prepared_plan_result(request, session=session, cache_key=cache_key, plan_result=plan_result, message_prefix=f'已自动选择盘搜最佳候选并执行：#{index}', extra_data={'choice': index, 'source_type': 'pansou'}))
            if action == 'best_plan':
                best = self._best_scored_source_item(items)
                if not best:
                    return finish({'success': False, 'message': '当前盘搜结果没有可评分条目，请先查看列表后再生成计划。', 'data': self._assistant_response_data(session=session, data={'action': 'pansou_best_plan', 'ok': False, 'error_code': 'best_item_not_found'})})
                index = self._safe_int(best.get('index'), 0)
                selected = dict(best)
                share_url = self._clean_text(selected.get('url'))
                if not share_url:
                ...
                access_code = self._clean_text(selected.get('password'))
                final_path = target_path or (self._p115_default_path if self._is_115_url(share_url) else self._quark_default_path)
                actions = [{'name': 'route_share', 'session': session, 'session_id': cache_key, 'url': share_url, 'access_code': access_code, 'path': final_path}]
                result = self._save_assistant_pick_plan_response(workflow='pansou_best_plan', session=session, session_id=cache_key, actions=actions, execute_body={'workflow': 'pansou_best_plan', 'session': session, 'session_id': cache_key, 'choice': index, 'path': final_path}, message='盘搜最佳候选计划已生成', score_items=[selected], extra_data={'choice': index, 'provider': '115' if self._is_115_url(share_url) else 'quark' if self._is_quark_url(share_url) else selected.get('channel'), 'target_path': final_path, 'selected_item': selected})
            ...
            access_code = self._clean_text(selected.get('password'))
            final_path = target_path or (self._p115_default_path if self._is_115_url(share_url) else self._quark_default_path)
            route_result = await self.api_share_route(_JsonRequestShim(request, {'url': share_url, 'access_code': access_code, 'path': final_path, 'trigger': 'Agent影视助手 智能入口盘搜选择', 'apikey': self._extract_apikey(request, body)}))
            if not route_result.get('success'):
                if self._is_115_url(share_url):
                    self._save_pending_p115_share(cache_key, share_url=share_url, access_code=access_code, target_path=final_path, source='assistant_pansou_pick', title=selected.get('note') or '', last_error=str(route_result.get('message') or ''))
                    return finish({'success': False, 'message': f'{str(route_result.get('message') or '转存失败')}\n{self._format_p115_resume_hint(selected.get('note') or '')}', 'data': self._assistant_response_data(session=session, data=route_result.get('data') or {})})
                if self._is_quark_url(share_url) and self._is_quark_share_restricted_message(str(route_result.get('message') or '')):
                    recovered = await self._assistant_retry_pansou_quark_candidates(request, items=items, selected_index=index, selected_url=share_url, session=session, target_path=final_path, apikey=self._extract_apikey(request, body))
                    if recovered:
                        return finish(recovered)
                return finish({'success': False, 'message': str(route_result.get('message') or '转存失败'), 'data': self._assistant_response_data(session=session, data=route_result.get('data') or {})})
            if self._is_115_url(share_url):
                self._clear_pending_p115_share(cache_key)
            ...
            text_message = '\n'.join(['盘搜结果已执行转存', f'资源：{selected.get('note') or '未命名资源'}', f'类型：{provider or selected.get('channel') or '-'}', f'目录：{directory or '-'}'])
            return finish({'success': True, 'message': text_message, 'data': self._assistant_response_data(session=session, data={'action': 'share_route', 'ok': True})})
        if kind == 'assistant_mp_recommend':
            items = state.get('items') or []
            selected_index = self._safe_int(state.get('selected_index'), 0)
            pick_action = self._normalize_pick_action(body.get('action'))
            next_mode = self._clean_text(body.get('mode') or body.get('search_mode') or body.get('target') or '').lower()
            if index <= 0:
                index = selected_index
            if index <= 0 and (pick_action in {'detail', 'best', 'plan', 'best_execute'} or next_mode):
                index = 1
            if index <= 0:
            ...
            if pick_action == 'detail':
                self._save_session(cache_key, {'selected_index': index, 'selected_item': selected})
            ...
            mode_aliases = {'原生': 'mp', 'mp搜索': 'mp', '盘搜': 'pansou', 'ps': 'pansou', '计划': 'smart_plan', 'smart_plan': 'smart_plan', '确认': 'smart_execute', '执行': 'smart_execute', '直接执行': 'smart_execute', 'smart_execute': 'smart_execute', '决策': 'smart_decision', '资源决策': 'smart_decision', '智能决策': 'smart_decision', 'smart': 'smart_decision', 'smart_decision': 'smart_decision'}
            if not next_mode:
                if pick_action == 'plan':
            ...
            media_type_aliases = {'电影': 'movie', 'movie': 'movie', 'movies': 'movie', '电视剧': 'tv', '剧集': 'tv', '番剧': 'tv', 'tv': 'tv', 'series': 'tv', 'all': 'auto', '全部': 'auto'}
            ...
            routed_body = {'session': session, 'session_id': cache_key, 'mode': next_mode, 'keyword': next_keyword, 'media_type': selected_media_type, 'path': target_path, 'apikey': self._extract_apikey(request, body)}
            if next_mode in {'smart_decision', 'smart_plan', 'smart_execute'}:
                routed_body['origin'] = 'mp_recommend'
            else:
                routed_body['recommend_handoff'] = self._assistant_recommend_handoff_state(source=self._clean_text(state.get('source')), requested_source=self._clean_text(state.get('requested_source') or state.get('source')), media_type=selected_media_type, selected_index=index, selected_item=selected)
            return finish(await self.api_assistant_route(_JsonRequestShim(request, routed_body)))
        if kind == 'assistant_smart_search':
            if action == 'best_execute':
                return finish(await self._assistant_smart_best_execute_response(request, session=session, cache_key=cache_key, state=state, target_path=target_path))
            if action == 'best_plan':
                return finish(self._assistant_smart_best_plan_response(session=session, cache_key=cache_key, state=state, target_path=target_path))
            if action == 'best':
                ...
                self._save_session(cache_key, delegate_state)
                try:
                    result = await self.api_assistant_pick(_JsonRequestShim(request, {'session': session, 'session_id': cache_key, 'choice': 0, 'action': 'best', 'path': target_path, 'compact': compact, 'apikey': self._extract_apikey(request, body)}))
                finally:
                    self._save_session(cache_key, state)
                return finish(result)
            source_states = state.get('source_states') if isinstance(state.get('source_states'), dict) else {}
            requested_mode = self._clean_text(body.get('mode') or body.get('search_mode')).lower()
            mode_aliases = {'盘搜': 'pansou', 'ps': 'pansou', '原生': 'mp_pt', 'mp': 'mp_pt', 'pt': 'mp_pt'}
        ...
        if kind == 'assistant_mp_candidate':
            ...
            if action == 'detail':
                ...
                self._save_session(cache_key, {'candidates': enriched})
                return finish({'success': True, 'message': self._format_mp_candidate_lines(enriched, page=current_page, page_size=page_size), 'data': self._assistant_response_data(session=session, data={'action': 'mp_candidates_detail', 'ok': True, 'page': current_page, 'candidates': enriched})})
            if action == 'next_page':
                total_pages = max(1, (len(candidates) + page_size - 1) // page_size)
                if current_page >= total_pages:
                    return {'success': False, 'message': '已经是最后一页了，可以直接回复编号继续选择。'}
                next_page = current_page + 1
                self._save_session(cache_key, {'page': next_page})
                return finish({'success': True, 'message': self._format_mp_candidate_lines(candidates, page=next_page, page_size=page_size), 'data': self._assistant_response_data(session=session, data={'action': 'mp_candidates_next_page', 'ok': True, 'page': next_page, 'total_pages': total_pages})})
            ...
            if pending_mode == 'mp_download_title':
                result = await self._assistant_smart_resource_execute(request, keyword=search_keyword, session=session, cache_key=cache_key, media_type=self._clean_text(candidate.get('media_type') or state.get('media_type') or 'auto').lower() or 'auto', year=candidate_year or self._clean_text(state.get('year')), source_order=['mp_pt'], target_path=self._clean_text(state.get('target_path')), origin='mp_download_title')
                if result.get('success'):
                    result['message'] = f'已选择：{self._format_candidate_label(candidate)}\n{result.get('message') or ''}'.strip()
                result_data = dict(result.get('data') or {})
            ...
            if pending_mode == 'cloud_transfer_execute':
                ...
                result = await self._assistant_cloud_transfer_execute(request, keyword=search_keyword, session=session, cache_key=cache_key, media_type=self._clean_text(candidate.get('media_type') or state.get('media_type') or 'auto').lower() or 'auto', year=candidate_year or self._clean_text(state.get('year')), source_order=['pansou'], target_path=self._clean_text(state.get('target_path')), session_preference_overrides=overrides, origin=cloud_provider or 'cloud_transfer')
                if result.get('success'):
                    result['message'] = f'已选择：{self._format_candidate_label(candidate)}\n{result.get('message') or ''}'.strip()
                result_data = dict(result.get('data') or {})
                result_data['selected_candidate'] = candidate
                ...
                return finish(result)
            preferences = self._normalize_assistant_preferences((self._assistant_preferences or {}).get(self._normalize_preference_key(session=session)))
            result = await self._assistant_mp_media_search(keyword=search_keyword, session=session, cache_key=cache_key, preferences=preferences)
            result_data = dict(result.get('data') or {})
            result_data['selected_candidate'] = candidate
            result_data['original_keyword'] = self._clean_text(state.get('keyword'))
            result['data'] = result_data
        ...
        if kind == 'assistant_mp':
            ...
            if action == 'next_page':
                ...
                if current_page >= total_pages:
                    return {'success': False, 'message': '已经是最后一页了，可以直接回复编号下载。'}
                next_page = current_page + 1
                all_items = state.get('all_items') if isinstance(state.get('all_items'), list) else []
                if all_items:
                    preview = self._slice_mp_preview_items(all_items, page=next_page, page_size=page_size)
                else:
                    preview = self._mp_search_cache_preview(cache_key, preferences=preferences, page=next_page, page_size=page_size)
                    all_items = self._mp_search_all_preview_items(cache_key, preferences=preferences)
                updated_state = {'items': preview, 'all_items': all_items, 'page': next_page, 'page_size': page_size, 'total': total}
                self._save_session(cache_key, updated_state)
            ...
            if action == 'best':
                result = await self._assistant_mp_best_result_detail(session=session, cache_key=cache_key, preferences=preferences)
                if state.get('recommend_handoff'):
                    result_data = dict(result.get('data') or {})
                    result_data.update(self._assistant_recommend_handoff_short_metadata(state))
                ...
                return finish(result)
            if action == 'best_execute':
                plan_result = await self._assistant_mp_best_download_plan(session=session, cache_key=cache_key, preferences=preferences)
                return finish(await self._assistant_execute_prepared_plan_result(request, session=session, cache_key=cache_key, plan_result=plan_result, message_prefix='已自动选择当前评分最高的 PT 候选并执行计划', extra_data={'source_type': 'mp_pt'}))
            if action == 'best_plan':
                result = await self._assistant_mp_best_download_plan(session=session, cache_key=cache_key, preferences=preferences)
                if state.get('recommend_handoff'):
                    result_data = dict(result.get('data') or {})
                    result_data.update(self._assistant_recommend_handoff_short_metadata(state))
                    result_data['decision_summary'] = self._assistant_recommend_handoff_plan_summary(state)
            ...
            if action == 'plan':
                action = ''
            if action == 'detail':
                return {'success': False, 'message': 'PT 搜索结果不提供详情，请直接回复编号下载。'}
            if index <= 0:
                return {'success': False, 'message': '请选择有效编号，例如：1'}
            result = await self._assistant_mp_download(choice=index, session=session, cache_key=cache_key, preferences=preferences)
            if state.get('recommend_handoff'):
                result_data = dict(result.get('data') or {})
                result_data.update(self._assistant_recommend_handoff_short_metadata(state))
                result_data['decision_summary'] = self._assistant_recommend_handoff_detail_summary(state)
