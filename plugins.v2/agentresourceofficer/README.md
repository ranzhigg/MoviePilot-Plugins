...
把 `飞书命令入口`、`外部智能体`、`盘搜`、`115`、`夸克`、`MoviePilot 原生搜索 / PT 下载` 收进同一套稳定工作流。
...
最近公开 Release：https://github.com/liuyuexi1987/MoviePilot-Plugins/releases/tag/v0.3.1
...
- 你希望智能体不要自己乱拼盘搜、115、夸克接口，而是统一交给插件执行。
...
外部智能体负责理解用户需求和展示结果；资源搜索、转存、下载和 Cookie 修复都交给插件。
...
- `agent-resource-officer skill / helper` 更适合资源流，比如盘搜、编号选择后的 115 / 夸克处理、PT 编号下载、翻页、盘搜详情和 Cookie 修复。
...
- `更新检查 <片名>`、`查更新 <片名>`、`检查 <片名>` 这些旧写法已并回搜索语义；现在直接用 `搜索 / 盘搜搜索 / MP搜索 / PT搜索` 即可。
...
- 云盘结果才有详情卡；想看详情用 `选择 1 详情` 或 `详情 1`。
